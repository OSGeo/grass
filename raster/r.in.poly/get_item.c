...
int get_item(FILE * fd, int format, int *type, int *cat_int,
    double *cat_double, double **x, double **y, int *count,
    ...
    *type = 0;
    ...
    while (1) {
            ...
            if (format == USE_FCELL || format == USE_DCELL) {
                ...
                if (sscanf(buf + 1, "%lf%[^\n]", cat_double, lbl) == 2) {
                    ...
                    if (format == USE_FCELL) {
                        ...
                        Rast_set_d_cat((DCELL *) cat_double,
                            (DCELL *) cat_double, lbl, labels);
                    }
                }
                continue;
            }
        ...
        if (sscanf(buf, "%s %s", east, north) != 2) {
            G_warning(_("Illegal coordinate <%s, %s>, skipping."), east,
                north);
            continue;
        }
