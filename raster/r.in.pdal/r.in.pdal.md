...
*[v.in.pdal](v.in.pdal.html)* is that *r.in.pdal* creates a raster
instead of just importing the points into GRASS GIS. However,
*r.in.pdal* does not merely rasterizes the points from the point cloud.
*r.in.pdal* uses binning to derive values for individual raster cells,
...
:   This computes the variance of point values for each cell. Variance
    and derivatives use the biased estimator (n). It is calculated by
    Welford algorithm.
...
mode
:   This computes the mode (the most common value) of point values for
    each cell. If there are more than one mode for a cell, an arbitrary
...
sidnmax
:   Maximum number of points in cell per source ID. Points are counted
    for each source separately and largest count is used as the final
    cell value. Corresponds to majority density (*Dmax*) as defined by
    Smeeckaert et al., 2013.

sidnmin
:   Minimum number of points in cell per source ID. Points are counted
    for each source separately and smallest count is used as the final
    cell value. Corresponds to minority density (*Dmin*) as defined by
    Smeeckaert et al., 2013.
...
various attributes can be found in LAS Specification chapter \"Point
...
Lab](http://geospatial.ncsu.edu/osgeorel/) (base_raster option,
documentation),\
