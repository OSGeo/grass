...
class BasicTest(TestCase):
    ...
    def tearDown(self):
        ...
        """
        self.runModule("g.remove", flags="f", type="raster", name=self.basic_raster)
    ...
    def test_method_count(self):
        ...
        self.assertModule(
            "r.in.pdal",
            input=self.las_file,
            output=self.basic_raster,
            flags="eo",
            method="n",
        )
        self.assertRasterExists(self.basic_raster)
        self.assertRasterMinMax(map=self.basic_raster, refmin=0, refmax=self.npoints)
    ...
    def test_method_mean(self):
        ...
        self.assertRasterExists(self.basic_raster)
        self.assertRasterMinMax(map=self.basic_raster, refmin=200, refmax=500)
