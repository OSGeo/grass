...
def temp_region():
    ...
    gs.run_command(
        "g.remove", type="region", name=temp_region_name, flags="f", quiet=True
    )
...
def cleanup_test_maps():
    ...
    try:
        ...
        if current_mapset in rasters and rasters[current_mapset]:
            ...
            if test_maps:
                gs.run_command(
                    "g.remove", type="raster", name=test_maps, flags="f", quiet=True
                )
    except Exception:
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "requires_location: marks tests that require specific GRASS location"
    )
