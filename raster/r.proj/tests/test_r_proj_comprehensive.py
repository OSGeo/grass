...
def utm_location():
    ...
    if not os.path.exists(location_path):
        try:
            ...
            gs.run_command(
                "g.proj", flags="c", project=location_name, epsg=26917, quiet=True
            )
        except:
            # Fallback to legacy parameter name
            gs.run_command(
                "g.proj", flags="c", location=location_name, epsg=26917, quiet=True
            )
...
def latlon_location():
    ...
    if not os.path.exists(location_path):
        try:
            # Try newer parameter name first
            gs.run_command(
                "g.proj", flags="c", project=location_name, epsg=4326, quiet=True
            )
        except:
            # Fallback to legacy parameter name
            gs.run_command(
                "g.proj", flags="c", location=location_name, epsg=4326, quiet=True
            )
...
def utm_test_raster(utm_location):
    ...
    # Set region
    gs.run_command(
        "g.region", n=4000000, s=3900000, e=650000, w=550000, res=100, flags="p"
    )

    # Create test raster
    raster_name = "test_utm_raster"
    gs.run_command(
        "r.mapcalc", expression=f"{raster_name} = row() * 10 + col()", overwrite=True
    )
...
class TestRProjCRSTransformations:
    ...
    @pytest.mark.slow
    def test_print_bounds_flag_p(
        self, utm_test_raster, latlon_location, preserve_location
    ):
        """
        ...
        try:
            ...
            assert (
                "north" in result_lower or "n" in result_lower
            ), "Should show north boundary"
            assert (
                "south" in result_lower or "s" in result_lower
            ), "Should show south boundary"
    ...
    def test_location_creation_latlon(self, latlon_location):
        ...
        assert os.path.isdir(
            location_path
        ), "Lat/Lon test location should be a directory"

    def test_parameter_naming_compatibility(
        self, utm_test_raster, latlon_location, preserve_location
    ):
        """
        ...
        try:
            ...
            assert (
                modern_works or legacy_works
            ), "Either 'project' or 'location' parameter should work"
