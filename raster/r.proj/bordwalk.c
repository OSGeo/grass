...
static int inside(const struct Cell_head *ref_hd, double hx, double hy)
{
    return
        (hx <= ref_hd->east) &&
        (hx >= ref_hd->west) && (hy <= ref_hd->north) && (hy >= ref_hd->south);
}
...
static void proj_update(const struct pj_info *from_pj,
    const struct pj_info *to_pj, const struct pj_info *trans_pj, int dir,
    struct Cell_head *to_hd, double hx, double hy)
{
    if (GPJ_transform(from_pj, to_pj, trans_pj, dir, &hx, &hy, NULL) < 0) {
        G_fatal_error(_("unable to transform coordinates %g, %g"), hx, hy);
        return;
    }
    update(to_hd, hx, hy);
}
    ...
    const struct Cell_head *from_hd, struct Cell_head *to_hd)
{
    ...
    for (idx = from_hd->west + from_hd->ew_res / 2; idx < from_hd->east;
        ...
        proj_update(from_pj, to_pj, trans_pj, dir, to_hd, idx,
            from_hd->north - from_hd->ns_res / 2);
    ...
    for (idx = from_hd->north - from_hd->ns_res / 2; idx > from_hd->south;
        idx -= from_hd->ns_res)
        proj_update(from_pj, to_pj, trans_pj, dir, to_hd,
            from_hd->east - from_hd->ew_res / 2, idx);
    ...
    for (idx = from_hd->east - from_hd->ew_res / 2; idx > from_hd->west;
        idx -= from_hd->ew_res)
        proj_update(from_pj, to_pj, trans_pj, dir, to_hd, idx,
            from_hd->south + from_hd->ns_res / 2);
    ...
    for (idx = from_hd->south + from_hd->ns_res / 2; idx < from_hd->north;
        idx += from_hd->ns_res)
        proj_update(from_pj, to_pj, trans_pj, dir, to_hd,
            from_hd->west + from_hd->ew_res / 2, idx);

    debug("Left", to_hd);
}

static int proj_inside(const struct pj_info *from_pj,
    const struct pj_info *to_pj, const struct pj_info *trans_pj, int dir,
...
static void reverse_check(const struct pj_info *from_pj,
    ...
    const struct Cell_head *to_hd, struct Cell_head *cur_hd)
{
    ...
    const struct pj_info *trans_pj, int dir)
{
    ...
    /* Right */
    for (idx = from_hd->north; idx > from_hd->south; idx -= from_hd->ns_res)
    ...
    /* Left */
    for (idx = from_hd->south; idx < from_hd->north; idx += from_hd->ns_res)
