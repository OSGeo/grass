...
static struct map_store *get_parameter_input(const char *type,
    char **map_names, char **positions, char *file, int *number_of_maps);
static void linear_interpolation(struct map_store **inp, int num_inputs,
    struct map_store **outp, int num_outputs);
static void interpolate_row_linear(struct map_store *left,
    struct map_store *right, struct map_store *out, int ncols);
static void start_interpolation(struct map_store *inputs, int num_inputs,
    struct map_store *outputs, int num_outputs, int interpol_method);
...
int main(int argc, char *argv[])
{
    ...
    parm.samplingpos->description =
        _("Sampling point position for each output map");
    ...
    inputs =
        ...
        parm.datapos->answers, parm.infile->answer, &num_inputs);
    outputs =
        get_parameter_input("output", parm.output->answers,
        parm.samplingpos->answers, parm.outfile->answer, &num_outputs);
...
struct map_store *get_parameter_input(const char *type, char **map_names,
    char **positions, char *file, int *number_of_maps)
{
    ...
    if (file) {
        ...
        for (;;) {
            ...
            p->buf = NULL;
            p->has_run = 0;
            G_verbose_message(_("Preparing %s map <%s> at position %g"), type,
                p->name, p->pos);
        }

        if (num_maps < 1)
            G_fatal_error(_("No raster map name found in %s file <%s>"), type,
                file);
...
void start_interpolation(struct map_store *inputs, int num_inputs,
    struct map_store *outputs, int num_outputs, int interpol_method)
{
    int i;
 ...
 *
 * */
void interpolate_row_linear(struct map_store *left, struct map_store *right,
    struct map_store *out, int ncols)
{
    DCELL v;
    DCELL u1;
    DCELL u2;
    ...
    for (col = 0; col < ncols; col++) {
        ...
        if (Rast_is_d_null_value(&u1) || Rast_is_d_null_value(&u2)) {
            ...
            v = (1 - ((out->pos - left->pos) / dist)) * u1 + ((out->pos -
                    left->pos) / dist) * u2;
        }
        out->buf[col] = v;
    }
