...
static CELL do_renumber(int *in_fd, DCELL * rng, int nin,
    int diag, int minsize, int cfd, CELL label, CELL * index, int out_fd)
{
...
static double get_diff2(DCELL ** a, int acol, DCELL ** b, int bcol,
    DCELL * rng, int n)
{
    int i;
    double diff, diff2;
    ...
    for (row = 0; row < nrows; row++) {
        ...
        for (col = 1; col <= ncols; col++) {
            ...
            if (diag) {
                ...
                bcol = col + 1;
                do {
                    if (get_diff2(cur_in, col, prev_in, bcol, rng,
                            nin) <= thresh2) {
                        cur_clump[col] = *temp_clump;
                        if (OLD == 0) {
                            OLD = *temp_clump;
