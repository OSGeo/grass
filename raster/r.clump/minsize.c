            ...
            if (!rbtree_find(visited, &rc)) {
                ...
                if (ngbr_id == this_id) {
                    ...
                    if (rng) {
                        ...
                        if (!have_Ri) {
                            boffset =
                                (off_t) sizeof(DCELL) * (next.row * ncols +
                                next.col);
                        ...
                        boffset =
                            (off_t) sizeof(DCELL) * (rown * ncols + coln);
                        lseek(bfd, boffset, SEEK_SET);
...
int merge_small_clumps(int *in_fd, int nin, DCELL * rng,
    int diag, int minsize, CELL * n_clumps, int cfd, int out_fd)
{
    ...
    if (rng) {
        ...
        for (row = 0; row < nrows; row++) {
            ...
            for (col = 0; col < ncols; col++) {
                ...
                if (write(bfd, bbuf,
                        sizeof(DCELL) * nin) != (int)(sizeof(DCELL) * nin))
                    G_fatal_error(_("Unable to write to temp file"));
            }
        }
        G_percent(row, nrows, 2);
        ...
        for (col = 0; col < ncols; col++) {
            ...
            while (reg_size < minsize && best_id > 0) {
                ...
                if (best_id > 0) {
                    ...
                    update_cid_box(cfd, rowmin, rowmax, colmin, colmax,
                        this_id, best_id);
                    /* mark as merged */
