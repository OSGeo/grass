...
void wtrshed(int fm, int fd, int nl, int ns, int mxbuf)
{
	...
	for (i = 0; i < mxbuf; i++) {
	    ...
	    if (read(fm, bas[i].p, bufsz) == -1)
                G_fatal_error(_("Error #%d reading bas"), errno);

	    lseek(fd, dir[i].offset, SEEK_SET);
	    if (read(fd, dir[i].p, bufsz) == -1)
                G_fatal_error(_("Error #%d reading dir"), errno);
	...
	for (i = 1; i < nl - 1; i += 1) {
	    ...
	    if (write(fm, bas[sline].p, bufsz) == -1)
                G_fatal_error(_("Error #%d writing bas"), errno);
		...
		if (read(fm, bas[mxbuf - 1].p, bufsz) == -1)
                    G_fatal_error(_("Error #%d reading bas"), errno);

		lseek(fd, dir[mxbuf - 1].offset, SEEK_SET);
		if (read(fd, dir[mxbuf - 1].p, bufsz) == -1)
                    G_fatal_error(_("Error #%d reading dir"), errno);
	...
	for (i = nl - 2; i >= 1; i -= 1) {
	    ...
	    if (write(fm, bas[nline - 1].p, bufsz) == -1)
                G_fatal_error(_("Error #%d reading bas"), errno);
		...
		if (read(fm, bas[0].p, bufsz) == -1)
                    G_fatal_error(_("Error #%d reading bas"), errno);

		lseek(fd, dir[0].offset, SEEK_SET);
		if (read(fd, dir[0].p, bufsz) == -1)
                    G_fatal_error(_("Error #%d reading dir"), errno);
