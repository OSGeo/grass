...
void flink(int i, int j, int nl, int ns, CELL *p1, CELL *p2, CELL *p3, int *active, int *goagain)
{
   ...
   cwork=p2[j];
   if(G_is_c_null_value(p2+j) || cwork>=0 || cwork == -256)return;
   cwork=-cwork;
...
/* look one row back */
   cwork=p1[j-1];
   if(cwork>0 && cwork!=4 && c[6])outflow+=64;
   cwork=p1[j];
   if(cwork>0 && cwork!=8 && c[7])outflow+=128;
   cwork=p1[j+1];
   if(cwork>0 && cwork!=16 && c[0])outflow+=1;

/* look at this row */
   cwork=p2[j-1];
   if(cwork>0 && cwork!=2 && c[5])outflow+=32;
   cwork=p2[j+1];
   if(cwork>0 && cwork!=32 && c[1])outflow+=2;

/* look one row forward */
   cwork=p3[j-1];
   if(cwork>0 && cwork!=1 && c[4])outflow+=16;
   cwork=p3[j];
   if(cwork>0 && cwork!=128 && c[3])outflow+=8;
   cwork=p3[j+1];
   if(cwork>0 && cwork!=64 && c[2])outflow+=4;
...
/* select a direction when there are multiple non-flat links */
   ...
   for(i=1;i<nl-1;i+=1)
   {
      ...
      {
         offset=j*isz;
         if( G_is_c_null_value((CELL *)(bnd->b[0]+offset)) )continue;
         memcpy(&cvalue,bnd->b[0]+offset,isz);
         if(cvalue > 0)cvalue=select_dir(cvalue);
   ...
   do
   {
      ...
      for(i=1;i<nl-1;i++)
      {
         ...
         do
         {
            ...
            {
               flink(i,j,nl,bnd->ns,
                  (CELL *)bnd->b[0],(CELL *)bnd->b[1],(CELL *)bnd->b[2],
                  &active[i],&goagain);
               if(goagain)activity=1;
            }
         }while(goagain);
