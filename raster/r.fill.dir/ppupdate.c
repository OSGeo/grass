	      ...
	      struct band3 *basins)
{
    ...
    lseek(fb, 0, SEEK_SET);
    for (i = 0; i < nl; i += 1) {
	if (read(fe, elev->b[1], elev->sz) == -1)
            G_fatal_error(_("Error #%d reading elevation"), errno);
	if (read(fb, basins->b[1], basins->sz) == -1)
            G_fatal_error(_("Error #%d reading basins"), errno);
	...
	if (write(fe, elev->b[1], elev->sz) == -1)
            G_fatal_error(_("Error #%d writing elevation"), errno);
    }
