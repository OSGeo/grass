...
template <class T>
AMI_STREAM<T> *cell2stream(char *cellname, elevation_type T_max_value,
                           long *nodata_count)
{
    Rtimer rt;
    AMI_err ae;
    ...
    *nodata_count = 0;
    ...
    AMI_STREAM<T> *str = new AMI_STREAM<T>();
    {
        ...
        str->name(&foo);
        if (stats)
            *stats << "Reading raster map <" << cellname << "> to stream <"
                   << foo << ">." << endl;
        G_verbose_message(_("Reading data from <%s> to stream <%s>"), cellname,
                          foo);
    }
...
/* ---------------------------------------------------------------------- */
template <class T>
void stream2_CELL(AMI_STREAM<T> *str, dimension_type nrows,
                  dimension_type ncols, char *cellname, bool usefcell = false)
{
    Rtimer rt;
    AMI_err ae;
    ...
    str->seek(0);
    {
        ...
        str->name(&foo);
        if (stats)
            *stats << "Writing stream <" << foo << "> to raster map <"
                   << cellname << ">.\n";
    }
    ...
    for (int i = 0; i < nrows; i++) {
        for (int j = 0; j < ncols; j++) {
            ...
            if (ae != AMI_ERROR_NO_ERROR) {
                str->sprint();
                G_fatal_error(
                    _("stream2cell: Reading stream failed at (%d,%d)"), i, j);
            }
...
void stream2_CELL(AMI_STREAM<T> *str, dimension_type nrows,
                  dimension_type ncols, FUN fmt, char *cellname)
{
...
/* ---------------------------------------------------------------------- */
template <class T, class FUN>
void stream2_FCELL(AMI_STREAM<T> *str, dimension_type nrows,
                   dimension_type ncols, FUN fmt, char *cellname)
{
...
void stream2_FCELL(AMI_STREAM<T> *str, dimension_type nrows,
                   dimension_type ncols, FUN1 fmt1, FUN2 fmt2, char *cellname1,
                   char *cellname2)
{
    Rtimer rt;
    AMI_err ae;
    ...
    ae = str->read_item(&elt);
    assert(ae == AMI_ERROR_NO_ERROR || ae == AMI_ERROR_END_OF_STREAM);
    G_important_message(_("Writing to raster maps <%s,%s>..."), cellname1,
                        cellname2);
