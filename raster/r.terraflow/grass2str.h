        ...
        for (int j = 0; j < ncols; j++) {
            ...
            if (isnull) {
                ...
                if ((d > (DCELL) T_max_value) || (d < (DCELL) T_min_value)) {
                    G_fatal_error
                        ("Value out of range, reading raster map <%s> "
                        "at (i=%d, j=%d) value=%.1f", cellname, i, j, d);
                }
            }
            /* write x to stream */
            ae = str->write_item(x);
    ...
    for (int i = 0; i < nrows; i++) {
        for (int j = 0; j < ncols; j++) {
            ...
            if (ae != AMI_ERROR_NO_ERROR) {
                str->sprint();
                G_fatal_error(_("stream2cell: Reading stream failed at (%d,%d)"), i, j);
            }
...
template < class T, class FUN >
    void
stream2_FCELL(AMI_STREAM < T > *str, dimension_type nrows,
