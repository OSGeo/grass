...
template < class T > void printStream(ostream & s, AMI_STREAM < T > *str)
{
    T *elt;
    AMI_err ae;
...
printStream2Grid(AMI_STREAM < T > *str,
    dimension_type nrows, dimension_type ncols, const char *name, FUN fmt)
{
    T *elt, nodata;
    AMI_err ae;
    ofstream fstrm(name);
...
template < class T, class FUN >
    ...
    dimension_type nrows, dimension_type ncols, char *name, FUN fmt)
{
    T *elt;
    AMI_err ae;
    ofstream fstrm(name);
...
template < class T, class TT, class FUN >
    AMI_STREAM < T > *mergeStream2Grid(AMI_STREAM < T > *grid,
    dimension_type rows, dimension_type cols, AMI_STREAM < TT > *str, FUN fo)
{
    AMI_err ae, aeS;
    T *gep;                     /* grid element */
