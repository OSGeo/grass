...
public:
    ...
    float get(const unsigned short i) const { return weight.get(i); }

    /* return the total contour */
    float totalContour() const { return sumcontour; }

    float totatWeight() const { return sumweight; }

    float dx() const { return cell_dx; }

    float dy() const { return cell_dy; }
