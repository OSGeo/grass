...
/************************************************************/
template < class T > unionFind < T >::unionFind()
{
...
/************************************************************/
/* return true if element x is in the structure */
template < class T > inline bool unionFind < T >::inSet(T x)
{
...
/************************************************************/
template < class T > void unionFind < T >::print()
{
...
template < class T > inline void unionFind < T >::makeSet(T x)
{
    /* cout << "makeSet " << x << "\n"; print(); */
    assert(x > 0);
    if (x >= maxsize) {
...
/************************************************************/
/* returns the representative of the set containing x */
template < class T > inline T unionFind < T >::findSet(T x)
{
    /* valid entry */
    assert(inSet(x));
    if (parent[x] != x) {
...
/************************************************************/
/* unites the dynamic sets that contain x and y; */
template < class T > inline void unionFind < T >::makeUnion(T x, T y)
{
...
/************************************************************/
/* returns main memory usage if the max nb of makeset calls is n */
template < class T > inline size_t unionFind < T >::mmusage(T n)
{
    if (n < UNION_INITIAL_SIZE)
        n = UNION_INITIAL_SIZE;
    return (n * sizeof(T));
}
