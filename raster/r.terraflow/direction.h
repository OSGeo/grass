...
public:
    ...
    /* Modifies the window by setting to 1 only the neighbors to which
       ...
       16 *   1
       8  4   2 */
    directionWindow(direction_type dir) : genericWindow<bool>()
    {
    ...
    void checkDirection(short di, short dj, int skipit, elevation_type el,
                        elevation_type elneighb) const
    {
#ifndef NDEBUG
        if (skipit == 1) {
    ...
    void correctDirection(short di, short dj, int skipit, dimension_type i,
                          dimension_type j, elevation_type elev_crt,
                          direction_type dir, elevation_type elev_neighb)
    {

        if (skipit && get(di, dj) == true) {
            cout << "WARNING:  at (" << i << "," << j << " , h=" << elev_crt
                 << ", dir=" << dir << ")"
                 << "direction points to non-valid neighbor (" << i + di << ","
                 << j + dj << ", h=" << elev_crt - elev_neighb << ")\n";
...
direction_type encodeDirection(const genericWindow<elevation_type> &elevwin,
                               ...
                               const dimension_type ncols, dimension_type row,
                               dimension_type col);
