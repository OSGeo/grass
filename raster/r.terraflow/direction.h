        ...
        elevation_type elev_neighb)
    {
        ...
        if (skipit && get(di, dj) == true) {
            cout << "WARNING:  at ("
                ...
                ")" << "direction points to non-valid neighbor (" << i +
                di << "," << j + dj << ", h=" << elev_crt -
                elev_neighb << ")\n";
...
direction_type encodeDirection(const genericWindow < elevation_type > &elevwin,
    ...
    const dimension_type ncols, dimension_type row, dimension_type col);

direction_type encodeDirectionMFD(const genericWindow < elevation_type >
    &elevwin, const dimension_type nrows, const dimension_type ncols,
    dimension_type row, dimension_type col);

direction_type encodeDirectionSFD(const genericWindow < elevation_type >
    &elevwin, const dimension_type nrows, const dimension_type ncols,
    dimension_type row, dimension_type col);
