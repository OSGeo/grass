...
class flowPriority
{
  ...
  public:
                 flowPriority(elevation_type a = 0, toporank_type b = 0, dimension_type c = 0, dimension_type d = 0):
    h(a), toporank(b), i(c),
        j(d) {
    }

    flowPriority(const flowPriority & p):h(p.h), toporank(p.toporank), i(p.i),
        j(p.j)
    {
    }
...
template < class T > class sweepItemBaseType {
  public:
    dimension_type i, j;
  ...
  /***************************************************************/
    sweepItemBaseType(const dimension_type & gi, const dimension_type & gj,
        ...
        T * a2,
        T * b2,
        T * c2):i(gi), j(gj), dir(gdir), elevwin(a1, b1, c1), toporwin(a2, b2,
        c2)
    {
    }
