...
int fillPriority::compare(const fillPriority &a, const fillPriority &b)
{
    if (a.el < b.el)
        return -1;
    if (a.el > b.el)
...
int fillPriority::qscompare(const void *a, const void *b)
{
...
int operator<(const fillPriority &a, const fillPriority &b)
{
    if (a.el < b.el)
        return 1;
    if (a.el > b.el)
...
int operator==(const fillPriority &a, const fillPriority &b)
{
    return (a.el == b.el) && (a.depth == b.depth) && (a.i == b.i) &&
           (a.j == b.j);
}
...
ostream &operator<<(ostream &s, const fillPriority &p)
{
    return s << "[fillPriority el=" << p.el << ", d=" << p.depth << ", " << p.i
             << "," << p.j << "]";
}

/* ********************************************************************** */

ostream &operator<<(ostream &s, const labelElevType &p)
{
...
char *boundaryType::print(const boundaryType &p)
{
    static char buf[4];
    if (p.isValid()) {
        buf[0] = '1';
...
ostream &operator<<(ostream &s, const boundaryType &p)
{
    return s << "[boundaryType " << (labelElevType)p << ", " << p.label2 << "]";
}
...
void waterWindower::processWindow(dimension_type i, dimension_type j,
                                  waterGridType &point, waterWindowBaseType *a,
    ...
    for (dimension_type row = 0; row < nrows; row++) {
        for (dimension_type col = 0; col < ncols; col++) {
...
#ifndef NDEBUG
            {
                ...
                XXX winp->sanityCheck();
                assert(pq->is_empty() ||
                       winp->getPriority() <= tmp.getPriority());
            }
#endif
            while (pq->min(plabel) &&
...
#endif
                if (IS_BOUNDARY(winp->i, winp->j, nrows,
                                ncols)) { /* edge of grid */
                    ...
                    for (int j = -1; j < 2; j++) {
                        ...
                        if (!is_nodata(winp->getElevation(k)) &&
                            ...
                            if (winp->getPriority() < prio) {
                                ...
                                cerr << "WARNING: time travel attempted"
                                     << endl;
                                cerr << "inst priority is " << prio << endl;
                                cerr << "source is " << *winp
                                     << "; prio=" << winp->getPriority()
                                     << endl;
                                assert(0);
                            }
#else
...
private:
    ...
    void processPair(labelElevType &pt, dimension_type i, dimension_type j,
                     labelElevType &n);
...
void boundaryDetector::processPair(labelElevType &pt, dimension_type i,
                                   dimension_type j, labelElevType &n)
{
    ...
    else if (IS_BOUNDARY(i, j, nrows, ncols) &&
             pt.getLabel() != LABEL_BOUNDARY) {
...
void boundaryDetector::processWindow(dimension_type i, dimension_type j,
                                     labelElevType &point, labelElevType *a,
                                     labelElevType *b, labelElevType *c)
{
    if (point.getLabel() == LABEL_UNDEF)
        return;
    /* NODATA_FIX */
                    ...
                    AMI_STREAM<boundaryType> *boundaryStr)
{
    ...
    /*  NODATA_FIX
            ...
            boundaryType bt(-1, -1, ELEVATION_MIN, LABEL_BOUNDARY,
       LABEL_NODATA); AMI_err ae = boundaryStr->write_item(bt); assert(ae ==
       AMI_ERROR_NO_ERROR);
    */
}
...
int compressedWaterWindowBaseType::computeDelta(waterWindowBaseType *center,
                                                int index,
                                                waterWindowBaseType *p) const
{
    ...
    if (index > 7)
        return 0; /* we store our depth elsewhere */
...
compressedWaterWindowBaseType::compressedWaterWindowBaseType(
    dimension_type gi, dimension_type gj, waterWindowBaseType *a,
...
fillPriority compressedWaterWindowBaseType::getPriority() const
{
    return fillPriority(getElevation(), getDepth(), i, j);
}

bfs_depth_type compressedWaterWindowBaseType::getDepth(int k) const
{
    if (getElevation() != getElevation(k))
        return DEPTH_INITIAL;
    return depth + ((depth_delta >> (norm(k) * 2)) & 0x3) - 1;
}

void compressedWaterWindowBaseType::sanityCheck()
{
    assert(i >= -1);
    assert(j >= -1);
...
ostream &operator<<(ostream &s, const compressedWaterWindowBaseType &p)
{
    return s << "[compressedWaterWindowBaseType " << p.i << "," << p.j << " "
             << directionSymbol(p.getDirection()) << " e=" << p.getElevation()
             << " d =" << p.getDepth() << "]";
}

/* ********************************************************************** */

labelElevType compressedWaterWindowType::getCenter() const
{
    return labelElevType(i, j, getElevation(), label);
}

void compressedWaterWindowType::sanityCheck()
{
    assert(label >= LABEL_UNDEF);
    compressedWaterWindowBaseType::sanityCheck();
}

ostream &operator<<(ostream &s, const compressedWaterWindowType &p)
{
    return s << "[compressedWaterWindowType " << p.i << "," << p.j << "  "
             << directionSymbol(p.getDirection()) << " e=" << p.getElevation()
             << " d=" << p.getDepth() << " l=" << p.label;
}
