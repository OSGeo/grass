...
class labelElevType:public ijBaseType
{
  ...
  labelElevType(dimension_type gi, dimension_type gj, elevation_type gel, cclabel_type glabel):
    ijBaseType(gi, gj), el(gel),
        label(glabel) {
    };

    cclabel_type getLabel() const
    {
        return label;
    };
...
class boundaryType:public labelElevType
{
  ...
  boundaryType(dimension_type gi, dimension_type gj, elevation_type gel, cclabel_type glabel1, cclabel_type glabel2):
    labelElevType(gi, gj, gel, glabel1),
        label2(glabel2) {
  ...
  :    labelElevType(let) {
    ...
    bool isValid() const
    {
        return label2 != LABEL_UNDEF;
    }
    cclabel_type getLabel1() const
    {
        return getLabel();
    }
    cclabel_type getLabel2() const
    {
        return label2;
    }
...
class fillPriority:public ijBaseType
{
  ...
  fillPriority(elevation_type gel, bfs_depth_type gdepth, dimension_type gi, dimension_type gj):
    ijBaseType(gi, gj), el(gel),
        depth(gdepth) {
    }
...
class fillPLabel:public fillPriority
{
  ...
  public:
    ...
    fillPLabel(const fillPriority & gpriority,
        const cclabel_type glabel):fillPriority(gpriority), label(glabel)
    {
    }
    fillPriority getPriority() const
    {
        return (fillPriority) (*this);
    }
    cclabel_type getLabel() const
    {
        return label;
...
class waterWindowBaseType
{
  ...
  waterWindowBaseType(elevation_type gel, direction_type gdir, bfs_depth_type gdepth):
    el(gel), dir(gdir),
        depth(gdepth) {
    };
    friend int
        operator==(const waterWindowBaseType & a,
...
class waterType:public ijBaseType
{
  ...
  protected:
    ...
    cclabel_type label;
  public:
                 waterType():label(LABEL_UNDEF)
    {
    };                          /* needed to sort */

  waterType(dimension_type gi, dimension_type gj, direction_type gdir, cclabel_type glabel = LABEL_UNDEF, bfs_depth_type gdepth = DEPTH_INITIAL):
    ijBaseType(gi, gj), dir(gdir), depth(gdepth),
        label(glabel) {
    }
...
{
  public:
    direction_type operator() (waterType p)
    {
        return p.getDirection();
    }
    direction_type operator() (direction_type p)
    {
        return p;
    }
};
...
class waterGridType:public waterWindowBaseType
{
  ...
  waterGridType(elevation_type gel, direction_type gdir = DIRECTION_UNDEF, cclabel_type glabel = LABEL_UNDEF, bfs_depth_type gdepth = DEPTH_INITIAL):
    waterWindowBaseType(gel, gdir, gdepth),
        label(glabel) {
    }
    waterGridType(elevation_type gel,
        const waterType & w):waterWindowBaseType(gel, w.dir, w.depth),
        label(w.label)
    {
    }
    ...
    void setLabel(cclabel_type lbl)
    {
        label = lbl;
    };
...
class packed8bit
{
  ...
  public:
    ...
    void setBit(int k, int v = 1) {
        value = (int)value | ((v ? 1 : 0) << k);
    };
    void resetBit(int k)
    {
        value &= ~(0x1 << k);
...
class compressedWaterWindowBaseType:public ijBaseType
{
  ...
  public:
    ...
    fillPriority getPriority() const;
    elevation_type getElevation(int k = 4) const
    {
        return el[k];
    };
...
class compressedWaterWindowType:public compressedWaterWindowBaseType
{
  ...
  :    compressedWaterWindowBaseType(gi, gj, a, b, c), label(glabel) {
    ...
    friend ostream & operator<<(ostream & s,
        const compressedWaterWindowType & p);
};
