...
class plateauType:public ijBaseType
{
  public:                      /* struct, so members public */
    ...
    bool valid;
  public:
       plateauType(dimension_type gi, dimension_type gj, direction_type gdir, cclabel_type gcclabel = LABEL_UNDEF):
    ijBaseType(gi, gj), cclabel(gcclabel), dir(gdir),
        valid(true) {
    };
...
class plateauStats
{
  ...
  public:
    ...
    plateauStats(cclabel_type l):
        ...
        jMin(dimension_type_max), jMax(0), size(0), label(l), hasSpill(false)
    {
    };
