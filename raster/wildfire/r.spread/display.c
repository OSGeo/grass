...
*/
	...
	R_move_abs (l, t+(b-t)/20); R_cont_abs (r, t+(b-t)/20);
	R_move_abs (r, t+(b-t)/10); R_cont_abs (l, t+(b-t)/10);
	R_move_abs (l+(r-l)/2, t); R_cont_abs (l+(r-l)/2, t+(b-t)/10);

	/*logo box*/
	x1 = l; 		y1 = t; 
	x2 = l + 0.5*(r - l); 	y2 = t + 0.05*(b - t); 
	R_standard_color (5);
	...
	x1_st = l + 0.5*(r - l); 	y1_st = t; 
	x2_st = r; 			y2_st = t + 0.05*(b - t); 
...
*/	R_standard_color (9);
	...
	R_move_abs ((int)(x1_st + 0.03*(x2_st - x1_st)), (int)(y1_st + 0.75*(y2_st - y1_st)));
	R_standard_color (1); 	
	R_text ("Elapsed Spread Time 00:00");

	/*starting watch-time box*/
	x1 = l; 		y1 = t + 0.05*(b - t); 
	x2 = l + 0.5*(r - l); 	y2 = t + 0.1*(b - t); 
	R_standard_color (9);
	...
	x1_ct = l + 0.5*(r - l); 	y1_ct = t + 0.05*(b - t); 
	x2_ct = r; 			y2_ct = t + 0.1*(b - t); 
	R_standard_color (9);
	R_box_abs (x1_ct+1, y1_ct+1, x2_ct, y2_ct-1);
	R_text_size ((int)(0.05*(x2_ct - x1_ct)), (int)(0.5*(y2_ct - y1_ct)));
	R_move_abs ((int)(x1_ct + 0.03*(x2_ct - x1_ct)), (int)(y1_ct + 0.75*(y2_ct - y1_ct)));
	R_standard_color (8); 
...
{	
	...
	if (cell_value > old_value)
	{
		...
		R_move_abs ((int)(x1_st + 0.03*(x2_st - x1_st)), (int)(y1_st + 0.75*(y2_st - y1_st)));
		R_standard_color (1); 	
		sprintf (buf, "Elapsed Spread Time %d%d:%d%d", (cell_value/600), (cell_value/60 - cell_value/600*10), ((cell_value - cell_value/60*60)/10), ((cell_value - cell_value/60*60) - (cell_value - cell_value/60*60)/10*10));
		R_text (buf);
