...
main (int argc, char *argv[])
{
	...
	G1, G2, G3, G4, G5,
	wo_dead[14],			/*dead fuel total load*/
	wn_dead,			/*net dead fuel total load*/
	wn_live,			/*net live fuel (total) load*/
	class_sum,
	moisture[4],			/*moistures of 1-h,10-h,100-h,live fuels*/
	...
	*spotdist=NULL;	/*cell buffer for max spotting distance map layer*/
	...
	for (model = 1; model <= 13; model++) {
		...
		for (class = 0; class <= 3; class++) {
			Ffactor_all[class][model] = WO[class][model]*SIGMA[class][model]/class_sum;
			sigma[model] = sigma[model] + SIGMA[class][model]*Ffactor_all[class][model];
		}
		class_sum = 0.0;
		...
		for (class = 0; class <= 2; class++) {
			Ffactor_in_dead[class][model] = WO[class][model]*SIGMA[class][model]/class_sum;
		}
		...
		Ffactor_dead[model] = class_sum/(class_sum + WO[3][model]*SIGMA[3][model]);
	...
	for ( row=0 ; row < nrows ; row++ ) {
		...
		for (col = 0; col < ncols; col++) {
			...
			if (!(parm.mois_1h->answer) && parm.mois_10h->answer && parm.mois_100h->answer) 
				moisture[0] = moisture[1] - 0.01;
			if (!(parm.mois_10h->answer) && parm.mois_1h->answer && parm.mois_100h->answer) 
				moisture[1] = moisture[0] + 0.01;
			if (!(parm.mois_100h->answer) && parm.mois_1h->answer && parm.mois_10h->answer) 
				moisture[2] = moisture[1] + 0.01;
			...
			if (SIGMA[3][fuel[col]] > 0.0) {
				for (class = 0; class <= 2; class++) {
					if (SIGMA[class][fuel[col]] == 0.0)
						continue;
					fined = fined + WO[class][fuel[col]]*exp(-138.0/SIGMA[class][fuel[col]]);
                                        wmfd = wmfd + WO[class][fuel[col]]*exp(-138.0/SIGMA[class][fuel[col]])*moisture[class];
				}
				fdmois = wmfd/fined;
				finel = WO[3][fuel[col]]*exp(-500.0/SIGMA[3][fuel[col]]);
				xmext = 2.9*(fined/finel)*(1 - fdmois/MX[fuel[col]]) - 0.226;
                        } else 
				xmext = MX[fuel[col]];
			...
			Mf_dead = 0.0; wn_dead = 0.0; class_sum = 0.0;
                        for (class = 0; class <= 2; class++) {
				Mf_dead = Mf_dead + moisture[class]*Ffactor_in_dead[class][fuel[col]];
				wn_dead = wn_dead + WO[class][fuel[col]]*Gfactor_in_dead[class][fuel[col]]*(1 - ST);
				Qig[class] = 250 + 1116*moisture[class];
				class_sum = class_sum + Ffactor_all[class][fuel[col]]*epsilon[class][fuel[col]]*Qig[class]; 
			}
			etaM_dead = 1.0 - 2.59*(Mf_dead/MX[fuel[col]]) + 5.11*(Mf_dead/MX[fuel[col]])*(Mf_dead/MX[fuel[col]]) - 3.52*(Mf_dead/MX[fuel[col]])*(Mf_dead/MX[fuel[col]])*(Mf_dead/MX[fuel[col]]);
			if (Mf_dead >= MX[fuel[col]])
				etaM_dead = 0.0;
			etaM_live = 1.0 - 2.59*(moisture[3]/xmext) + 5.11*(moisture[3]/xmext)*(moisture[3]/xmext) - 3.52*(moisture[3]/xmext)*(moisture[3]/xmext)*(moisture[3]/xmext);
			if (moisture[3] >= xmext)
				etaM_live = 0.0;
			wn_live = WO[3][fuel[col]]*(1 - ST);
			Qig[3] = 250 + 1116*moisture[3];
			class_sum = class_sum + Ffactor_all[3][fuel[col]]*epsilon[3][fuel[col]]*Qig[3];
			...
			gammamax = pow(sigma[fuel[col]], 1.5)/(495 + 0.0594*pow(sigma[fuel[col]], 1.5));
			gamma = gammamax*pow(beta/betaop, A)*exp(A*(1-beta/betaop));
			xi = exp((0.792 + 0.681*pow(sigma[fuel[col]], 0.5))*(beta + 0.1))/(192 + 0.2595*sigma[fuel[col]]);
			...
			if (parm.slope->answer && parm.aspect->answer) {
				phis = 5.275*pow(beta, -0.3)*tan(0.01745*slope[col])*tan(0.01745*slope[col]);
			} else phis = 0.0;
			...
			if (parm.dir->answer && parm.aspect->answer) {
				sin_fac = phiw*sin(0.01745*dir[col]) + phis*sin(0.01745*aspect[col]);
				cos_fac = phiw*cos(0.01745*dir[col]) + phis*cos(0.01745*aspect[col]);
...
/*printf("\nparm.dir->aanswer=%s, parm.aspect->aanswer=%s, phis=%f, phi_ws=%f, aspect[col]=%d,Rdir=%f",parm.dir->answer,parm.aspect->answer,phis,phi_ws,aspect[col],Rdir);*/
			...
			if (spotting)
				spotdist[col] = spot_dist(fuel[col], R, vel[col], Rdir, row, col);
...
/*printf("(%d, %d)\nR0=%.2f, vel=%d, dir=%d, phiw=%.2f, s=%d, as=%d, phis=%.2f, R=%.1f, Rdir=%.0f\n", row, col, R0, vel[col], dir[col], phiw, slope[col], aspect[col], phis, R, Rdir);*/
	...
	if (parm.mois_10h->answer)	G_close_cell(mois_10h_fd);
	if (parm.mois_100h->answer)	G_close_cell(mois_100h_fd);
	G_close_cell(mois_live_fd);
	if (parm.vel->answer)	G_close_cell(vel_fd);
	if (parm.dir->answer)	G_close_cell(dir_fd);
	if (parm.slope->answer)	G_close_cell(slope_fd);
	if (parm.aspect->answer)	G_close_cell(aspect_fd);
