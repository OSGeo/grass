...
int exact_checks(GDALDataType export_datatype,
    ...
    double nodataval, const char *nodatakey, int default_nodataval)
{
    double dfCellMin;
    double dfCellMax;
    int fd;
    ...
    G_message(_("Using GDAL data type <%s>"),
        GDALGetDataTypeName(export_datatype));
...
int export_band(GDALDatasetH hMEMDS, int band,
    ...
    int no_metadata, int writenodata, double offsetval, double scaleval)
{
    ...
    const char *name)
{
    ...
    case GDT_Byte:
        if (min < TYPE_BYTE_MIN || max > TYPE_BYTE_MAX) {
            ...
            G_warning(_("GDAL datatype: %s, range: %d - %d"),
                GDALGetDataTypeName(datatype), TYPE_BYTE_MIN, TYPE_BYTE_MAX);
    ...
    case GDT_CInt16:
        if (min < TYPE_INT16_MIN || max > TYPE_INT16_MAX) {
            G_warning(_("Selected GDAL datatype does not cover data range."));
            G_warning(_("GDAL datatype: %s, range: %d - %d"),
                GDALGetDataTypeName(datatype), TYPE_INT16_MIN, TYPE_INT16_MAX);
            G_warning(_("Raster map <%s> range: %g - %g"), name, min, max);
            return 1;
        }
        else
    ...
    case GDT_CInt32:
        if (min < TYPE_INT32_MIN || max > TYPE_INT32_MAX) {
            G_warning(_("Selected GDAL datatype does not cover data range."));
            G_warning(_("GDAL datatype: %s, range: %d - %d"),
                GDALGetDataTypeName(datatype), TYPE_INT32_MIN, TYPE_INT32_MAX);
            G_warning(_("Raster map <%s> range: %g - %g"), name, min, max);
            return 1;
        }
        else
