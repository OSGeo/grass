...
static void scr_cell (struct Cell_head *wind, int top, int bot, int left, int right, double *m);
...
back:
     ...
     if (sel[0][0] != 'x' && sel[1][0] != 'x' && sel[2][0] != 'x' &&
        ...
        G_system("clear");
        fprintf(stderr, "    Did you mean to not make any attribute group");
...

{
  ...
  fprintf(stderr, "       Left button:     What are row & column coordinates at this point?\n");
  fprintf(stderr, "       Middle button:   Mark %s point\n", str[opt]);
  fprintf(stderr, "       Right  button:   Finish region-connect to first point\n\n");
  return;
}
