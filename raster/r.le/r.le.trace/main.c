...
skip:
  if(!fp) return;
  if(p->num == 1) {
     fprintf(fp, "Patch         Patch   Patch   Patch     ----Shape Index----  Twist   Omega\n");
     fprintf(fp, "Number        Attr.   Area    Perim.    P/A    CP/A     RCC  Number  Index\n");
  }
  fprintf(fp, "%6d %12.3f %7.0f %7.0f %7.3f %7.3f %7.3f  %6d %6.3f\n",
     p->num, p->att, p->area, p->perim, shp1, shp2, shp3, p->twist, p->omega);
}
...
*/
  ...
  cell_clip(buf, null_buf, row0, col0, nrows, ncols, index,
            &centernull);
     ...
     if (1) {
        ...
        if (choice->trace) {
           ...
           if ((((int)patch->area % 4) - 1) == 0) {
              if (patch->area > 1.0)
                 patch->omega = (2.0 * patch->area + 2.0 - (float)patch->twist)/
                                (2.0 * patch->area - 2.0);
              else
                 patch->omega = 0.0;
           }
