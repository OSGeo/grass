...
{  
  ...
  if (0 > (finput = G_open_cell_old(choice->fn, G_mapset()))) {
     fprintf(stdout, "\n");
     fprintf(stdout, "   ********************************************************\n");
     fprintf(stdout, "    The raster map you specified with the 'map=' parameter \n");
     fprintf(stdout, "    was not found in your mapset.                          \n");
     fprintf(stdout, "   ********************************************************\n");
     exit(1);
  }
  ...
  if (choice->wrum == 'm')
     ...
     if (choice->att[0]) {
        ...
        fprintf(fp0, "                 MEAN        ST. DEV.    MINIMUM     MAXIMUM\n");
        fprintf(fp0, "Scale Unit       PIXEL ATT.  PIXEL ATT.  PIXEL ATT.  PIXEL ATT.\n");
        fclose(fp0);
     } 
     ...
     if (choice->div[0]) {
        ...
        fprintf(fp1, "                                                     INVERSE\n");
        fprintf(fp1, "Scale Unit       RICHNESS    SHANNON     DOMINANCE   SIMPSON\n");
        fclose(fp1);
     }

     if (choice->te2[0]) {
        fp2 = fopen0("r.le.out/t1-5.out", "w");
        fprintf(fp2, "                             ANGULAR     INVERSE\n");
        fprintf(fp2, "Scale Unit       CONTAGION   SEC. MOM.   DIFF. MOM.  ENTROPY     CONTRAST\n");
        fclose(fp2);
     } 
...
*/
  ...
  if (nc < 1 || nr < 1) {
     fprintf(stdout, "\n");
     fprintf(stdout, "   *******************************************************\n");
     fprintf(stdout, "    The moving window size specified in file r.le.para/   \n");
     fprintf(stdout, "    move_wind is less than 1 row or column.  Check this   \n");
     fprintf(stdout, "    file or redefine the moving window using r.le.setup.  \n");
     fprintf(stdout, "   *******************************************************\n");
     exit(1);
  }
  ...
  if (wind.rows < nr + y0 || wind.cols < nc + x0) {
     fprintf(stdout, "\n");
     fprintf(stdout, "   *******************************************************\n");
     fprintf(stdout, "    Moving window search area in file r.le.para/move_wind \n");
     fprintf(stdout, "    does not match the dimensions of the current region.  \n");    
     fprintf(stdout, "    You must either rerun r.le.setup to make a new        \n"); 	
     fprintf(stdout, "    r.le.para/move_wind file or reset the region to match \n");    
     fprintf(stdout, "    the r.le.para/move_wind file 		               \n");    
     fprintf(stdout, "   *******************************************************\n");
     exit(1);
  }
  ...
  fprintf(stdout, "If a MASK is not present (see r.mask) a beep may sound and a\n");
  fprintf(stdout, "   warning may be printed or appear in a window; ignore this warning.\n");
        ...
        for(j = 0; j < nc; j++) {
           ...
           switch (data_type) {
              case (CELL_TYPE):
	             ...
	             *(tmp + j) == 0) &&
	             *(nul_buf + j) == 0.0)
	             get_rich_whole((double)*(tmp + j), richwhole, &cntwhole);
                 break;
              case (FCELL_TYPE):
                 if ((*(ftmp + j) ||
	             *(ftmp + j) == 0) && 
	             *(nul_buf + j) == 0.0) 
	             get_rich_whole((double)*(ftmp + j), richwhole, &cntwhole);
                 break;
              case (DCELL_TYPE):
                 if ((*(dtmp + j) || 
	             *(dtmp + j) == 0) && 
  ...
  for(i = 0; i < nr; i++) {
     ...
     if (fmask > 0) {                  
        ...
        G_get_raster_row_nomask(fmask, row_buf, y0 + i + u_l/2, CELL_TYPE);
        ...
        for(j = 0; j < nc; j++) {
           ...
           if (row_buf[x0 + j + u_w/2])
              cell_clip_drv(x0 + j, y0 + i, u_w, u_l, buff, j, cntwhole, radius);
        }
     }
...

{
  ...
  if (!(fp = fopen(name, flag))){
     ...
     fprintf(stdout, "       which defines the moving window or sampling units \n");
...

{
  ...
  if (!(fp = fopen(name, flag))){
     fprintf(stdout, "\n");
     fprintf(stdout, "   ***************************************************\n");
     fprintf(stdout, "    You chose a juxtaposition measure, but r.le.pixel \n");
     fprintf(stdout, "       can't find file \"%s\"             		   \n",name);
     fprintf(stdout, "       which defines the weights for different edges  \n");
     fprintf(stdout, "    First use a text editor to make this file         \n");   
     fprintf(stdout, "   ***************************************************\n");
     exit(1);
  }
  return fp;
...

{
  ...
  if (!(fp = fopen(name, flag))){
     fprintf(stdout, "\n");
     fprintf(stdout, "   ***************************************************\n");
     fprintf(stdout, "    You chose an edge measure, but r.le.pixel         \n");
     fprintf(stdout, "       can't find file \"%s\"             		   \n",name);
     fprintf(stdout, "       which defines the types of edges to be counted \n");
     fprintf(stdout, "    First use a text editor to make this file         \n");   
     fprintf(stdout, "   ***************************************************\n");
     exit(1);
  }
...
{ 
  ...
  if ((n-i) % d == 0 ) {
     ...
     done2[24] = '\0';
     fprintf (stdout,"WINDOWS LEFT  = %8d     EST. COMPLETION = %s\r",(n-i),done2); 
     fflush(stdout);
     k ++; 
  }
  return;
  ...
  for(i = 0; i < nscl; i++) {
     ...
     if (choice->units) {
        for (k = 0; k < wind.rows + 3; k++) {
           ...
           fprintf(stdout, "\n");
           fprintf(stdout, "   ***************************************************\n");
           fprintf(stdout, "    You cannot choose more than 15 scales             \n");
           fprintf(stdout, "   ***************************************************\n");
           exit(0);
        }
     }
...
double dist;
  ...
  if (ncols < left + u_w || nrows < top + u_l) {
     fprintf(stdout, "\n");
     fprintf(stdout, "   ******************************************************\n");
     fprintf(stdout, "    Sampling units do not fit within the current region. \n");
     fprintf(stdout, "    Either correct the region or redo the sampling unit  \n");
     fprintf(stdout, "    selection using r.le.setup.  This error message came \n");
     fprintf(stdout, "    from an analysis of the r.le.para/units file and the \n");
     fprintf(stdout, "    current region setting.                              \n");
     fprintf(stdout, "   ******************************************************\n");
     exit(1);
  }	
