...

{
   ...
   cor = G_define_flag();
   cor->key              = 'c';
   cor->description      = "Output map 'interior' with patch cores (specify co1 & co2)";
   ...
   edge->key             = "co1";
   edge->description     = "Depth-of-edge-influence in pixels (integer) for use with co2";
   ...
   if (!strcmp(sampling_method->answer,"w") && cor->answer)
      ...
      fprintf(stderr, "\n");
      fprintf(stderr, "   ***********************************************\n");
      fprintf(stderr, "    You requested output of map 'core' with patch \n");
   ...
   if (!strcmp(sampling_method->answer,"w") && pat->answer)
      ...
      fprintf(stderr, "\n");
      fprintf(stderr, "   **********************************************\n");
      fprintf(stderr, "    You requested output of map 'num' with patch \n");
   ...
   if (!strcmp(sampling_method->answer,"u") && units->answer)
      ...
      fprintf(stderr, "\n");
      fprintf(stderr, "   ***************************************************\n");
      fprintf(stderr, "    You requested output of map 'units' with sampling \n");
   ...
   if (!strcmp(sampling_method->answer,"r")) {
      if (region->answer)
         G_strcpy(choice->reg,region->answer);
           ...
           fprintf(stderr, "\n");
           fprintf(stderr, "   ***********************************************\n");
           fprintf(stderr, "    You requested sampling by region, but did not \n");
   ...
   if (region->answer)
      if (strcmp(sampling_method->answer,"r")) {
           fprintf(stderr, "\n");
           fprintf(stderr, "   ***********************************************\n");
           fprintf(stderr, "    You requested sampling by region, by using    \n");
   ...
   for (i = 0; i < 9;  i++) choice->att[i]   = 0;
   for (i = 0; i < 9;  i++) choice->size[i]  = 0;
   for (i = 0; i < 11; i++) choice->core[i]  = 0;
   for (i = 0; i < 8;  i++) choice->shape[i] = 0;
   for (i = 0; i < 5;  i++) choice->boundary[i] = 0;
   ...
   if (att->answer) {
      ...
      for (i = 0; att->answers[i] != NULL; i++) {
         ...
         else if (!strcmp(att->answers[i],"a8"))   choice->att[8] = 1;
      }
   }
   ...
   if (size->answer) {
      ...
      for (i = 0; size->answers[i] != NULL; i++) {
         ...
         else if (!strcmp(size->answers[i],"s7"))   choice->size[7] = 1;
         else if (!strcmp(size->answers[i],"s8"))   choice->size[8] = 1;
      }
   }
   ...
   if (boundary->answer) {
      ...
      for (i = 0; boundary->answers[i] != NULL; i++) {
         count++;
              if (!strcmp(boundary->answers[i],"n1")) choice->boundary[1]=1;
         else if (!strcmp(boundary->answers[i],"n2")) choice->boundary[2]=1;
         else if (!strcmp(boundary->answers[i],"n3")) choice->boundary[3]=1;
         else if (!strcmp(boundary->answers[i],"n4")) choice->boundary[4]=1;
      }
   }
