...
gs.run_command("g.region", res=50, res3=50, t=25, b=0, n=1000, s=0, w=0, e=2000)

gs.message(_("Create all the input maps needed for groundwater flow computation"))

# Initial condition of the piezometric head, we have a huge gradient from 275m to 50m
# over a distance of 2000m
gs.run_command("r.mapcalc", expression="phead_1=if(col() == 1 , 275, 50)")
# Set the active cells and the dirichlet boundary condition,
# the first and the last cells a dirichlet
gs.run_command("r.mapcalc", expression="status_1=if(col() == 1 || col() == 40 , 2, 1)")
# We have a no wells
gs.run_command("r.mapcalc", expression="well_1=0")
# The hydraulic conductivity is 0.0001 m/s
gs.run_command("r.mapcalc", expression="hydcond_1=0.0001")
# The recharge, well we have no recharge at all
gs.run_command("r.mapcalc", expression="recharge_1=0")
# We have a confined aquifer with a height of 25m
gs.run_command("r.mapcalc", expression="top_conf_1=25")
# Bottom of the aquifer starts at 0m
gs.run_command("r.mapcalc", expression="bottom_1=0")
...
gs.run_command("r.mapcalc", expression="syield_1=0.0001")
gs.run_command("r.mapcalc", expression="null_1=0.0")
...
gs.run_command("r.mapcalc", expression="c_1=if(col() == 10 && row() == 10 , 0.0, 0.0)")
...
gs.run_command("r.mapcalc", expression="cin_1=0.0")
# We have a transfer boundary condition
gs.run_command("r.mapcalc", expression="tstatus_1=if(col() == 1 || col() == 40 , 3, 1)")
# No diffusion coefficient known for the solution
gs.run_command("r.mapcalc", expression="diff_1=0.0")
# Normal retardation
gs.run_command("r.mapcalc", expression="R_1=1.0")
...
gs.run_command("r.mapcalc", expression="poros_2=1")
...
# Get the maximum concentration
range = gs.parse_command("r.info", "r", map="stresult_conf_2")
# Normalize the result
gs.run_command(
    "r.mapcalc",
