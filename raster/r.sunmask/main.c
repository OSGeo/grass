 ...
 * PURPOSE:
 * SPDX-FileCopyrightText: 1999-2013 by the GRASS Development Team
 *
...
int main(int argc, char *argv[])
{
    ...
    parm.east->label = _("Easting coordinate (point of interest)");
    ...
    parm.north->label = _("Northing coordinate (point of interest)");
    ...
    if (data_type == CELL_TYPE) {
        if ((Rast_read_range(name, "", &range)) < 0)
            G_fatal_error(_("Unable to open range file for raster map <%s>"),
                          name);
