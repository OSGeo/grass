                                ...
                                double *result)
{
    ...
    Coppie *cc = NULL;  /* here store the pair of cell with the weight. these
    ...
    while ((l = read(file_fd, row, NMAX)) > 0) {
        char *strFile_tmp = concatena(strFile, row);
        if (strFile_tmp == NULL) {
            close(file_fd);
              ...
              long tc, long *siz G_UNUSED)
{
    ...
    while (it < tc) {
        ...
        if ((equalsGenericCell((cp_tmp->c1), ce1) == GC_EQUAL) &&
            ...
            if ((cp_tmp->d) != pe) {
                G_warning("different weight for the same cell type. I consider "
                          "right the first");
            }
