...
static avl_node *avl_individua( const avl_tree root, const generic_cell k, avl_node **father, int *direction );
static int avl_height(const avl_tree root );
static avl_node* critical_node( avl_node *added, int *pos1, int *pos2, const avl_node *prec);
...
int avl_add(avl_tree *root, const generic_cell k, const long n)
{
    ...
    switch( rotation )
    { 
	case AVL_SS: avl_rotation_ll( critical ); break;
	case AVL_SD: avl_rotation_lr( critical ); break;
