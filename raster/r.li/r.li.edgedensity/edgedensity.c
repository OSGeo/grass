...
int main(int argc, char *argv[])
{
    ...
    module->description =_("Calculates edge density index on a raster map, using a 4 neighbour algorithm");
    ...
    return calculateIndex(conf->answer, edgedensity, par, raster->answer, output->answer);
		...
		for(i=0; i < ad->cl; i++)/* for each cell in the row */
		{
			...
			{		
				if((i+1)==ad->cl) /*last cell of the row */
					...
					if(masked && mask_corr[i+1+ad->x]==0)
						G_set_c_null_value(&nextCell,1);
					else
				...
				if( (G_is_null_value(&infCell,CELL_TYPE)) || (corrCell!=infCell))
				{
					bordoCorr++; 
				}
		
		
				if((G_is_null_value(&nextCell,CELL_TYPE)) || (corrCell!=nextCell))
				{
					bordoCorr++; 
				}
	...
	{	
		...
		for(i=0; i < ad->cl; i++)/* for each cell in the row */
		{
			...
			if(!(G_is_null_value(&corrCell,DCELL_TYPE)))
			{
				...
				if( (G_is_null_value(&infCell,DCELL_TYPE)) || (corrCell!=infCell))
				{
					bordoCorr++; 
				}
		
		
				if((G_is_null_value(&nextCell,DCELL_TYPE)) || (corrCell!=nextCell))
				{
					bordoCorr++; 
				}
	...
	{	
		...
		for(i=0; i < ad->cl; i++)/* for each cell in the row */
		{
			...
			{	
				...
				if( (G_is_null_value(&infCell,FCELL_TYPE)) || (corrCell!=infCell))
				{
					bordoCorr++; 
				}
		
		
				if((G_is_null_value(&nextCell,FCELL_TYPE)) || (corrCell!=nextCell))
				{
					bordoCorr++; 
				}
