 ...
 * AUTHOR(S):    Serena Pallecchi student of Computer Science University of Pisa
 *               (Italy) Commission from Faunalia Pontedera (PI) www.faunalia.it
 *               Rewrite: Markus Metz
 *
 * PURPOSE:      calculates edge density index
 * SPDX-FileCopyrightText: 2006-2014 by the GRASS Development Team
 *
...
int main(int argc, char *argv[])
{
    ...
    module->description = _("Calculates edge density index on a raster map, "
                ...
                if (!Rast_is_c_null_value(&corrCell)) {
                    ...
                    if (Rast_is_c_null_value(&ptype) || corrCell == ptype) {
                        if (j > 0 && !(masked && mask_buf[j - 1] == 0) &&
                            (Rast_is_c_null_value(&precCell) ||
                             precCell != corrCell)) {
                            nedges++;
                        }
                        if (i > 0 && !(masked && mask_sup[i - 1] == 0) &&
                            (Rast_is_c_null_value(&supCell) ||
                             supCell != corrCell)) {
                            nedges++;
                        }
                    }
                }
                else if (Rast_is_c_null_value(&corrCell) &&
                         !(masked && mask_buf[j] == 0)) {
                    if (!Rast_is_c_null_value(&precCell)) {
                        if (Rast_is_c_null_value(&ptype) || precCell == ptype) {
                            nedges++;
        ...
        for (j = 0; j < ad->cl; j++) {
            ...
            if (brdr) {
                ...
                if (!Rast_is_d_null_value(&corrCell)) {
                    ...
                    if (Rast_is_d_null_value(&ptype) || corrCell == ptype) {
                        if (j > 0 && !(masked && mask_buf[j - 1] == 0) &&
                            (Rast_is_d_null_value(&precCell) ||
                             precCell != corrCell)) {
                            nedges++;
                        }
                        if (i > 0 && !(masked && mask_sup[i - 1] == 0) &&
                            (Rast_is_d_null_value(&supCell) ||
                             supCell != corrCell)) {
                            nedges++;
                        }
                    }
                }
                else if (Rast_is_d_null_value(&corrCell) &&
                         !(masked && mask_buf[j] == 0)) {
                    if (!Rast_is_d_null_value(&precCell)) {
                        if (Rast_is_d_null_value(&ptype) || precCell == ptype) {
                            nedges++;
        ...
        for (j = 0; j < ad->cl; j++) {
            ...
            if (brdr) {
                ...
                if (!Rast_is_f_null_value(&corrCell)) {
                    ...
                    if (Rast_is_f_null_value(&ptype) || corrCell == ptype) {
                        if (j > 0 && !(masked && mask_buf[j - 1] == 0) &&
                            (Rast_is_f_null_value(&precCell) ||
                             precCell != corrCell)) {
                            nedges++;
                        }
                        if (i > 0 && !(masked && mask_sup[i - 1] == 0) &&
                            (Rast_is_f_null_value(&supCell) ||
                             supCell != corrCell)) {
                            nedges++;
                        }
                    }
                }
                else if (Rast_is_f_null_value(&corrCell) &&
                         !(masked && mask_buf[j] == 0)) {
                    if (!Rast_is_f_null_value(&precCell)) {
                        if (Rast_is_f_null_value(&ptype) || precCell == ptype) {
                            nedges++;
