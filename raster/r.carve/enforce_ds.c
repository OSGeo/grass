...
static void process_line_segment(const int npts, void *rbuf, Point2 * pgxypts,
    ...
    struct Map_info *outMap, const struct parms *parm);
...
int enforce_downstream(int infd, int outfd,
    struct Map_info *Map, struct Map_info *outMap, struct parms *parm)
{
                ...
                case CELL_TYPE:
                    {
                        ...
                        if (Rast_is_d_null_value(&min)) {
                            if (!(Rast_is_c_null_value(&cbuf[r * rastcols +
                                            c])))
                ...
                case FCELL_TYPE:
                    {
                        ...
                        if (Rast_is_d_null_value(&min)) {
                            if (!(Rast_is_f_null_value(&fbuf[r * rastcols +
                                            c])))
                ...
                case DCELL_TYPE:
                    {
                        ...
                        if (Rast_is_d_null_value(&min)) {
                            if (!(Rast_is_d_null_value(&dbuf[r * rastcols +
                                            c])))
...
static void process_line_segment(const int npts, void *rbuf,
    ...
    struct BM *bm, struct Map_info *outMap, const struct parms *parm)
{
        ...
        for (r = row1; r <= row2; r++) {
            ...
            for (c = col1; c <= col2; c++) {
                ...
                distance = sqrt(dig_distance2_point_to_line(cellx, celly, 0,
                        ...
                        0, 0, &cellx, &celly, NULL, NULL, NULL));
