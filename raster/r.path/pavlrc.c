...
struct pavlrc *pavlrc_find(const struct pavlrc_table *tree,
    const struct pavlrc *item)
{
...
struct pavlrc *pavlrc_t_first(struct pavlrc_traverser *trav,
    struct pavlrc_table *tree)
{
...
/* Initializes |trav| for |tree|.
   ...
   or |NULL| if |tree| is empty. */
struct pavlrc *pavlrc_t_last(struct pavlrc_traverser *trav,
    struct pavlrc_table *tree)
{
    assert(tree != NULL && trav != NULL);
...
/* Searches for |item| in |tree|.
   ...
   and returns |NULL|. */
struct pavlrc *pavlrc_t_find(struct pavlrc_traverser *trav,
...
struct pavlrc *pavlrc_t_replace(struct pavlrc_traverser *trav,
    struct pavlrc *new)
{
    struct pavlrc *old;
...
struct pavlrc_table *pavlrc_copy(const struct pavlrc_table *org,
    pavlrc_copy_func * copy, struct libavl_allocator *allocator)
{
