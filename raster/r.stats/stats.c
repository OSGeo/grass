...
int 
reset_null_vals (CELL *cell, int ncols)
{
    while (ncols-- > 0)
    {
...
print_cell_stats (char *fmt, int with_percents, int with_counts, int with_areas, int with_labels, char *fs)
{
	...
	for (n = 0; n < node_count; n++)
	{
		...
		if(no_nulls && nulls_found) continue;
	    }
                ...
                else /* find out which floating point range to print */
		{
		   ...
		   {
		     if(cat_ranges)
		           fprintf (stdout,"%s%s", fs, labels[i].labels[node->values[i]]);
                     else
		           fprintf (stdout,"%sfrom %s to %s", fs, G_get_d_raster_cat(&dLow, &labels[i]), G_get_d_raster_cat(&dHigh, &labels[i])); }   
                }
