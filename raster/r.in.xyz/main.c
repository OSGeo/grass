...
int main(int argc, char *argv[])
{
    ...
    vcol_opt->answer = "0";
    vcol_opt->label =
        _("Alternate column number of data values in input file");
    ...
    for (pass = 1; pass <= npasses; pass++) {
        ...
        while (0 != G_getl2(buff, BUFFSIZE - 1, in_fp)) {
            ...
            if (bin_index) {
                ...
                ptr =
                    G_incr_void_ptr(ptr,
                    ((arr_row * cols) + arr_col) * Rast_cell_size(CELL_TYPE));
        ...
        for (row = 0; row < rows; row++) {
            ...
            case METHOD_N:     /* n is a straight copy */
                Rast_raster_cpy(raster_row,
                    n_array +
                    (row * cols * Rast_cell_size(CELL_TYPE)), cols, CELL_TYPE);
                break;
