...
that was set with [g.region](g.region.html) for processing. Sometimes it
...
requires this feature is [t.rast.algebra](t.rast.algebra.html). The
...
-   Setting the *region* parameter to *current* will result in the use
    of the current region as computational region. This is the default.
...
-   The parameter *intersect* will force r.mapcalc to compute the
    ...
    region at runtime. The region of the mapset will not be modified.
    The smallest spatial resolution of all raster maps will be used for
    processing.
...
respectively. The GRASS shell script *[r.blend](r.blend.html)* extracts
...
both NULL, then \"x == y\" and \"x != y\" are both NULL rather than 1
and 0 respectively.\
...
above, MASK is only applied when reading, not when writing a raster map.
...
represents luminance. See \"Fundamentals of Digital Image Processing,\"
by Anil K. Jain (Prentice Hall, NJ, 1989; p 67).

## SEE ALSO

*[g.region](g.region.html), [r.bitpattern](r.bitpattern.html),
