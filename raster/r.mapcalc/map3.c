                      ...
                      int row)
{
    ...
    case CELL_TYPE:
        for (i = 0; i < columns; i++) {
            ...
            if (Rast3d_put_value
                (handle, i, row, depth, (char *)&x, DCELL_TYPE) < 0)
    ...
    case FCELL_TYPE:
        for (i = 0; i < columns; i++) {
            ...
            if (Rast3d_put_value
                (handle, i, row, depth, (char *)&x, FCELL_TYPE) < 0)
                G_fatal_error(_("Error writing data"));
        }
        break;
    case DCELL_TYPE:
...
static void init_colors(map * m)
{
    ...
    if (Rast3d_read_colors((char *)m->name, (char *)m->mapset, &m->colors) <
        0)
...
int open_output_map(const char *name, int res_type)
{
    ...
    handle =
        Rast3d_open_new_opt_tile_size((char *)name, RASTER3D_USE_CACHE_XYZ,
                                      &current_region3,
                                      res_type ==
                                      FCELL_TYPE ? FCELL_TYPE : DCELL_TYPE,
                                      32);
