...
int main(int argc, char *argv[])
{
    ...
    struct
    {
        struct Option *model,
            ...
            *vel, *dir, *elev, *slope, *aspect, *base, *max, *maxdir,
            *spotdist;
    } parm;
    ...
    if (parm.slope->answer && !(parm.aspect->answer)) {
        G_fatal_error(_("An aspect layer should be given "
                "if the slope layer <%s> has been given"), parm.slope->answer);
    }
    if (!(parm.slope->answer) && parm.aspect->answer) {
    ...
    for (row = 0; row < nrows; row++) {
        ...
        for (col = 0; col < ncols; col++) {
            ...
            if (SIGMA[3][fuel[col]] > 0.0) {
                for (class = 0; class <= 2; class++) {
                    ...
                    wmfd =
                        wmfd +
                        WO[class][fuel[col]] * exp(-138.0 /
                        SIGMA[class][fuel[col]]) * moisture[class];
                }
                fdmois = wmfd / fined;
            ...
            if (parm.slope->answer && parm.aspect->answer) {
                phis =
                    5.275 * pow(beta,
                    -0.3) * tan(M_D2R * slope[col]) * tan(M_D2R * slope[col]);
            }
            else
                phis = 0.0;
