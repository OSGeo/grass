 ...
 * PURPOSE:      This is the main program for simulating elliptical spread.
 *
 *      It
 ...
 *         to it.
 * SPDX-FileCopyrightText: 2000-2006 Other GRASS authors
...
int main(int argc, char *argv[])
{
    ...
    struct {
        struct Option *max, *dir, *base, *start, *spotdist, *velocity, *mois,
            *least, *comp_dens, *init_time, *time_lag, *backdrop, *out, *x_out,
            *y_out;
    } parm;
    struct {
    ...
    module->label = _("Simulates elliptically anisotropic spread.");
    module->description =
    ...
    parm.base->label = _("Raster map containing base ROS (cm/min)");
    ...
    parm.max->label = _("Raster map containing maximal ROS (cm/min)");
    ...
    parm.dir->description =
        _("Name of an existing raster map layer in the user's "
          "current mapset search path containing directions of the maximum "
          "ROSes, "
          "clockwise from north (degree)."); /* TODO: clockwise from north? see
                                                r.ros */
    ...
    parm.start->label = _("Raster map containing starting sources");
    parm.start->description = _("Name of an existing raster map layer in the "
                                "user's current mapset search path containing "
                                "starting locations of the "
                                "spread phenomenon. Any positive integers in "
                                "this map are recognized as "
    ...
    parm.spotdist->label = _("Raster map containing maximal spotting distance "
                             "(m, required with -s)");
    parm.spotdist->description = _("Name of an existing raster map layer in "
                                   "the user's current mapset search path "
                                   "containing the maximum potential "
    ...
    parm.velocity->description =
        _("Name of an existing raster map layer in the "
          "user's current mapset search path containing wind velocities at "
          "half of "
    ...
    parm.mois->label =
        _("Raster map containing fine fuel moisture of the cell receiving a "
          "spotting firebrand (%, required with -s)");
    parm.mois->description = _(
    ...
    parm.least->label = _("Basic sampling window size needed to meet certain "
                          "accuracy (3)"); /* TODO: what is 3 here? default? */
    parm.least->description = _("An odd integer ranging 3 - 15 indicating "
                                "the basic sampling window size within which "
                                "all cells will be considered "
                                "to see whether they will be reached by the "
                                "current spread cell. The default "
    ...
    parm.comp_dens->label =
        _("Sampling density for additional computing (range: 0.0 - 1.0 "
          "(0.5))"); /* TODO: again, what is 0.5?, TODO: range not set */
    parm.comp_dens->description =
        _("A decimal number ranging 0.0 - 1.0 indicating "
          "additional sampling cells will be considered to see whether they "
          "will be "
          "reached by the current spread cell. The closer to 1.0 the decimal "
          "number "
          "is, the longer the program will run and the higher the simulation "
          "accuracy "
    ...
    parm.init_time->label = _("Initial time for current simulation (0) (min)");
    parm.init_time->description =
        _("A non-negative number specifying the initial "
          "time for the current spread simulation (minutes). This is useful "
          "when multiple "
    ...
    parm.time_lag->label =
        _("Simulating time duration LAG (fill the region) (min)"); /* TODO: what
                                                                      does this
                                                                      mean? */
    parm.time_lag->description =
        _("A non-negative integer specifying the simulating "
          "duration time lag (minutes). The default is infinite, but the "
          "program will "
          "terminate when the current geographic region/mask has been filled. "
          "It also "
    ...
    parm.backdrop->label = _("Name of raster map as a display backdrop");
    parm.backdrop->description =
        _("Name of an existing raster map layer in the "
          "user's current mapset search path to be used as the background on "
          "which "
    ...
    parm.out->label = _("Raster map to contain output spread time (min)");
    parm.out->description = _("Name of the new raster map layer to contain "
                              "the results of the cumulative spread time "
                              "needed for a phenomenon to reach "
    ...
    parm.x_out->label = _("Name of raster map to contain X back coordinates");
    parm.x_out->description = _("Name of the new raster map layer to contain "
                                "the results of backlink information in UTM "
                                "easting coordinates for each "
                                "cell.");
    ...
    parm.y_out->label = _("Name of raster map to contain Y back coordinates");
    parm.y_out->description = _("Name of the new raster map layer to contain "
                                "the results of backlink information in UTM "
                                "northing coordinates for each "
                                "cell.");
    ...
    flag.start_is_time->description =
        _("Designed to be used with output"
    ...
    if (spotting) {
        if (!(parm.spotdist->answer && parm.velocity->answer &&
              parm.mois->answer)) {
            G_warning("SPOTTING DISTANCE, fuel MOISTURE, or wind VELOCITY map "
                      "not given w/ -s");
            G_usage();
            exit(EXIT_FAILURE);
        }
    ...
    /*Check the given initial time and simulation time lag, assign the default
     * if needed */
