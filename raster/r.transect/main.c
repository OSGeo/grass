 ...
 * SPDX-FileCopyrightText: 1999-2006,2009 by the GRASS Development Team
 *
...
int main(int argc, char *argv[])
{
    ...
    module->description = _("Outputs raster map layer values lying along "
    ...
    coord = G_define_flag();
    coord->key = 'g';
    coord->description = _("Output easting and northing in first two columns "
                           "of four column output");

    if (G_parser(argc, argv))
        exit(EXIT_FAILURE);

    return profile(coord->answer, parms.map->answer, parms.null_str->answer,
