...
![](rcost_knightsmove.png){border="1"}\
...
extracted with *[r.path](r.path.html)* by using the direction output map
of *r.cost*.
...
*r.cost* can be used as inputs to *[r.path](r.path.html)* , in order to
...
movement direction map, *[r.path](r.path.html)* can be used to find the
minimum cost path.

## SEE ALSO

*[r.walk](r.walk.html), [r.path](r.path.html),
