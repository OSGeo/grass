...
void joules2(struct SunGeometryConstDay *sunGeom,
    ...
    double longitude,
    double *Pbeam_e, double *Pdiff_e, double *Prefl_e, double *Pinsol_t);


void calculate(double singleSlope, double singleAspect,
    double singleAlbedo, double singleLinke, struct GridGeometry gridGeom);
...
#pragma omp for schedule(dynamic)                                                        \
                ...
                if (sunVarGeom.z_orig != UNDEFZ) {
                    ...
                    if (someRadiation) {
                        joules2(&sunGeom, &sunVarGeom, &sunSlopeGeom,
                            &sunRadVar, &gridGeom, horizonarray + shadowoffset,
                            latitude, longitude, &Pbeam_e, &Pdiff_e, &Prefl_e,
                            &Pinsol_t);
                        if (beam_rad != NULL)
    ...
    if (albedo == NULL)
        Rast_append_format_history(&hist,
            " Ground albedo:                            %.3f", sunRadVar.alb);
    else
        Rast_append_format_history(&hist,
