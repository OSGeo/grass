...
static void ImportBand(GDALRasterBandH hBand, const char *output,
    struct Ref *group_ref, int *rowmap, int *colmap, int col_offset);
static void SetupReprojector(const char *pszSrcWKT, const char *pszDstLoc,
    struct pj_info *iproj, struct pj_info *oproj, struct pj_info *tproj);
            ...
            if (create_target) {
                ...
                if (!hSRS || GPJ_osr_to_grass(&gcpcellhd, &proj_info,
                    ...
                    if (0 != G_make_location_crs(parm.target->answer,
                            &gcpcellhd,
                            proj_info, proj_units, gdalsrid, gdalwkt)) {
    ...
    if (complex) {              /* CEOS SAR et al.: import flipped to match GRASS coordinates */
        for (row = 0; row < nrows; row++) {
            ...
            for (indx = ncols - 1; indx >= 0; indx--) { /* CEOS: flip east-west during import - MN */
                if (eGDT == GDT_Int32) {
                    if (colmap[indx] < 0) {
                        Rast_set_c_null_value(&(((CELL *) cellReal)[ncols -
                                    indx]), 1);
                        Rast_set_c_null_value(&(((CELL *) cellImg)[ncols -
                                    indx]), 1);
                    }
                    else {
                    ...
                    if (colmap[indx] < 0) {
                        Rast_set_f_null_value(&(((FCELL *) cellReal)[ncols -
                                    indx]), 1);
                        Rast_set_f_null_value(&(((FCELL *) cellImg)[ncols -
                                    indx]), 1);
                    }
                    else {
                    ...
                    if (colmap[indx] < 0) {
                        Rast_set_d_null_value(&(((DCELL *) cellReal)[ncols -
                                    indx]), 1);
                        Rast_set_d_null_value(&(((DCELL *) cellImg)[ncols -
                                    indx]), 1);
                    }
                    else {
    ...
    for (row = 0; row < nrows; row++) {
        ...
        for (col = 0; col < ncols; col++) {
            ...
            if (field_type[col] == GFT_Integer)
                ...
                fprintf(fp, "%.15g", GDALRATGetValueAsDouble(gdal_rat, row,
                        col));
            else
