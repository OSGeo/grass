...
static void process_row_ii(int row)
{
    ...
    Rast_fpreclass_perform_ii(&rcl_struct, (CELL *) in_rast,
                              (CELL *) out_rast, ncols);
...
static void process_row_if(int row)
{
    if (no_mask)
        ...
        Rast_get_c_row(in_fd, (CELL *) in_rast, row);
    Rast_fpreclass_perform_if(&rcl_struct, (CELL *) in_rast,
                              (FCELL *) out_rast, ncols);
...
static void process_row_id(int row)
{
    if (no_mask)
        ...
        Rast_get_c_row(in_fd, (CELL *) in_rast, row);
    Rast_fpreclass_perform_id(&rcl_struct, (CELL *) in_rast,
                              (DCELL *) out_rast, ncols);
...
static void process_row_fi(int row)
{
    ...
    Rast_fpreclass_perform_fi(&rcl_struct, (FCELL *) in_rast,
                              (CELL *) out_rast, ncols);
    Rast_put_row(out_fd, (CELL *) out_rast, CELL_TYPE);
}
...
static void process_row_ff(int row)
{
    if (no_mask)
        ...
        Rast_get_f_row(in_fd, (FCELL *) in_rast, row);
    Rast_fpreclass_perform_ff(&rcl_struct, (FCELL *) in_rast,
                              (FCELL *) out_rast, ncols);
    Rast_put_f_row(out_fd, (FCELL *) out_rast);
}
...
static void process_row_fd(int row)
{
    if (no_mask)
        ...
        Rast_get_f_row(in_fd, (FCELL *) in_rast, row);
    Rast_fpreclass_perform_fd(&rcl_struct, (FCELL *) in_rast,
                              (DCELL *) out_rast, ncols);
    Rast_put_row(out_fd, (DCELL *) out_rast, DCELL_TYPE);
}
...
static void process_row_di(int row)
{
    ...
    Rast_fpreclass_perform_di(&rcl_struct, (DCELL *) in_rast,
                              (CELL *) out_rast, ncols);
    Rast_put_row(out_fd, (CELL *) out_rast, CELL_TYPE);
}
...
static void process_row_df(int row)
{
    if (no_mask)
        ...
        Rast_get_d_row(in_fd, (DCELL *) in_rast, row);
    Rast_fpreclass_perform_df(&rcl_struct, (DCELL *) in_rast,
                              (FCELL *) out_rast, ncols);
    Rast_put_f_row(out_fd, (FCELL *) out_rast);
}
...
static void process_row_dd(int row)
{
    if (no_mask)
        ...
        Rast_get_d_row(in_fd, (DCELL *) in_rast, row);
    Rast_fpreclass_perform_dd(&rcl_struct, (DCELL *) in_rast,
                              (DCELL *) out_rast, ncols);
    Rast_put_row(out_fd, (DCELL *) out_rast, DCELL_TYPE);
}
