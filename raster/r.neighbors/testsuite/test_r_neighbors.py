...
[gauss=float] [quantile=float[,float,...]] [--overwrite] [--help]
[--verbose] [--quiet] [--ui]
"""
...
class TestNeighbors(TestCase):
    ...
    raster_min = "test_neighbors_min"
    raster_max = "test_neighbors_max"
    raster_avg = "test_neighbors_avg"
    raster_stddev = "test_neighbors_stddev"
    raster_perc_90 = "test_neighbors_perc90"
    raster_sum = "test_neighbors_sum"
    raster_q01 = "test_neighbors_q01"
    raster_q03 = "test_neighbors_q03"
    raster_q95 = "test_neighbors_q95"
    raster_var = "test_neighbors_var"
    raster_div = "test_neighbors_div"
    raster_range = "test_neighbors_range"
    raster_mode = "test_neighbors_mode"
...
sum=2009777199.90327""",
        ...
        self.assertModule(
            ...
            input="elevation",
            output=",".join(outputs),
            method=["minimum", "perc90", "average", "maximum", "stddev"],
        )
        for idx, rmap in enumerate(outputs):
            self.assertRasterFitsUnivar(
                raster=rmap, reference=references[idx], precision=1e-5
            )
            self.to_remove.append(rmap)
...
cells=2025000
min=642.146484375
max=4529.56787109375
range=3887.42138671875
mean=3195.50768966749
mean_of_abs=3195.50768966749
stddev=592.174516879002
variance=350670.65844088
coeff_var=18.5314689992382
sum=6470903071.57666""",
        ]
        self.assertModule(
            "r.neighbors",
            overwrite=True,
            ...
            size=7,
            output=",".join(outputs),
            method=["minimum", "perc90", "average", "maximum", "stddev", "sum"],
        )
        for idx, rmap in enumerate(outputs):
            self.assertRasterFitsUnivar(
                raster=rmap, reference=references[idx], precision=1e-5
...
sum=223510266.558102""",
        ...
        self.runModule(
            "r.mapcalc",
            overwrite=True,
            expression="{}=if(y()==15,1,null())".format(self.selection),
        )
        self.to_remove.append(self.selection)
        self.assertModule(
            "r.neighbors",
            overwrite=True,
            input="elevation",
...
sum=190083059.850574""",
        ...
        self.assertModule(
            ...
            gauss=2.0,
            output=",".join(outputs),
            method=["perc90", "average", "stddev", "sum"],
        )
        for idx, rmap in enumerate(outputs):
            self.assertRasterFitsUnivar(
                raster=rmap, reference=references[idx], precision=1e-5
