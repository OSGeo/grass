...
function create_dmg () {
    ...
    hdiutil create -srcfolder "$grass_app_bundle" \
        ...
        -format UDRW \
        -size ${dmg_size}k "$dmg_tmpfile" || (rm -rf "$tmpdir" && exit_nice 1)
...
EOF
    ...
    hdiutil convert "${dmg_tmpfile}" \
        -format UDZO -imagekey zlib-level=9 -o "${DMG_FILE}" || (rm -rf "$tmpdir" && exit_nice 1)
...
if [[ "$conda_update_stable" -eq 1 ]]; then
    if [ "$conda_req_file" != "$conda_dev_file" ]; then
        echo "Error, file given with '--conda-file' must be same as ${conda_dev_file}";
        exit_nice 1
    fi
