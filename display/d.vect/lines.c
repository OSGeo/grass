...
static int draw_line(int, int, int,
    ...
    RGBA_Color *, int *, int *, int *, int *, int *);
    ...
    dbCatValArray * cvarr_rot, int nrec_rot)
{
    ...
    if ((colors || cvarr_rgb) && get_num_color_rules_skipped() > 0)
        G_warning(n_("%d invalid color rule for lines skipped",
                ...
                get_num_color_rules_skipped()), get_num_color_rules_skipped());

    if (n_points > 0)
        G_verbose_message(n_("%d point plotted", "%d points plotted",
                n_points), n_points);
    if (n_lines > 0)
        G_verbose_message(n_("%d line plotted", "%d lines plotted", n_lines),
            n_lines);
    if (n_centroids > 0)
        G_verbose_message(n_("%d centroid plotted", "%d centroids plotted",
                n_centroids), n_centroids);
    if (n_boundaries > 0)
        G_verbose_message(n_("%d boundary plotted", "%d boundaries plotted",
    ...
    int *n_faces)
{
    ...
    if (colors || cvarr_rgb || nrec_width > 0 || nrec_size > 0 || nrec_rot > 0)
        /* only first category */
        Vect_cat_get(Cats,
            (Clist->field > 0 ? Clist->field : (Cats->n_cats >
    ...
    if (nrec_width) {
        width = (int)get_property(cat, line, cvarr_width,
            (double)width_scale, (double)default_width);
