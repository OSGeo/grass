...
int display_shape(struct Map_info *Map, int type, struct cat_list *Clist, const struct Cell_head *window, const struct color_rgb *bcolor, const struct color_rgb *fcolor, int chcat, const char *icon, double size, const char *size_column, int sqrt_flag, const char *rot_column,     /* lines only */
    int id_flag, int cats_colors_flag, char *rgb_column,
    int default_width, char *width_column, double width_scale, char *z_style)
{
    ...
    if (rgb_column) {
        ...
        if (cvarr_rgb.ctype != DB_C_TYPE_STRING) {
            G_warning(_("Color definition column ('%s') not a string. "
                    ...
                    "Unable to colorize features."), rgb_column, "v.colors");
            rgb_column = NULL;
        }
        else {
            if (nrec_rgb < 0)
    ...
    if (width_column) {
        ...
        for (i = 0; i < cvarr_width.n_values; i++) {
            G_debug(4, "cat = %d  %s = %d", cvarr_width.value[i].cat,
                width_column,
                (cvarr_width.ctype ==
                    DB_C_TYPE_INT ? cvarr_width.value[i].
                    val.i : (int)cvarr_width.value[i].val.d));
        }
    }
    ...
    if (type & GV_AREA &&
        ...
        stat +=
            display_area(Map, Clist, window, bcolor, fcolor, chcat, id_flag,
...
int get_table_color(int cat, int line,
    struct Colors *colors, dbCatValArray * cvarr, int *red, int *grn, int *blu)
{
    int custom_rgb;
    ...
    double default_value)
{
    ...
    if (db_CatValArray_get_value(cvarr, cat, &cv) != DB_OK) {
        ...
        if (value < 0) {
            G_important_message(_("Invalid negative value - feature %d with category %d"), line,
                cat);
            value = default_value;
        }
    }
