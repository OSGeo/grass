...
int display_area(struct Map_info *Map, struct cat_list *Clist,
    ...
    struct Colors *zcolors, dbCatValArray * cvarr_rgb, struct Colors *colors,
    dbCatValArray * cvarr_width, int nrec_width)
{
    ...
    n_ipoints_alloc = 10;
    IPoints =
        (struct line_pnts **)G_malloc(n_ipoints_alloc *
    ...
    for (area = 1; area <= num; area++) {
        ...
        if (n_isles >= n_ipoints_alloc) {
            IPoints =
                (struct line_pnts **)G_realloc(IPoints,
                (n_isles + 10) * sizeof(struct line_pnts *));
