...
void write_into_legend_file(const char *legfile, const char *icon,
    const char *title, double stats_min, double stats_max, double *breakpoints,
    int nbreaks, int size, struct color_rgb bcolor, struct color_rgb *colors,
    int default_width, int *frequencies, const char *topo)
{
    FILE *fd;
    int i;
    ...
    fprintf(fd, "%s|%d|ps|%d:%d:%d|%d:%d:%d|%d|%s|%d\n",
        ...
        bcolor.r, bcolor.g, bcolor.b, default_width, topo, frequencies[0]);
    /* Middle lines */
    for (i = 1; i < nbreaks; i++) {
        fprintf(fd, "%.2f - %.2f|%s|%d|ps|%d:%d:%d|%d:%d:%d|%d|%s|%d\n",
            breakpoints[i - 1], breakpoints[i],
            icon, size, colors[i].r, colors[i].g, colors[i].b,
            bcolor.r, bcolor.g, bcolor.b, default_width, topo, frequencies[i]);
    }
    /* Last one */
