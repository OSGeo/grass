...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < cvarr.n_values; i++) {
        G_debug(4, "cat = %d  %s = %d", cvarr.value[i].cat,
            column_opt->answer,
            (cvarr.ctype ==
                DB_C_TYPE_INT ? cvarr.value[i].val.i : (int)cvarr.value[i].val.
                d));
    }
        ...
        if (overlap == 0) {
            ...
            if (Vect_get_num_primitives(&Map, GV_BOUNDARY) > 0)
                stat =
                    ...
                    colors, has_color ? &bcolor : NULL, chcat, &window,
                    default_width);
        ...
        while (TRUE) {
            ...
            if (nfeatures > 0) {
                write_into_legend_file(legend_file_opt->answer,
                    icon_opt->answer, title, stats.min, stats.max, breakpoints,
                    nbreaks, size, bcolor, colors, default_width, frequencies,
                    "point");
                break;
            }
            nfeatures = Vect_get_num_primitives(&Map, GV_LINE);
            if (nfeatures > 0) {
                write_into_legend_file(legend_file_opt->answer,
                    icon_line_opt->answer, title, stats.min, stats.max,
            ...
            if (nfeatures > 0) {
                write_into_legend_file(legend_file_opt->answer,
                    icon_area_opt->answer, title, stats.min, stats.max,
                    breakpoints, nbreaks, size, bcolor, colors, default_width,
                    frequencies, "area");
                break;
            }
