...
# this is not FHS compliant: hide grass-%%{version} in %%{libdir}/grass
...
mv %{buildroot}%{_libdir}/%{name}-%{version} %{buildroot}%{_libdir}/%{name}
mv %{buildroot}%{_bindir}/%{name}%{shortver} %{buildroot}%{_bindir}/%{name}
...
sed -i -e 's|%{buildroot}%{_libdir}/%{name}-%{version}|%{_libdir}/%{name}|g' \
	%{buildroot}%{_bindir}/%{name}
# fix GRASS_HOME and RUN_GISBASE in Platform.make
sed -i -e 's|%{buildroot}%{_libdir}/%{name}-%{version}|%{_libdir}/%{name}|g' \
	%{buildroot}%{_libdir}/%{name}/include/Make/Platform.make
# fix INST_DIR in Platform.make
sed -i -e 's|%{name}-%{version}|%{name}|g' \
	%{buildroot}%{_libdir}/%{name}/include/Make/Platform.make
# fix ARCH_DISTDIR in Grass.make
sed -i -e 's|%{buildroot}%{_libdir}/%{name}-%{version}|%{_libdir}/%{name}|g' \
	%{buildroot}%{_libdir}/%{name}/include/Make/Grass.make
# fix ARCH_BINDIR in Grass.make
sed -i -e 's|%{buildroot}%{_bindir}|%{_bindir}|g' \
	%{buildroot}%{_libdir}/%{name}/include/Make/Grass.make
# fix GISDBASE in demolocation
sed -i -e 's|%{buildroot}%{_libdir}/%{name}|%{_libdir}/%{name}|g' \
	%{buildroot}%{_libdir}/%{name}/demolocation/.grassrc%{shortver}
# Correct font path
sed -i -e 's|%{buildroot}%{_libdir}/%{name}|%{_libdir}/%{name}|' \
	%{buildroot}%{_libdir}/%{name}/etc/fontcap
# fix paths in grass.pc
sed -i -e 's|%{_prefix}/%{name}-%{version}|%{_libdir}/%{name}|g' \
	%{name}.pc
...
mv %{buildroot}%{_libdir}/%{name}/include/%{name}/config.h \
   %{buildroot}%{_libdir}/%{name}/include/%{name}/config-%{cpuarch}.h 
install -p -m 644 %{SOURCE2} %{buildroot}%{_libdir}/%{name}/include/%{name}/config.h

# Make man pages available on the system, convert to utf8 and avoid name conflict
mkdir -p %{buildroot}%{_mandir}/man1
for man in $(ls %{buildroot}%{_libdir}/%{name}/docs/man/man1/*.1)
do
...
%files
%exclude %{_libdir}/%{name}/driver/db/*
%exclude %{_libdir}/%{name}/lib
%exclude %{_libdir}/%{name}/include
%exclude %{_libdir}/%{name}/gui
%{_libdir}/%{name}
%{_bindir}/*
%{_datadir}/appdata/*
...
%{_libdir}/%{name}/lib/*.%{version}.so
%{_libdir}/%{name}/lib/*.a
%dir %{_libdir}/%{name}/driver
%dir %{_libdir}/%{name}/driver/db
%{_libdir}/%{name}/driver/db/*
