...
int Nliteral_from_logical_cmd(Nv_data * data, Tcl_Interp * interp, int argc,
                  char *argv[])
{
    int *surf_list, *vect_list, *site_list, *vol_list, num_surfs, num_vects, num_sites, num_vols,
    i, s1_length;
    char err_string[200];
    Nv_clientData *cdata;
...
{
    if (type != VOL) {
    Tcl_SetResult(interp, "Type must be VOL for isosurf",
              TCL_VOLATILE);
    return (TCL_ERROR);
    }
...
{
    if (type != VOL) {
    Tcl_SetResult(interp, "Type must be VOL for slice",
              TCL_VOLATILE);
    return (TCL_ERROR);
    }
