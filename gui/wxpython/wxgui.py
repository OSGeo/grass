        ...
        def show_main_gui():
            ...
            try:
                ...
                if not globalvar.CheckWxVersion(min_required_wx_version):
                    ...
                    warning(
                        _(
                            "Current version of wxPython {current_version} is lower "
                            "than minimum required version {min_version}"
                        ).format(
                            current_version=wx.__version__,
                            min_version=".".join(map(str, min_required_wx_version)),
                        )
                    )
                else:
                    raise
