...
class SingleMapFrame(wx.Frame):
    """Frame with one map window.

    It is base class for non-dockable map frames. It creates a frame and
    forwards methods used in SingleMapPanel class.
    ...
    def __init__(
        ...
        wx.Frame.__init__(self, parent=parent, id=id, **kwargs)
        self.panel = SingleMapPanel(parent=self,
                                    giface=giface,
                                    title=title,
                                    Map=Map,
                                    auimgr=auimgr,
    ...
    def __setattr__(self, name, value):
        if not name == 'panel' and name == "MapWindow":
            return self.panel.__setattr__(name, value)
...
class DoubleMapFrame(wx.Frame):
    """Frame with two map windows.

    It is base class for non-dockable map frames which needs two maps.
    It creates a frame and forwards methods used in DoubleMapPanel class.
    ...
    def __init__(
        ...
        print(parent)
        print(guiparent)
        print(parent.firstMapWindow)
        self.parent = parent
        print(self.parent)

        wx.Frame.__init__(self, parent=guiparent, id=id, **kwargs)
        self.panel = DoubleMapPanel(parent=guiparent,
                                    title=title,
                                    firstMap=firstMap,
                                    secondMap=secondMap,
                                    auimgr=auimgr,
    ...
    def __setattr__(self, name, value):
        if name == "firstMapWindow":
            return self.panel.__setattr__(name, self.parent.firstMapWindow)
        elif name == "secondMapWindow":
            return self.panel.__setattr__(name, self.parent.secondMapWindow)
        elif name == "firstMap":
            return self.panel.__setattr__(name, self.parent.firstMap)
        elif name == "secondMap":
            return self.panel.__setattr__(name, self.parent.secondMap)
        elif name == "MapWindow":
            return self.panel.__setattr__(name, self.parent.MapWindow)
