...
class PreferencesDialog(PreferencesBaseDialog):
    ...
    def _createGeneralPage(self, notebook):
        ...
        box = StaticBox(
            ...
            id=wx.ID_ANY,
            label=" %s " %
            _("Data Catalog settings"))
        sizer = wx.StaticBoxSizer(box, wx.VERTICAL)
        lazyLoadingDataCatalog = wx.CheckBox(parent=panel, label=_(
                "At startup load maps from current mapset only (in the Data tab)"),
            name='IsChecked')
        lazyLoadingDataCatalog.SetValue(
            self.settings.Get(
                ...
                subkey='enabled'))
        self.winId[
            'datacatalog:lazyLoading:enabled'] = lazyLoadingDataCatalog.GetId()
        sizer.Add(
            lazyLoadingDataCatalog,
            proportion=1,
            flag=wx.ALL | wx.EXPAND,
            border=5)
        border.Add(sizer, proportion=0, flag=wx.ALL | wx.EXPAND, border=3)
