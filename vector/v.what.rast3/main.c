...
int main(int argc, char *argv[])
{
    ...
    for (point = 0; point < point_cnt; point++) {
        ...
        if (typeIntern == FCELL_TYPE) {
            Rast3d_get_window_value(map, cache[point].y, cache[point].x,
                cache[point].z, &cache[point].fvalue, FCELL_TYPE);
        }
        else {
            Rast3d_get_window_value(map, cache[point].y, cache[point].x,
                cache[point].z, &cache[point].dvalue, DCELL_TYPE);
        }
    }
    ...
    for (point = 0; point < point_cnt; point++) {
        ...
        if (typeIntern == FCELL_TYPE)
            is_empty =
                Rast3d_is_null_value_num(&cache[point].fvalue, FCELL_TYPE);
    ...
    G_verbose_message(n_("%d category loaded from vector",
            "%d categories loaded from vector", point_cnt), point_cnt);
    ...
    if (dupl_cnt > 0)
        G_message(n_("%d duplicate category in vector",
                "%d duplicate categories in vector", dupl_cnt), dupl_cnt);
    if (upderr_cnt > 0)
