...
static int write_skeleton(struct line_pnts *Points)
{
    ...
    if (ret) {
        for (i = 0; i < Vect_get_area_num_isles(&In, area1); i++) {
            Vect_get_isle_points(&In, Vect_get_area_isle(&In, area1, i),
                APoints);
            if (Vect_line_check_intersection(Points, APoints, 0)) {
                ret = 0;
                break;
    ...
    if (!ret && area1 != area2) {
        ...
        if (ret) {
            for (i = 0; i < Vect_get_area_num_isles(&In, area2); i++) {
                Vect_get_isle_points(&In, Vect_get_area_isle(&In, area2, i),
                    APoints);
                if (Vect_line_check_intersection(Points, APoints, 0)) {
                    ret = 0;
                    break;
    ...
    if (e->ep[le] != NULL && e->ep[re] != NULL) {       /* both end defined */
        ...
        if (knownPointAtLeft == -1) {
            ...
            if (!extend_line(Box.S, Box.N, Box.W, Box.E,
                    e->a, e->b, e->c, x2, y2, &x1, &y1, knownPointAtLeft)) {
        ...
        if (extend_line(Box.S, Box.N, Box.W, Box.E,
                e->a, e->b, e->c, x1, y1, &x2, &y2, knownPointAtLeft)) {
