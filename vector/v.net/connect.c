...
int connect_arcs(struct Map_info *In, struct Map_info *Pnts,
    struct Map_info *Out, int afield, int nfield, double thresh, int snap)
{
    int narcs;
    ...
    while ((type = Vect_read_next_line(Pnts, Points, Cats)) >= 0) {
        ...
        line =
            ...
            GV_LINES, thresh, WITHOUT_Z, exclude_list, NULL);
        ...
        if (dist > 0.0) {
            if (snap) {
                ...
                Vect_reset_line(Pout);
                Vect_append_point(Pout, px, py, pz);
                Vect_append_point(Pout, Points->x[0], Points->y[0],
                    Points->z[0]);
                maxcat++;
