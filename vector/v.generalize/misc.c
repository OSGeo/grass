    ...
    for (i = 0; i < nfields; i++) {
        ...
        ret = db_copy_table_by_ints(IFi->driver, IFi->database, IFi->table,
            OFi->driver, Vect_subst_var(OFi->database,
                Out), OFi->table, IFi->key, ocats[i], nocats[i]);
    ...
    struct line_pnts *Points, struct line_cats *Cats)
{
    ...
    node =
        dig_find_node(&(Out->plus), Points->x[0], Points->y[0], Points->z[0]);
    if (node) {
    ...
    if (!topo_debug) {
        ...
        dig_del_line(&Out->plus, line, APoints->x[0], APoints->y[0],
            APoints->z[0]);
    ...
    if (centr_l_n != centr_l_o || centr_r_n != centr_r_o) {
        ...
        if (centr_l_n != centr_l_o) {
            ...
            if (centr_l_o) {
                ...
                ret2 =
                    Vect_point_in_area_outer_ring(BPoints->x[0], BPoints->y[0],
                    Out, area_l_n, &abox);
                ...
                if (ret2 != ret3) {
                    ...
                    G_warning("Left old centroid in new area outer ring: %d",
                        ret2);
                    G_warning("Left old centroid in new area as poly: %d",
                        ret3);
                }
            }
        }
        if (centr_r_n != centr_r_o) {
            ...
            if (centr_r_o) {
                ...
                if (ret2 != ret3) {
                    ...
                    G_warning("Right old centroid in new area outer ring: %d",
                        ret2);
                    G_warning("Right old centroid in new area as poly: %d",
                        ret3);
                }
            }
        }
        ...
        if (centr_l_n != centr_l_o) {
            ...
            if (centr_l_o > 0) {
                ...
                if (Vect_point_in_area(BPoints->x[0], BPoints->y[0], Out,
                    ...
                    G_warning("Old left centroid is in new left area %d",
                        area_l_n);
            ...
            G_fatal_error("Left centroid old %d, restored %d", centr_l_o,
                centr_l_n);
            return 0;
        }
        if (centr_r_n != centr_r_o) {
            ...
            if (centr_r_n > 0) {
                ...
                if (Vect_point_in_area(BPoints->x[0], BPoints->y[0], Out,
                    ...
                    G_warning("New right centroid is in new right area %d",
                        area_r_n);

                    G_warning("New right centroid on outer ring: %d",
                        Vect_point_in_area_outer_ring(BPoints->x[0],
                            BPoints->y[0], Out, area_r_n, &areabox));
            ...
            if (centr_r_o > 0) {
                ...
                if (Vect_point_in_area(BPoints->x[0], BPoints->y[0], Out,
                    ...
                    G_warning("Old right centroid is in new right area %d",
                        area_r_n);
            ...
            G_fatal_error("Right centroid old %d, restored %d", centr_r_o,
                centr_r_n);
            return 0;
        }
