...
int main(int argc, char *argv[])
{
    ...
    if (Vect_open_old2(&From, opt.from->answer, "",
            opt.from_field->answer) < 0)
            ...
            while (!done) {
                ...
                for (i = 0; i < aList->n_values; i++) {
                    ...
                    line2area(&To, FPoints, ftype, aList->id[i],
                        &aList->box[i], &tmp_fx, &tmp_fy, &tmp_fz, &tmp_falong,
                        &tmp_fangle, &tmp_tx, &tmp_ty, &tmp_tz, &tmp_talong,
                        &tmp_tangle, &tmp_dist, with_z);
                    ...
                    if (poly == 3) {
                        ...
                        if (poly == 1) {
                            ...
                            for (j = 0; j < nisles; j++) {
                                ...
                                line2area(&From, TPoints, ttype, area, &fbox,
                                    &tmp2_tx, &tmp2_ty, &tmp2_tz, &tmp2_talong,
                                    &tmp2_tangle, &tmp2_fx, &tmp2_fy, &tmp2_fz,
                                    &tmp2_falong, &tmp2_fangle, &tmp2_dist,
                                    with_z);
