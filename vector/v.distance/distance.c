    ...
    if ((ttype & GV_LINES)) {
        ...
        for (i = 0; i < FPoints->n_points; i++) {
            ...
            tmp_seg = line_distance(TPoints, FPoints->x[i],
                ...
                with_z, &tmp_x, &tmp_y, &tmp_z, &tmp_dist, NULL, &tmp_along);
            if (*dist > tmp_dist) {
                *dist = tmp_dist;
                *fx = FPoints->x[i];
                *fy = FPoints->y[i];
                ...
                if (iPoints->n_points) {
                    ...
                    *fz = *tz = iPoints->z[0];
                    ...
                    fseg = line_distance(FPoints, iPoints->x[0],
                        ...
                        with_z, NULL, NULL, NULL, NULL, NULL, falong);
                    tseg = line_distance(TPoints, iPoints->x[0],
                        iPoints->y[0], iPoints->z[0],
                        with_z, NULL, NULL, NULL, NULL, NULL, talong);
                    /* fangle, tangle */
                    *fangle = sangle(FPoints, fseg);
                    *tangle = sangle(TPoints, tseg);
                }
    ...
    double *talong, double *tangle, double *dist, int with_z)
{
    ...
    *tz = Points->z[0];
    ...
    line2line(Points, type, aPoints, GV_BOUNDARY,
        fx, fy, fz, falong, fangle, tx, ty, tz, talong, tangle, dist, with_z);
