 ...
 *
 * SPDX-FileCopyrightText: 2011-2015 Other GRASS authors
...
int main(int argc, char *argv[])
{
    ...
    vector_mask_opt->description =
        _("Name of vector map with areas where the points should be imported");
    ...
    if (!notab_flag->answer && !id_layer) {
        G_message(_("-%c flag is not set but ID layer is not specified"),
                  notab_flag->key);
    ...
    if (vector_mask_opt->answer) {
        VectorMask_init(&vector_mask, vector_mask_opt->answer,
                        vector_mask_field_opt->answer,
                        (int)invert_mask_flag->answer);
    }
    ...
    if (not_valid && only_valid)
        G_message(_("%llu input points were not valid and filtered out"),
                  not_valid);
    if (n_outside)
