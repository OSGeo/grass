...
int main(int argc, char *argv[])
{
    ...
    col_opt->description =
        _("If not given and input is 3D vector map then z-coordinates are used.");
    ...
    if (vector && !map) {
        ...
        G_verbose_message(_("Points in input vector map <%s> will be interpolated"), vector);
    }
        ...
        while (last_column == FALSE) {  /* For each subregion column */
            ...
            if (grid == FALSE && ext == TRUE) {
                observ_ext =
                    ...
                    &elaboration_reg, &npoints_ext, dim_vect, 1);
            }
            else
                npoints_ext = 1;
            ...
            if (grid == TRUE && have_mask) {
                ...
                observ_ext =
                    ...
                    original_box, general_box, &npoints_ext, dim_vect, mean);
            }
            ...
            if (npoints > 0 && npoints_ext > 0) {
                ...
                if (G_strncasecmp(solver->answer, "cg", 2) == 0)
                    G_math_solver_cg_sband(N, parVect, TN, nparameters, BW,
                        atoi(iter->answer), atof(error->answer));
                else
                ...
                if (grid == TRUE) {     /* GRID INTERPOLATION ==> INTERPOLATION INTO A RASTER */
                    ...
                    if (!have_mask) {
                        P_Regular_Points(&elaboration_reg, &original_reg,
                            general_box, overlap_box, &out_seg, parVect, stepN,
                            stepE, dims.overlap, mean, nsplx, nsply, nrows,
                            ncols, bilin);
                    }
                    else {
