    ...
    struct bound_box General, int *num_points, int dim_vect, double mean)
{
    ...
    if (Original->north > General.N) {
        startrow =
            (double)((Original->north - General.N) / Original->ns_res - 1);
        if (startrow < 0)
            startrow = 0;
    }
    else
    ...
    if (General.W > Original->west) {
        startcol =
            (double)((General.W - Original->west) / Original->ew_res - 1);
        if (startcol < 0)
            startcol = 0;
    }
    else
...
int P_Sparse_Raster_Points(SEGMENT * out_seg, struct Cell_head *Elaboration,
    ...
    struct bound_box Overlap, struct Point *obs, double *param, double pe,
    double pn, double overlap, int nsplx, int nsply, int num_points, int bilin,
    double mean)
{
    int i, row, col;
    ...
    for (i = 0; i < num_points; i++) {
        ...
        if (bilin)
            interpolation =
                ...
                nsply, Elaboration->west, Elaboration->south, param);
        else
            interpolation =
 ...
 * grow each side */
int align_elaboration_box(struct Cell_head *elaboration,
    struct Cell_head *original, int type)
{
    int row, col;
