...
int main(int argc, char *argv[])
{
    ...
    Vect_copy_map_lines_field(&In, Vect_get_field_number(&In,
            opt.field->answer), &Out);
    ...
    for (i = 0; i < ntools; i++) {
        ...
        case TOOL_RMAREA:
            ...
            count = Vect_remove_small_areas(&Out, threshs[i], pErr, &size);
        ...
        case TOOL_RMSA:
            G_message(_("Tool: Remove small angles at nodes"));
            if (!flag.combine->answer) {
                count = Vect_clean_small_angles_at_nodes(&Out, otype, pErr);
            }
            else {
                int nmod;
