...
method) to find optimal parameters for given data. In this method, every
point in the input point file is temporarily excluded from the
computation and interpolation error for this point location is computed.
During this procedure no output grid files can be simultanuously
computed. The procedure for larger datasets may take a very long time,
so it might be worth to use just a sample data representing the whole
dataset.

*Example (based on [Slovakia3d
dataset](http://www.grassbook.org/data_menu2nd.php)):*
...
module [v.in.db](v.in.db.html) can be used to generate a 3D vector map
...
*[g.region](g.region.html), [v.in.ascii](v.in.ascii.html),
[r3.mask](r3.mask.html), [v.in.db](v.in.db.html),
[v.surf.rst](v.surf.rst.html), [v.univar](v.univar.html)*
