...
int main(int argc, char *argv[])
{
    ...
    if (cellinp == NULL && outz == NULL && cellout == NULL && gradient == NULL
        && aspect1 == NULL && aspect2 == NULL && ncurv == NULL && gcurv == NULL
        && mcurv == NULL) {
        sig1 = 1;
    }
    ...
    if ((data =
            data_new(x_orig, y_orig, z_orig, n_rows, n_cols, n_levs,
                0)) == NULL)
        G_fatal_error(_("Unable to create %s"), "octdata");
    if ((functions =
            OT_functions_new(oct_compare, oct_divide_data, oct_add_data,
                oct_intersect, oct_division_check, oct_get_points)) == NULL)
            ...
            if ((cellinp != NULL)) {
                ...
                if ((cellout != NULL)) {
                    ...
                    Rast_append_format_history(&hist,
                        "tension=%f, smoothing=%f", fi, rsm);
                    Rast_append_format_history(&hist,
                        "dnorm=%f, dmin=%f, wmult=%f, zmult=%f", dnorm,
                        atof(parm.dmin1->answer), wmult, zmult);
                    Rast_append_format_history(&hist,
                        "segmax=%d, npmin=%d, npmax=%d, rmsdevi=%f", KMAX,
                        npmin, KMAXPOINTS, sqrt(ertot / KMAX2));
                    Rast_append_format_history(&hist,
                        "wmin_data=%f, wmax_data=%f", wmin, wmax);
