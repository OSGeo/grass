...
int cnode ( int city ) { return ( cities[city] ); }

void add_city ( int city, 
	        int after ) /* index !!! to cycle, after which to put it */
{
    int i, j;
...
int main(int argc, char **argv)
{
    ...
    struct Option *map, *output, *afield_opt, *tfield_opt, *afcol, *type_opt, *term_opt;
    struct Flag   *geo_f;
    ...
    module->label = _("Creates a cycle connecting given nodes (Traveling salesman problem).");
    ...
    for ( i = 0; i < ncities; i++ ) {
	    ...
	    city = costs[i][j].city;
            G_debug (2, "%d -> %d = %f\n", cities[i], cities[city], costs[i][j].cost );
	}
    }
    ...
    G_debug ( 2, "biggest costs %d - %d\n", city, costs[city][ncities - 2].city );
    ...
    *  into cycle between 2 nearest nodes */
    for ( i = 0; i < ncities - 2; i++ ) {
        ...
        G_debug ( 2, "---- %d ----\n", i );
        for ( j = 0; j < ncities; j++ ) {
            if ( cused[j] == 1 ) continue;
	    tmpcost = 0;
            for ( k = 0; k < ncities - 1; k++ ) {
                G_debug ( 2, "? %d (%d) - %d (%d) \n", j, cnode(j), costs[j][k].city, 
			                                    cnode(costs[j][k].city) );
		if ( !cused[ costs[j][k].city ] ) continue; /* only used */
