...
static void update_stats(struct Stats *st, int n,
    double dx, double dy, double *dz, double dg, double d2)
{
    st->x += dx;
    st->y += dy;
    if (dz)
...
static void compute_rms(struct Control_Points *cp,
    struct Control_Points_3D *cp3, int order, int use3d, int orthorot,
    char *sep, FILE * fp)
{
    int n;
    int count, npoints;
    ...
    if (use3d)
        ...
        fprintf(fp,
            "index%sfwd_dx%sfwd_dy%sback_dx%sback_dy%sfwd_RMS%sback_RMS", sep,
            sep, sep, sep, sep, sep);
