...
int main(int argc, char *argv[])
{
    ...
    struct Option *old, *new, *delim_opt, *tdelim_opt, *columns_opt,
        *xcol_opt, *ycol_opt, *zcol_opt, *catcol_opt, *format_opt, *skip_opt;
    ...
    G_asprintf(&desc,
               "point;%s;standard;%s",
               _("simple x,y[,z] list"), _("GRASS vector ASCII format"));
    ...
    ycol_opt->label =
        _("Number of column used as y coordinate (points mode)");
    ...
    zcol_opt->label =
        _("Number of column used as z coordinate (points mode)");
    ...
    if (zcoorf->answer && format == GV_ASCII_FORMAT_POINT &&
        !zcol_opt->answer)
    ...
    if (format == GV_ASCII_FORMAT_POINT) {
        ...
        points_analyse(ascii, tmpascii, fs, td, &rowlen, &ncols, &minncols,
                       &nrows, &coltype, &colsample, &collen, skip_lines,
                       xcol, ycol, zcol, catcol, region_flag->answer,
                       ignore_flag->answer);
