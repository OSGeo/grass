...
int export_lines(struct Map_info *In, int field, int otype, int multi,
    ...
    OGRLayerH Ogr_layer, struct field_info *Fi, dbDriver * driver, int ncol,
    int *colctype, const char **colname, int doatt, int nocat, int *n_noatt,
    int *n_nocat)
{
    if (multi)
    ...
    return export_lines_single(In, field, otype, donocat, force_poly,
        Ogr_featuredefn, Ogr_layer,
        Fi, driver, ncol, colctype, colname, doatt, nocat, n_noatt, n_nocat);
}

/* export line as single feature */
int export_lines_single(struct Map_info *In, int field, int otype, int donocat,
    int force_poly, OGRFeatureDefnH Ogr_featuredefn, OGRLayerH Ogr_layer,
    struct field_info *Fi, dbDriver * driver, int ncol, int *colctype,
    const char **colname, int doatt, int nocat, int *n_noatt, int *n_nocat)
{
