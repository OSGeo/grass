...
int main(int argc, char *argv[])
{
        ...
        if (Ogr_projection) {
            ...
            if ((obj = proj_create(NULL, inwkt))) {
                if (proj_get_type(obj) == PJ_TYPE_BOUND_CRS) {
                    ...
                    if (source_crs) {
                        inwkt =
                            G_store(proj_as_wkt(NULL, source_crs,
                                PJ_WKT2_LATEST, NULL));
                        if (inwkt && !*inwkt) {
                            G_free(inwkt);
                            inwkt = NULL;
    ...
    if (otype & (GV_POINTS | GV_LINES | GV_KERNEL | GV_FACE)) {
        ...
        n_feat +=
            export_lines(&In, field, otype, flags.multi->answer ? TRUE : FALSE,
            donocat, ftype == GV_BOUNDARY ? TRUE : FALSE, Ogr_featuredefn,
            Ogr_layer, Fi, Driver, ncol, colctype, colname, doatt,
    ...
    if (Vect_get_num_areas(&In) > 0 && (otype & GV_AREA)) {
        ...
        n_feat +=
            ...
            donocat, Ogr_featuredefn, Ogr_layer, Fi, Driver, ncol, colctype,
            colname, doatt, flags.nocat->answer ? TRUE : FALSE, &n_noatt,
