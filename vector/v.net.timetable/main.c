    ...
    int from_time, int to_time)
{
    ...
    sprintf(buf, "insert into %s values (%d, %d, %d, %d, %d, %d, %d, %d)",
        table, cat, path, from_id, to_id, route_id, index, from_time, to_time);
    db_set_string(&sql, buf);
    G_debug(3, "%s", db_get_string(&sql));
    if (db_execute_immediate(driver, &sql) != DB_OK) {
        db_close_database_shutdown_driver(driver);
...
int main(int argc, char *argv[])
{
    ...
    stop_time_opt->description = _("Name of column with stop timestamps");
    ...
    if (NetA_init_timetable_from_db
        (&In, tfield, atoi(walk_layer_opt->answer), route_id_opt->answer,
            stop_time_opt->answer, to_stop_opt->answer,
            walk_length_opt->answer, &timetable, &route_ids, &stop_ids) != 0)
    ...
    if (afield > 0) {
        ...
        if (0 != Vect_net_build_graph(&In, mask_type, afield, nfield,
                afcol->answer, abcol->answer, ncol->answer, 0, 0))
    ...
    while (1) {
        ...
        ret =
            ...
            &tx, &ty, &start_time, &min_change, &max_changes, &walking_change);
        if (ret == 9) {
        ...
        ret =
            ...
            start_time, min_change, max_changes, walking_change, &result);
        if (ret == -1) {
