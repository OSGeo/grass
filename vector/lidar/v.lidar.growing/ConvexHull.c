...
/*----------------------------------------------------------------------------------------------------*/
void regGrow8 (struct Cell_head Elaboration, struct element_grow **mat, double **punti, int *lung, int r, int c,
	      int v, double Th_j, int maxP)
{
	...
	if (c - 1 >= 0) {
	    if ((mat[r][c - 1].clas > Th_j) && (mat[r][c - 1].clas < v) && (mat[r][c - 1].fi != 0)) 
	...
	if ((r - 1 >= 0) && (c - 1 >= 0)) {
	    if ((mat[r - 1][c - 1].clas > Th_j) && (mat[r - 1][c - 1].clas < v) && (mat[r - 1][c - 1].fi != 0))
		regGrow8(Elaboration, mat, punti, lung, r - 1, c - 1, v, Th_j, maxP);
	}
...
int checkHull(int cR, int cC, double **oldHull, int lungOld)
{
	...
	if (lungOld != lungHullNew) {
	    ...
	    for (count = 0; count < lungOld; count++) {
		if ((oldHull[count][0] != newP[count][0]) || (oldHull[count][1] != newP[count][1])) {
		    G_free_matrix(newPoint);
		    free_Pvector(newP, 0, lungOld + 1);
		    return 0;
		}
...
struct element_grow **P_alloc_element(int rows, int cols)
{
     ...
     m = (struct element_grow **) G_calloc ((rows+1), sizeof(struct element_grow *));
     m[0] = (struct element_grow *) G_calloc (rows*cols, sizeof(struct element_grow));
...
struct element_grow **structMatrix(long nrl, long nrh, long ncl, long nch)
{
	...
	m = (struct element_grow **)calloc((size_t) (nrow + NR_END), sizeof(struct element_grow *));
	...
	m[nrl] = (struct element_grow *)calloc((size_t) (nrow * ncol + NR_END), sizeof(struct element_grow));
...
void free_structmatrix (struct element_grow **m, long nrl, long nrh, long ncl, long nch)
{
