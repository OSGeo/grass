...
int 
transform_digit_file (struct Map_info *Old, struct Map_info *New, int shift_file,
		      double ztozero, double* trans_params_def, 
		      char *table, char** columns, int field)
{
	...
	if (table) { 
		...
		for (j = 0; j <= IDX_ZROT; j++) {
			...
			break;
		    case -1:
			G_fatal_error(_("Missing column <%s> in table <%s>"), columns[j], table);
		    default:
			G_fatal_error(_("Unsupported column type of <%s>"), columns[j]);
		    }
		    if (db_select_value(driver, table, fi->key, cat, columns[j], &val) != 1 ||
	...
	for ( i = 0; i < Points->n_points; i++ ) {
		...
		G_debug(3, "idx=%d, cat=%d, xshift=%g, yshift=%g, zshift=%g, "
			...
			i, cat, trans_params[IDX_XSHIFT], trans_params[IDX_YSHIFT], trans_params[IDX_ZSHIFT],
			trans_params[IDX_XSCALE], trans_params[IDX_YSCALE], trans_params[IDX_ZSCALE],
			trans_params[IDX_ZROT]);

		/* transform point */
		x = trans_params[IDX_XSHIFT] + trans_params[IDX_XSCALE] * Points->x[i] * cos(ang)
		    - trans_params[IDX_YSCALE] * Points->y[i] * sin(ang); 
		y = trans_params[IDX_YSHIFT] + trans_params[IDX_XSCALE] * Points->x[i] * sin(ang)
		...
		Points->y[i] = y;
             ...
             * to target elevation: */
            Points->z[i] = ((Points->z[i] + ztozero) * trans_params[IDX_ZSCALE]) + trans_params[IDX_ZSHIFT];
	}
