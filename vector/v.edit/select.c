...
struct ilist *select_lines(struct Map_info *Map, enum mode action_mode,
    struct GParams *params, double *thresh, struct ilist *List)
{
    int layer, type;
...
int sel_by_coordinates(struct Map_info *Map,
    int type, struct line_pnts *coords, double thresh, struct ilist *List)
{
    int i;
...
int sel_by_bbox(struct Map_info *Map,
    int type, double x1, double y1, double x2, double y2, struct ilist *List)
{
