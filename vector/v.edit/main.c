...
int main(int argc, char *argv[])
{
        ...
        while (params.bmaps->answers[i]) {
            ...
            if (strcmp(G_fully_qualified_name(params.map->answer, G_mapset()),
                ...
                G_fatal_error(_("Unable to open vector map <%s> as the background map. "
                        "It is given as vector map to be edited."), bmap);
            }
            nbgmaps++;
            BgMap =
                (struct Map_info **)G_realloc(BgMap,
                nbgmaps * sizeof(struct Map_info *));
            BgMap[nbgmaps - 1] =
        ...
        if (ret > 0) {
            ...
            if (snap != NO_SNAP) {      /* apply snapping */
                ...
                Vedit_snap_lines(&Map, BgMap, nbgmaps, List_added,
                    thresh[THRESH_SNAP], snap == SNAP ? FALSE : TRUE);
            }
    ...
    case MODE_VERTEX_MOVE:
        ...
        ret =
            ...
            thresh[THRESH_COORDS], thresh[THRESH_SNAP], move_x, move_y, move_z,
            move_first, snap);
        ...
        if (BgMap && BgMap[0]) {
            if (nbgmaps > 1)
                G_warning(_("Multiple background maps were given. "
                        ...
                        "vector map <%s>."), Vect_get_full_name(BgMap[0]));
