 ...
 * SPDX-FileCopyrightText: 2000-2009, 2011-2012 GRASS Development Team
...
int main(int argc, char *argv[])
{
    ...
    if (ver == 4 && format == GV_ASCII_FORMAT_POINT) {
        G_fatal_error(_("Format '%s' is not supported for old version"),
                      "point");
    }

    if (ver == 4 && strcmp(output, "-") == 0) {
        G_fatal_error(_("Parameter '%s' must be given for old version"),
                      "output");
    }
    ...
    if (ver == 4 && !pnt) {
        ...
        if ((att = G_fopen_new("dig_att", output)) == NULL)
            G_fatal_error(_("Unable to open dig_att file <%s>"), output);
    }
