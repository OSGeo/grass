...
#ifndef GEOS_3_3
    G_warning(_("Flags -%c/%c ignored by this version, GEOS >= 3.3 is required"), 's',
        'c');
#endif
#endif
            ...
            if (ltype & GV_POINTS || Points->n_points == 1) {
                if (straight_flag->answer) {
                    ...
                    Vect_append_point(arr_bc_pts.oPoints,
                        arr_bc_pts.oPoints->x[0], arr_bc_pts.oPoints->y[0],
                        arr_bc_pts.oPoints->z[0]);
                }
                else {
                    Vect_point_buffer2(Points->x[0], Points->y[0], da, db,
                        dalpha, !(straight_flag->answer), unit_tolerance,
                        &(arr_bc_pts.oPoints));
                }
