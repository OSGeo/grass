...
int copy_records(dbDriver * driver_in, dbString * table_name_in,
    dbDriver * driver_out, dbString * table_name_out, char *, int, int);
    ...
    *colnames = '\0';
        ...
        while (old->answers[i]) {
            ...
            if (i > 0 || append->answer) {
                ...
                for (col = 0; col < ncols; col++) {
                    ...
                    if (G_strcasecmp(key, db_get_column_name(column_out)) == 0) {
                        keycol = col;
                    }
                }
            }
    ...
    if (do_table) {
        ...
        if (fi_out) {
            driver_out =
                db_start_driver_open_database(fi_out->driver,
                Vect_subst_var(fi_out->database, &OutMap));
            if (!driver_out) {
                G_fatal_error(_("Unable to open database <%s> by driver <%s>"),
                    fi_out->database, fi_out->driver);
            }
        ...
        if (!append->answer) {
            ...
            if (db_create_index2(driver_out, fi_out->table,
                    fi_out->key) != DB_OK)
        ...
        if (do_table) {
            ...
            if (fi_in) {
                ...
                copy_records(driver_in, &table_name_in,
                    driver_out, &table_name_out, colnames, keycol, add_cat);
