...
double
numfunc (SYMBOL *func, SYMBOL *arglist)
{
  double res = 0.0;
  int argc = -1;
  ...
  if (argc == 0 && (!func->proto || !*func->proto))
    ...
    res = (*(d_func_dd)func->v.p)(arglist->v.d,
				  arglist->next->v.d);
  ...
  else if (argc == 3 && !strcmp (func->proto, "dai"))
  {
    ...
    res = (*(d_func_pp)func->v.p)(arglist->v.p,
				  arglist->next->v.p);
...
double
numop (int op, SYMBOL *opd1, SYMBOL *opd2)
{
