...
int centroid(OGRGeometryH hGeomAny, CENTR * Centr,
    struct spatial_index *Sindex, int field, int cat, double min_area,
    int type);
...
void check_projection(struct Cell_head *cellhd, ds_t hDS, int layer,
    char *geom_col, char *outloc, int create_only, int override,
    int check_only);

int create_spatial_filter(ds_t Ogr_ds, OGRGeometryH *,
    int, int *, char **,
    double *, double *, double *, double *, int, struct Option *);
...
void OGR_iterator_init(struct OGR_iterator *OGR_iter,
    ds_t Ogr_ds, char *dsn, int nlayers, int ogr_interleaved_reading);
...
int main(int argc, char *argv[])
{
    ...
    struct _flag
    {
        struct Flag *list, *no_clean, *force2d, *notab,
            *region, *over, *extend, *formats, *tolower, *no_import, *proj;
    } flag;
    ...
    G_asprintf(&desc,
        ...
        _("import lines as area boundaries"), _("import points as centroids"));
    ...
    if (flag.formats->answer) {
        ...
        for (iDriver = 0; iDriver < OGRGetDriverCount(); iDriver++) {
            ...
            fprintf(stdout, " %s (%s): %s\n",
                OGR_Dr_GetName(poDriver), pszRWFlag, OGR_Dr_GetName(poDriver));
        }
#endif
        exit(EXIT_SUCCESS);
    }
    ...
    check_projection(&cellhd, Ogr_ds, layers[0], param.geom->answer,
        param.outloc->answer,
        flag.no_import->answer, flag.over->answer, flag.proj->answer);
    ...
    if (create_spatial_filter(Ogr_ds, poSpatialFilter,
            ...
            &xmin, &ymin, &xmax, &ymax, flag.region->answer, param.spat)
        || attr_filter) {
        ...
        for (layer = 0; layer < nlayers; layer++) {
            ...
            if (param.geom->answer) {
                Ogr_featuredefn = OGR_L_GetLayerDefn(Ogr_layer);
                igeom =
                    OGR_FD_GetGeomFieldIndex(Ogr_featuredefn,
                    param.geom->answer);
                if (igeom < 0)
            ...
            OGR_L_SetSpatialFilter(Ogr_layer, poSpatialFilter[layer]);
#endif
            if (OGR_L_SetAttributeFilter(Ogr_layer,
                    attr_filter) != OGRERR_NONE)
...
for (layer = 0; layer < nlayers; layer++) {
    ...
    if (!flag.notab->answer) {
            ...
            for (i = 0; i < ncols_out - 1; i++) {
                ...
                while (i + i_a < ncols_out &&
                    ...
                    G_important_message(_("Column name <%s> renamed to <%s_%d>"),
                        col_info[i + i_a].name, col_info[i + i_a].name, i_a);
        ...
        qsort(col_info, ncols_out, sizeof(struct grass_col_info), cmp_col_idx);
        ...
        for (i = 0; i < ncols; i++) {
            ...
            if (Ogr_fstring && *Ogr_fstring) {
                ...
                /* should we use OGR_F_GetFieldAsDateTime() here ? */
                    ...
                    Ogr_ftype == OFTStringList || Ogr_ftype == OFTIntegerList
#if GDAL_VERSION_NUM >= 2000000
                    || Ogr_ftype == OFTInteger64List
#endif
                    ) {
                ...
                if (Ogr_ftype == OFTInteger ||
                    ...
                    Ogr_ftype == OFTTime || Ogr_ftype == OFTDateTime) {
                    G_rasprintf(&sqlbuf, &sqlbufsize, ", NULL");
                }
#endif
                else if (Ogr_ftype == OFTString ||
...
if (n_overlaps) {
    G_message(_("Overlapping area: %G (%d areas)"), overlap_area, n_overlaps);
}
Vect_hist_write(&Map, buf);

sprintf(buf, _("Area without category: %G (%d areas)\n"), nocat_area, n_nocat);
if (n_nocat) {
    G_message(_("Area without category: %G (%d areas)"), nocat_area, n_nocat);
}
Vect_hist_write(&Map, buf);
G_message("%s", separator);
}
...
if (failed_centr || err_boundaries || err_centr_out || err_centr_dupl
    ...
    if (failed_centr || err_boundaries || err_centr_out || err_centr_dupl) {
        ...
        if (err_boundaries) {
            strcat(error_msg, "\n");
            sprintf(error_msg + strlen(error_msg),
                _("Number of incorrect boundaries: %d"), err_boundaries);
        }
        if (err_centr_out) {
            strcat(error_msg, "\n");
            sprintf(error_msg + strlen(error_msg),
                _("Number of centroids outside area: %d"), err_centr_out);
        }
        if (err_centr_dupl) {
            strcat(error_msg, "\n");
            sprintf(error_msg + strlen(error_msg),
        ...
        if (snap < max_snap) {
            G_important_message(_("Estimated range of snapping threshold: [%g, %g]"), min_snap,
                max_snap);
        }
        ...
        if (snap < 0) {
            ...
            e1 = (int)((e1 + e2) / 2. - 0.5);
            snap = pow(10, e1);
            G_important_message(_("Try to import again, snapping with %g: 'snap=%g'"), snap,
                snap);
        }
        else if (snap < min_snap) {
            G_important_message(_("Try to import again, snapping with at least %g: 'snap=%g'"),
                min_snap, min_snap);
        }
        else if (snap * 10 < max_snap) {
            min_snap = snap * 10;
            G_important_message(_("Try to import again, snapping with %g: 'snap=%g'"), min_snap,
                min_snap);
        }
        else
    ...
    /* overlapping polygons */
        ...
        if (snap > 0) {
            if (snap < min_snap) {
                ...
                min_snap = snap * 10;
                G_important_message(_("Try to import again, snapping with %g: 'snap=%g'"),
                    min_snap, min_snap);
            }
            else
                /* assume manual cleaning is required */
    ...
    /* number of centroids does not match number of input polygons */
        ...
        if (ncentr < n_polygons) {
            G_important_message(_("%d input polygons got lost during import."),
                n_polygons - ncentr);
        }
        if (ncentr > n_polygons) {
            G_important_message(_("%d additional areas where created during import."),
                ncentr - n_polygons);
        }
        if (snap > 0) {
...
OGRFeatureH ogr_getnextfeature(struct OGR_iterator *OGR_iter,
    ...
    OGRGeometryH poSpatialFilter, const char *attr_filter)
{
    if (OGR_iter->requested_layer != layer) {
        ...
        if (!OGR_iter->ogr_interleaved_reading) {
            ...
            if (OGR_L_SetAttributeFilter(OGR_iter->Ogr_layer,
                    attr_filter) != OGRERR_NONE)
                G_fatal_error(_("Error setting attribute filter '%s'"),
                    attr_filter);
#if GDAL_VERSION_NUM < 2020000
    ...
    if (!OGR_iter->ogr_interleaved_reading) {
        ...
        while (1) {
            ...
            Ogr_feature = GDALDatasetGetNextFeature(OGR_iter->Ogr_ds,
                &(OGR_iter->Ogr_layer), NULL, NULL, NULL);
...
int create_spatial_filter(ds_t Ogr_ds, OGRGeometryH * poSpatialFilter,
    ...
    double *xmax, double *ymax, int use_region, struct Option *spat)
{
    int layer;
    ...
    for (layer = 0; layer < nlayers; layer++) {
        ...
        if (have_filter) {
            ...
            G_debug(2,
                "spatial filter for layer <%s>: xmin:%f ymin:%f xmax:%f ymax:%f",
                layer_names[layer], xminl[layer], yminl[layer], xmaxl[layer],
                ymaxl[layer]);
