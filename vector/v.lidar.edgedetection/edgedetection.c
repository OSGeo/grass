...
int edge_detection(struct Cell_head elaboration_reg,
                   struct bound_box Overlap_Box, double *parBilin,
                   double obsX, double obsY, double *partial, double alpha,
                   double residual, double gradHigh, double gradLow)
{
    /* 1 = PRE_TERRAIN */
    /* 2 = PRE_EDGE */
    /* 3 = PRE_UNKNOWN */
    ...
    if ((gradPto > gradHigh) && (residual > 0))
        ...
        if (Vect_point_in_box(obsX, obsY, 0.0, &Overlap_Box)) {
            ...
            Get_Gradient(Elaboration, obsX + stepE * cos(dirPto + M_PI),
                         obsY + stepN * sin(dirPto + M_PI), parBilin,
                         gradient);
            g[7][0] = gradient[0];
            g[7][1] = gradient[1];
...
void classification(struct Map_info *Out, struct Cell_head Elaboration,
                    ...
                    double mean, double alpha, double gradHigh,
                    double gradLow, double overlap, int *line_num,
                    int num_points, dbDriver * driver, char *tabint_name,
                    char *tab_name)
{
    int i, edge;
