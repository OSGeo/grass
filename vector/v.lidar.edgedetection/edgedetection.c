...
int edge_detection(struct Cell_head elaboration_reg,
    struct bound_box Overlap_Box, double *parBilin, double obsX, double obsY,
    double *partial, double alpha, double residual, double gradHigh,
    double gradLow)
{
    /* 1 = PRE_TERRAIN */
    /* 2 = PRE_EDGE */
        ...
        if (Vect_point_in_box(obsX, obsY, 0.0, &Overlap_Box)) {
            ...
            if ((fabs(atan(g[2][1] / g[2][0]) + M_PI / 2 - dirPto) < alpha) &&
                ...
                Get_Gradient(Elaboration,
                    ...
                    obsY + stepN * sin(dirPto + M_PI / 4), parBilin, gradient);
                g[1][0] = gradient[0];
                g[1][1] = gradient[1];
                ...
                Get_Gradient(Elaboration,
                    ...
                    obsY + stepN * sin(dirPto + M_PI / 2), parBilin, gradient);
                g[4][0] = gradient[0];
                g[4][1] = gradient[1];
    ...
    for (i = 0; i < num_points; i++) {  /* Sparse points */
        ...
        if (Vect_point_in_box(obs[i][0], obs[i][1], mean, &General)) {
            interpolation =
                ...
                nsplx, nsply, Elaboration.west, Elaboration.south, parBicub);
            interpolation += mean;
            ...
            *point->z += mean;
            ...
            if (Vect_point_in_box(obs[i][0], obs[i][1], interpolation, &Overlap)) {     /*(5) */
                ...
                edge =
                    ...
                    *point->y, gradient, alpha, residual, gradHigh, gradLow);
    ...
    int line_num, dbDriver * driver, char *tab_name)
{
    ...
    db_init_string(&sql);
    sprintf(buf,
        "SELECT ID, Interp, X, Y FROM %s WHERE ID=%d", tab_name, line_num);
    db_append_string(&sql, buf);
