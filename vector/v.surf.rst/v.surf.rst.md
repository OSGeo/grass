...
partial derivatives f~x~,f~y~,f~xx~, f~yy~,f~xy~ instead of slope,
aspect, profile, tangential and mean curvatures respectively. If the
input vector map have time stamp, the program creates time stamp for all
output maps.

User can either use *[r.mask](r.mask.html)* to set a mask or specify a
...
The user must run *[g.region](g.region.html)* before the program to set
...
using a cross-validation (CV) procedure (**-c** flag). The CV procedure
is based on removing one input data point at a time, performing the
...
also known as the \"leave-one-out\" or \"jack-knife\" method (Hofierka
et al., 2002; Hofierka, 2005). The differences (residuals) are then
stored in the **cvdev** output vector map. Please note that during the
CV procedure no other output maps can be set, the approximation is
...
e.g. *[v.univar](v.univar.html)*. It should be noted that
crossvalidation is a time-consuming procedure, usually reasonable for up
to several thousands of points. For larger data sets, CV should be
applied to a representative subset of the data. The cross-validation
procedure works well only for well-sampled phenomena and when minimizing
the predictive error is the goal. The parameters found by minimizing the
predictive (CV) error may not not be the best for for poorly sampled
phenomena (result could be strongly smoothed with lost details and
...
-   Hofierka, J., 2005, Interpolation of Radioactivity Data Using
    Regularized Spline with Tension. Applied GIS, Vol. 1, No. 2, pp.
    16-01 to 16-13. DOI: 10.2104/ag050016
...
-   H. Mitasova, L. Mitas, B.M. Brown, D.P. Gerdes, I. Kosinovsky, 1995,
    ...
    special issue on Integrating GIS and Environmental modeling,
    433-446.
...
-   [Neteler, M. and Mitasova, H., 2008, Open Source GIS: A GRASS GIS
    Approach, 3rd Edition,](http://www.grassbook.org) Springer, New
    York, 406 pages.
...
*[v.vol.rst](v.vol.rst.html), [v.surf.idw](v.surf.idw.html),
[v.surf.bspline](v.surf.bspline.html), [r.fillnulls](r.fillnulls.html),
