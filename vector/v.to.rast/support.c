    ...
    const char *rgb_column, int is_fp, const char *attr_column)
{
    ...
    if ((nrec =
            db_select_CatValArray(Driver, Fi->table, Fi->key, attr_column,
                NULL, &cvarr)) == -1)
...
int update_labels(const char *rast_name, const char *vector_map, int field,
    const char *label_column, int use, int val, const char *attr_column)
{
    int i, j;
    ...
    case USE_ATTR:
        {
            ...
            if (Vect_open_old(&Map, vector_map, G_find_vector2(vector_map,
                        "")) < 0)
                G_fatal_error(_("Unable to open vector map <%s>"), vector_map);
                ...
                if (is_fp) {
                    ...
                    for (i = 1; i < labels_n_values; i++) {
                        if (my_labels_rules[i - 1].d != my_labels_rules[i].d) {
                            ...
                            if (strcmp(db_get_string(&my_labels_rules[i].
                                        label),
                                    db_get_string(&my_labels_rules[i -
                                            1].label)) != 0) {
    ...
    case USE_VAL:
        {
            ...
            Rast_set_cats_title("Rasterized vector map from values",
                &rast_cats);
