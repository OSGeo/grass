...
int vect_to_rast(const char *vector_map, const char *raster_map,
    const char *field_name, const char *column, int cache_mb, int use,
    double value, int value_type, const char *rgbcolumn,
    const char *labelcolumn, int ftype, char *where, char *cats, int dense)
{
    struct Map_info Map;
    ...
    case USE_ATTR:
        ...
        if ((nrec =
                db_select_CatValArray(Driver, Fi->table, Fi->key, column,
                    where, &cvarr)) == -1)
        ...
        for (i = 0; i < cvarr.n_values; i++) {
            ...
            if (!cat_list ||
                Vect_cat_in_cat_list(cvarr.value[i].cat, cat_list)) {
    ...
    if (use != USE_Z && use != USE_D && (ftype & GV_AREA)) {
        if ((nareas = sort_areas(&Map, Points, field, cat_list)) == 0)
            G_warning(_("No areas selected from vector map <%s>"), vector_map);
