 ...
 *
 * SPDX-FileCopyrightText: 2001-2008,2014,2017 Other GRASS authors
...
typedef struct {     /* iso point along the line */
    int iso;         /* index of iso line in iso array of costs */
    double distance; /* distance along the line from the beginning for both
                        directions */
...
int main(int argc, char **argv)
{
    ...
    ucat_f->description = _("Default: one category for each iso-band");
    ...
    if (niso < 2)
        G_warning(
            _("Not enough costs, everything reachable falls to first band"));
    ...
    for (line = 1; line <= nlines; line++) {
        ...
        if ((center1 != -1 && n1cost != -1 && e1cost != -1) ||
            ...
            /* Find points on isolines along the line in both directions, add
             * them to array, first point is placed at the beginning/end of line
             */
            /* Forward */
            ...
            if (center1 != -1 && n1cost != -1 && e1cost != -1) {
                ...
                while (next_iso < niso) {
                    ...
                    if (npnts1 == apnts1) {
                        apnts1 += 1;
                        pnts1 = (ISOPOINT *)G_realloc(
                            pnts1, apnts1 * sizeof(ISOPOINT));
                    }
                    pnts1[npnts1].iso = next_iso;
                    pnts1[npnts1].distance = l1;
                    pnts1[npnts1].center = center1;
                    G_debug(3, "  forward %d : iso %d : distance %f : cost %f",
            ...
            if (center2 != -1 && n2cost != -1 && e2cost != -1) {
                ...
                while (next_iso < niso) {
                    ...
                    if (npnts2 == apnts2) {
                        apnts2 += 1;
                        pnts2 = (ISOPOINT *)G_realloc(
                            pnts2, apnts2 * sizeof(ISOPOINT));
                    }
                    pnts2[npnts2].iso = next_iso;
                    pnts2[npnts2].distance = l - l1;
                    pnts2[npnts2].center = center2;
                    G_debug(3, "  backward %d : iso %d : distance %f : cost %f",
            ...
            /* Limit number of points by maximum costs in reverse direction,
             * this may remove also the first point in one direction, but not in
             * both */
            /* Forward */
            if (npnts2 > 0) {
                for (i = 0; i < npnts1; i++) {
                    G_debug(3, "  pnt1 = %d dist1 = %f iso1 = %d max iso2 = %d",
            ...
            if (npnts1 > 0) {
                for (i = 0; i < npnts2; i++) {
                    G_debug(3, "  pnt2 = %d dist2 = %f iso2 = %d max iso1 = %d",
            ...
            /* Biggest cost should be equal if exist (npnts > 0). Cut out
             * overlapping segments, this can cut only points on line but not
             * first points */
            if (npnts1 > 1 && npnts2 > 1) {
                while (npnts1 > 1 && npnts2 > 1) {
                    if (pnts1[npnts1 - 1].distance >=
                        pnts2[npnts2 - 1].distance) { /* overlap */
                        npnts1--;
                        npnts2--;
                    }
                    else {
            ...
            /* Now we have points in both directions which may not overlap,
             * npoints in one direction may be 0 but not both */

            /* Join both arrays, iso of point is for next segment (point is at
             * the beginning) */
            /* In case npnts1 == 0 add point at distance 0 */
            if (npnts1 == 0) {
                G_debug(3, "  npnts1 = 0 -> add first at distance 0, cat = %d",
                        pnts2[npnts2 - 1].iso);
                pnts1[0].iso =
                    pnts2[npnts2 - 1]
                        .iso; /* use last point iso in reverse direction */
                pnts1[0].distance = 0;
            ...
            for (i = npnts2 - 1; i >= 0; i--) {
                ...
                if (npnts1 == apnts1) {
                    apnts1 += 1;
                    pnts1 =
                        (ISOPOINT *)G_realloc(pnts1, apnts1 * sizeof(ISOPOINT));
                }
                pnts1[npnts1].iso =
                    pnts2[i].iso - 1; /* last may be -1, but it is not used */
            ...
            for (i = 1; i < npnts1; i++) {
                ...
                ret = Vect_line_segment(Points, pnts1[i - 1].distance,
