...
int main(int argc, char **argv)
{
    ...
    struct Option *afield_opt, *nfield_opt, *afcol, *abcol, *ncol, *type_opt, *term_opt, *cost_opt;
    struct Flag   *geo_f;
	     ...
	     ( centre2 != -1 && n2cost != -1 && e2cost != -1 ) )
	{
	    ...
	    *  first point is placed at the beginning/end of line */
		...
		while ( next_iso < niso ) {
		    if ( e1cost == 0 ) break; /* Outside line */ 
		    l1 = (iso[next_iso] - s1cost) / e1cost;
		    if ( l1 >= l ) break; /* Outside line */
		    
		    if ( npnts1 == apnts1 ) {
			apnts1 += 1;
			pnts1 = (ISOPOINT *) G_realloc ( pnts1, apnts1*sizeof(ISOPOINT));
		    }
		...
		/* Calculate distances for points along line */
		while ( next_iso < niso ) {
		    if ( e2cost == 0 ) break; /* Outside line */ 
		    l1 = (iso[next_iso] - s2cost) / e2cost;
		    if ( l1 >= l ) break; /* Outside line */
		    
		    if ( npnts2 == apnts2 ) {
			apnts2 += 1;
			pnts2 = (ISOPOINT *) G_realloc ( pnts2, apnts2*sizeof(ISOPOINT));
		    }
	    ...
	    *  also the first point in one direction, but not in both */
		...
		for ( i = 0; i < npnts1; i++ ) {
		    G_debug ( 3, "  pnt1 = %d dist1 = %f iso1 = %d max iso2 = %d", 
				    i, pnts1[i].distance, pnts1[i].iso, pnts2[npnts2-1].iso);
		...
		for ( i = 0; i < npnts2; i++ ) {
		    G_debug ( 3, "  pnt2 = %d dist2 = %f iso2 = %d max iso1 = %d", 
				    i, pnts2[i].distance, pnts2[i].iso, pnts1[npnts1-1].iso);
	    ...
	    *  direction may be 0 but not both */
	    ...
	    if ( npnts1 == 0 ) {
                G_debug ( 3, "  npnts1 = 0 -> add first at distance 0, cat = %d", pnts2[npnts2-1].iso);
		...
		/* Check if identical */
		if ( pnts1[npnts1-1].distance ==  pnts2[i].distance ) continue;
	    ...
	    for ( i = 1; i < npnts1; i++ ) {
		cat = pnts1[i-1].iso + 1;
                G_debug ( 3, "  segment %f - %f cat %d", pnts1[i-1].distance, pnts1[i].distance, cat);
		ret = Vect_line_segment ( Points, pnts1[i-1].distance, pnts1[i].distance, SPoints );
		if ( ret == 0 ) {
