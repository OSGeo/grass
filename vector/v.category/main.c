...
int main(int argc, char *argv[])
{
    ...
    if (option == O_LYR) {
        ...
        if (Vect_open_old_head2(&In, in_opt->answer, "",
                field_opt->answer) < 2) {
    ...
    if (open_level > 1) {
        ...
        if (Vect_open_old2(&In, in_opt->answer, "",
                field_opt->answer) < open_level) {
    ...
    if (option == O_TRANS && nmodified > 0)
        for (i = 1; i < nfields; i++)
            G_important_message(_("Categories copied from layer %d to layer %d"), fields[0],
                fields[i]);
