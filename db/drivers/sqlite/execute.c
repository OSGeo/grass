...
int db__driver_execute_immediate(dbString * sql)
{
    ...
    if (ret != SQLITE_OK) {
        db_d_append_error("%s\n%s",
            _("Error in sqlite3_finalize():"), (char *)sqlite3_errmsg(sqlite));
        db_d_report_error();
        return DB_FAILED;
    }
...
int db__driver_begin_transaction(void)
{
    ...
    if (ret != SQLITE_OK) {
        db_d_append_error("%s\n%s",
            _("'BEGIN' transaction failed:"), (char *)sqlite3_errmsg(sqlite));
        db_d_report_error();
        return DB_FAILED;
    }
...
int db__driver_commit_transaction(void)
{
    ...
    if (ret != SQLITE_OK) {
        db_d_append_error("%s\n%s",
            _("'COMMIT' transaction failed:"), (char *)sqlite3_errmsg(sqlite));
        db_d_report_error();
        return DB_FAILED;
    }
