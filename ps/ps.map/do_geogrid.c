...
int do_geogrid_numbers (void)
{
	...
	if (first)
	{
	    ...
	    hx = x + 0.7 * strlen(num_text) * PS.geogrid_fontsize + 2 * margin;
	}
    }
	...
	if (x - PS.grid_fontsize < last_right) continue;
	if (x + PS.grid_fontsize > (int)PS.map_right) continue;
...
********************************************************/
void
get_ll_bounds(double *w, double *e, double *s, double *n)
{
    double east, west, north, south;
    double e1, w1, n1, s1;
    double ew, ns;
...
********************************************************/
void
check_coords(double e, double n, double *lon, double *lat, int par)
{
    double x, y;
    int proj = 0;
