...
int PS_vlegend(void)
{
    ...
    /* vec[][] is array of vectors on each row of legend, first to be drawn is
     * first in array, label is stored in last in row (last plotted, first in
     * the script) nvec[] is number of vector on each row index for both start
     * at 0, lpos in script starts from 1 If position is not used by any vector
     * it is used for next not used vector without lpos or lpos > vector.count
     */
    ...
    for (l = 0; l < vector.count; l++) {
        nvec[l] = 0;
        for (i = vector.count - 1; i >= 0;
             i--) { /* last in script plot first */
    ...
    for (h = 0; h < vector.cols; h++) {
        ...
        for (j = h * lc; j < st; j++) { /* each row */
            ...
            for (k = 0; k < nvec[j]; k++) {
                ...
                /* TODO */
                /*
                   if ((vector.layer[i].hwidth > 0. && vector.layer[i].hcolor ==
                   WHITE) || (vector.layer[i].hwidth < 1. &&
                   vector.layer[i].colors[0]  == WHITE))
                   {
