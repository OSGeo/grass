 ...
 * SPDX-FileCopyrightText: 2007 by the GRASS Development Team
 *
...
/**
 * \fn int load_catval_array_rgb (struct Map_info* map, int vec, dbCatValArray*
 * cvarr_rgb)
 *
 ...
 **** needed is to pass the column name from the vector struct to the fn as
 *well.
 */

/**
 * \fn int load_catval_array_size (struct Map_info* map, int vec, dbCatValArray*
 * cvarr_size)
 *
...
/**
 * \fn int load_catval_array_rot (struct Map_info* map, int vec, dbCatValArray*
 * cvarr_rot)
 *
 * \brief Loads categories and dynamic rotation values into dbCatValArray
 * structure
 *
 * \param map: pointer to a vector Map_info structure
 * \param level: layer number identifier
