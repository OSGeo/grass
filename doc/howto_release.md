...
# Should give the same as last commits visible on GitHub (<https://github.com/OSGeo/grass/commits/releasebranch_8_2>):
git log --max-count=5
```
...
Check on GitHub (should be the latest commit at <https://github.com/OSGeo/grass/commits/releasebranch_8_2>)
...
Some time is needed to run the checks, so before getting back to creating the tag,
...
```bash
# TODO: how to enforce the right release branch? Needs to be fixed to avoid catch-22 below with release notes.
...
TODO: does this still require a backport of the related GH workflow for G82?
