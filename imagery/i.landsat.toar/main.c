...
int main(int argc, char *argv[])
{
    ...
    G_asprintf(&desc,
        ...
        _("Date"),
        _("Sun Elevation"),
        _("Sensor"), _("Bands count"), _("Sun Azimuth Angle"), _("Time"));
    ...
    if (G_verbose() > G_verbose_std()) {
        ...
        for (i = 0; i < lsat.bands; i++) {
            ...
            fprintf(stderr, " BAND %d %s(code %d)\n", lsat.band[i].number,
                (lsat.band[i].thermal ? "thermal " : ""), lsat.band[i].code);
            fprintf(stderr,
    ...
    *****************************************/
    ...
    for (i = 0; i < lsat.bands; i++) {
        ...
        G_important_message(_("Writing %s of <%s> to <%s>..."),
            (frad->answer ? _("radiance") : (lsat.band[i].thermal) ?
                _("temperature") : _("reflectance")), band_in, band_out);
        for (row = 0; row < nrows; row++) {
        ...
        Rast_append_format_history(&history,
            ...
            (frad->answer ? "Radiance" : (lsat.band[i].thermal ?
                    "Temperature" :
                    "Reflectance")), lsat.number, lsat.sensor, metho->answer);
        ...
        Rast_append_format_history(&history,
            " Production date ....................... %s\n", lsat.creation);
        Rast_append_format_history(&history,
            " Earth-sun distance (d) ................ %.7lf", lsat.dist_es);
        Rast_append_format_history(&history,
