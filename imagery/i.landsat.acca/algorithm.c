    ...
    int single_pass, int with_shadow, int cloud_signature)
{
    ...
    /* FIRST FILTER ... */
    acca_first(out, band, with_shadow, count, hist_cold, hist_warm, signa);
...
void acca_first(Gfile * out, Gfile band[],
    int with_shadow, int count[], int cold[], int warm[], double stats[])
{
    ...
    for (row = 0; row < nrows; row++) {
        ...
        for (col = 0; col < ncols; col++) {
            ...
            for (i = BAND2; i <= BAND6; i++) {
                if (Rast_is_d_null_value((void *)((DCELL *) band[i].rast +
                            col))) {
                    code = NO_CLOUD;
                    break;
                }
            ...
            if (Rast_is_c_null_value((void *)((CELL *) out->rast + col))) {
                ...
                if (code == COLD_CLOUD || code == WARM_CLOUD) {
                    ((CELL *) tmp.rast)[col] = (code == WARM_CLOUD &&
                        review_warm == 0) ? IS_WARM_CLOUD : IS_COLD_CLOUD;
                }
                else
