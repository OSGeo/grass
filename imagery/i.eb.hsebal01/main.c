...
int main(int argc, char *argv[])
{
    ...
    if (flag2->answer) {
        ...
        for (row = 0; row < nrows; row++) {
            ...
            for (col = 0; col < ncols; col++) {
                ...
                if (Rast_is_d_null_value(&d_Rn) || Rast_is_d_null_value(&d_g0)) {
                    d_Roh[row][col] = -999.9;
                    d_Rah[row][col] = -999.9;
                    /* do nothing */
                }
                    ...
                    if (d_t0dem[row][col] <= 250.0 || d_z0m[row][col] < 0.01) {
                        ...
                        d_rah1 =
                            (1 / (d_u5 * pow(0.41,
                                    2))) * log(5 / d_z0m[row][col]) * log(5 /
                            (d_z0m[row][col] * 0.1));
                        d_roh1 =
                            ((998 -
                                d_eact[row][col]) / (d_t0dem[row][col] *
                                2.87)) +
        ...
        for (row = 0; row < nrows; row++) {
            ...
            for (col = 0; col < ncols; col++) {
                ...
                if (Rast_is_d_null_value(&d_t0dem[row][col]) ||
                    ...
                    d_roh1 =
                        ((998 -
                            d_eact[row][col]) / (d_t0dem[row][col] * 2.87)) +
                        (d_eact[row][col] / (d_t0dem[row][col] * 4.61));
                    if (d_roh1 > 5)
                        d_roh1 = 1.0;
                    else
    ...
    if (isnan(a) || isnan(b)) {
        ...
        Rast_close(outfd);
        G_fatal_error
            ("Delta T Convergence failed, exiting prematurily, please check output");
    }
            ...
            if (Rast_is_d_null_value(&d_t0dem[row][col]) ||
                ...
                if (d_h1 < -50 || d_h1 > 1000) {
                    ...
                    d_L =
                        ...
                        3) * d_t0dem[row][col] / (d_h1 * 9.81 * 0.41);
                    d_x = pow((1 - 16 * (5 / d_L)), 0.25);
                    d_psim =
                        2 * log((1 + d_x) / 2) + log((1 + pow(d_x,
                                2)) / 2) - 2 * atan(d_x) + 0.5 * M_PI;
                    d_psih = 2 * log((1 + pow(d_x, 2)) / 2);
                    d_u5 = (ustar / 0.41) * log(5 / d_z0m[row][col]);
                    d_rah2 =
                        (1 / (d_u5 * pow(0.41,
                                2))) * log((5 / d_z0m[row][col]) - d_psim)
                        * log((5 / (d_z0m[row][col] * 0.1)) - d_psih);
                    if (row == rowDry && col == colDry) {       /*collect dry pix info */
                        d_h1 =
                            (1004 * d_roh1) * (a * d_t0dem[row][col] +
                            b) / d_rah_dry;
                        d_L =
                            -1004 * d_roh1 * pow(ustar,
                            3) * d_t0dem[row][col] / (d_h1 * 9.81 * 0.41);
        ...
        for (col = 0; col < ncols; col++) {
            ...
            if (Rast_is_d_null_value(&d_t0dem[row][col]) ||
                ...
                if (d_rah3 < 1.0) {
                    ...
                    d_h3 =
                        (1004 * d_Roh[row][col]) * (a * d_t0dem[row][col] +
                        b) / d_rah3;
                }
                if (d_h3 < 0 && d_h3 > -50) {
                    d_h3 = 0.0;
