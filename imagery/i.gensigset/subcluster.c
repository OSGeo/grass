...
subcluster(struct SigSet *S, int Class_Index, int *Max_num, int maxsubclasses)
{
    ...
    *Max_num = (ndata_points + 1) / nparams_clust - 1;
    ...
    if (maxsubclasses < 1) {
        G_warning(_("Not enough pixels in class %d"), Class_Index + 1);
        Sig->nsubclasses = 0;
        Sig->used = 0;
        return;
    }
    ...
    G_debug(1, "Subclasses = %d Rissanen = %f", Sig->nsubclasses, min_riss);
...
static void add_SubSigs(
    ...
    struct SubSig *SubSig2, struct SubSig *SubSig3, int nbands)
{
    int b1, b2;
    double wt1, wt2;
    double tmp;
