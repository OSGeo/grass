...
int main(int argc, char *argv[])
{
    ...
    if (!strcasecmp(viflag, "pvi") &&
        ...
        G_fatal_error(_("pvi index requires red and nir maps and soil line slope"));
    ...
    if (!strcasecmp(viflag, "vari") &&
        (!(opt.red->answer) || !(opt.green->answer)
         || !(opt.blue->answer)))
    ...
    for (row = 0; row < nrows; row++) {
        ...
        for (col = 0; col < ncols; col++) {
            ...
            if (Rast_is_f_null_value(&d_redchan) ||
                ...
                if (!strcasecmp(viflag, "msavi"))
                    outrast[col] =
                        msa_vi(d_redchan, d_nirchan, soil_slope, soil_inter,
                               soil_noise);
