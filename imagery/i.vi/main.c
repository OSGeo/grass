...
int main(int argc, char *argv[])
{
    ...
    module->label = _("Calculates different types of vegetation indices.");
    ...
    opt.viname->options =
        "arvi,ci,dvi,evi,evi2,gvi,gari,gemi,ipvi,msavi,msavi2,ndvi,ndwi,pvi,savi,sr,vari,wdvi";
    ...
    opt.sl_int->description =
        _("Value of the intercept of the soil line (MSAVI only)");
    ...
    opt.sl_red->description =
        _("Value of the factor of reduction of soil noise (MSAVI only)");
    ...
    opt.bits->description =
        _("If data is in Digital Numbers (i.e. integer type), give the max bits (i.e. 8 for Landsat -> [0-255])");
    ...
    if (!strcasecmp(viflag, "evi2") && (!(opt.red->answer) ||
            !(opt.nir->answer)))
        G_fatal_error(_("evi2 index requires red and nir maps"));

    if (!strcasecmp(viflag, "vari") && (!(opt.red->answer) ||
            !(opt.green->answer)
            || !(opt.blue->answer)))
        G_fatal_error(_("vari index requires blue, green and red maps"));

    if (!strcasecmp(viflag, "gari") && (!(opt.red->answer) ||
            !(opt.nir->answer)
        ...
        for (col = 0; col < ncols; col++) {
            ...
            if (Rast_is_f_null_value(&d_redchan) ||
                ...
                if (!strcasecmp(viflag, "msavi"))
                    outrast[col] =
                        msa_vi(d_redchan, d_nirchan, msavip1, msavip2,
                        msavip3);
                ...
                if (!strcasecmp(viflag, "gvi"))
                    outrast[col] =
                        g_vi(d_bluechan, d_greenchan, d_redchan, d_nirchan,
                        d_chan5chan, d_chan7chan);

                if (!strcasecmp(viflag, "gari"))
                    outrast[col] =
                        ga_ri(d_redchan, d_nirchan, d_bluechan, d_greenchan);
