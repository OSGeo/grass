...
from grass.script import run_command, find_file, read_command
...
class TestIClusterWithSyntheticData(TestCase):
    ...
    def tearDownClass(cls):
        ...
        cls.runModule("g.remove", flags='f', type='raster', name=','.join(cls.input_maps))

    def setUp(self):
        """Ensure clean signature file state before each test"""
        if self._signature_exists():
            self.runModule("g.remove", flags='f', type='raster', name=self.signature_file)
    ...
    def test_convergence_parameters(self):
        ...
        with open(low_report, 'r') as f:
            low_content = f.read()
        with open(high_report, 'r') as f:
            high_content = f.read()
        low_iterations = len(re.findall(r'######## iteration \d+ ###########', low_content))
        high_iterations = len(re.findall(r'######## iteration \d+ ###########', high_content))
        self.assertGreater(
            high_iterations, 
            low_iterations,
            f"Higher convergence should require more iterations. Found {high_iterations} for high vs {low_iterations} for low."
        )
        low_final_conv = float(re.search(r'(\d+) classes \(convergence=(\d+\.\d+)%\)', low_content).group(2))
        high_final_conv = float(re.search(r'(\d+) classes \(convergence=(\d+\.\d+)%\)', high_content).group(2))    
        self.assertGreater(
            high_final_conv,
            low_final_conv,
