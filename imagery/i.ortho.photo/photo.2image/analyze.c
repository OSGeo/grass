...
/* lets do it */
	...
	while(1)
	{
		...
		break;
	    line++;
	    if(group.ref_equation_stat > 0 && group.photo_points.status[curp] > 0)
	    {
		color = BLACK;
	...
	report.bottom = cury;
	downarrow (&more, curp < group.photo_points.count ? BLACK : BACKGROUND);
...
static int compute_transformation(void)
{
    ...
    Compute_equation();
    if (group.ref_equation_stat <= 0) return 1;
 ...
 */
    ...
    {
	if (group.photo_points.status[n] <= 0) continue;
	count++;
	I_georef (group.photo_points.e2[n], group.photo_points.n2[n], &e1, &n1, group.E21, group.N21);
	I_georef (group.photo_points.e1[n], group.photo_points.n1[n], &e2, &n2, group.E12, group.N12);
...
static int do_report (FILE *fd)
{
	...
	FMT2(buf,
	    ...
	    group.photo_points.e2[n],
	    group.photo_points.n2[n]);
	fprintf (fd, "   %s\n", buf);
    }
    fprintf (fd, "\n");
    if (group.ref_equation_stat < 0)
...
static int show_point (int n,int true_color)
{
	...
	R_standard_color (RED);
    display_one_point (VIEW_MAP1, group.photo_points.e1[n], group.photo_points.n1[n]);
