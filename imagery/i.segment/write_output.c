...
int write_bands_ms(struct globals *globals)
{
    ...
    for (n = 0; n < globals->nbands; n++) {
        ...
        G_asprintf(&name[n], "%s%s", globals->Ref.file[n].name,
                   globals->bsuf);
    ...
    for (n = 0; n < globals->nbands; n++) {
        ...
        Rast_read_colors(globals->Ref.file[n].name,
                         globals->Ref.file[n].mapset, &colors);
