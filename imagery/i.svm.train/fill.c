...
void fill_problem(const char *name_labels, const char *mapset_labels, struct Ref band_refs, const char *mapset_group, struct svm_problem *problem) {
    ...
    for (row = 0; row < nrows; row++) {
        ...
        for (col = 0; col < ncols; col++) {
            ...
            if (label_num >= label_max) {
                ...
                problem->x = G_realloc(problem->x, (size_t)label_max * sizeof(struct svm_node *));
            }
            problem->l = label_num;
