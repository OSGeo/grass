 ...
 *               element (cell, dig, etc) in the search path for the subproject 
 *               in project
...
/************************************************************************
 * usage: i.find project subproject element file [element2 file2]...
 *
 * produces a file containing the names of files of type
 * element (cell, dig, etc) in the search path for the subproject in project
 *
 ...
 *
 * at present this routine requires that both the current project/subproject
 * and the specified project/subproject be valid for the user.
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 5 || argc % 2 == 0)
	G_fatal_error(_("usage: %s project subproject element file."), argv[0]);

    G_gisinit(argv[0]);

    /*
     * this code assumes that the SEARCH PATH is not read
     * until we call G__subproject_name() in find()
     */
    tempfile = G_tempfile();
...
static int find(FILE * fd, char *element)
{
    ...
    for (n = 0; ((subproject = G_get_subproject_name(n)) != NULL); n++) {
	int len;
	char dir[1024];
	struct dirent *dp;
	DIR *dfd;

	G_file_name(dir, element, "", subproject);
	if ((dfd = opendir(dir)) == NULL)
	    continue;

	len = strlen(subproject);
	if (len > len2)
	    len2 = len;

	while ((dp = readdir(dfd)) != NULL) {
	    if (dp->d_name[0] != '.') {
		fprintf(fd, "%s %s\n", dp->d_name, subproject);
