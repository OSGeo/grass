...
def test_get_backend_no_libraries():
    ...
    with patch.dict("sys.modules", {"folium": None, "ipyleaflet": None}), pytest.raises(ValueError, match="No supported mapping backend found"):
...
def test_get_backend_unknown_object():
    ...
    with pytest.raises(ValueError, match="No supported mapping backend found"):
