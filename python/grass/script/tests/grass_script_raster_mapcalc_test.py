...
class TestMapcalcRandFunction:
    """Tests for rand() function in mapcalc with default auto-seeding"""
    ...
    def test_mapcalc_rand_default_auto_seed(self):
        """Test that rand() works with default auto-seeding (flags='s')"""
        gs.mapcalc(
    ...
    def test_mapcalc_rand_disable_auto_seed(self):
        """Test that rand() can disable auto-seeding with empty flags"""
        from grass.script.core import CalledModuleError

        # Calling with flags='' should fail with unseeded PRNG error
...
class TestMapcalcStartRandFunction:
    """Tests for rand() function in mapcalc_start with default auto-seeding"""
    ...
    def test_mapcalc_start_rand_default_auto_seed(self):
        """Test that mapcalc_start with rand() works with default auto-seeding"""
        p = gs.mapcalc_start(
    ...
    def test_mapcalc_start_rand_disable_auto_seed(self):
        """Test that mapcalc_start with rand() can disable auto-seeding with empty flags"""
        p = gs.mapcalc_start(
        ...
        returncode = p.wait()
        # Should fail because PRNG is not seeded
        assert returncode != 0
