...
struct proj_unit *get_proj_unit(const char *arg)
{
    ...
    snprintf(buf, sizeof(buf), "%s/etc/proj/units.table", G_share_dir());
...
struct proj_desc *get_proj_desc(const char *arg)
{
    ...
    snprintf(buf, sizeof(buf), "%s/etc/proj/desc.table", G_share_dir());
...
struct proj_parm *get_proj_parms(const char *arg)
{
    ...
    snprintf(buf, sizeof(buf), "%s/etc/proj/parms.table", G_share_dir());
    ...
    for (done = 0; !done;) {
        ...
        if (strcmp(ask, "ask") == 0)
            ...
            parm->ask = 1;
            G_warning(_("Unrecognized 'ask' value in parms.table: %s"), ask);
        }
