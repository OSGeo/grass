...
int main(int argc, char *argv[])
{
    ...
    if (access(path, 0) == 0) {
        ...
        if (!G_yes
            (_("Would you still like to change some of the parameters?"), 0)) {
    ...
    sph_check = 0;
    if (G_yes
        (_("Do you wish to specify a geodetic datum for this location?"), 1)) {
        ...
        if (sph_check > 0) {
            if (sph_check == 2) {       /* ask radius */
                if (exist) {
                    ...
                    radius =
                        prompt_num_double(_("Enter radius for the sphere in meters"), RADIUS_DEF,
                        1);
            }                   /* end ask radius */
        }
    }
