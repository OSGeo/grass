...
def merge_md_files(md1, md2, content_modifier1, content_modifier2, extra_yaml=None):
    """Merge two markdown files by concatenating their YAML headers and content.

    The optional extra_yaml is appended to the combined YAML header.
    """
...
def source_edit_url(url_source, pgm):
    """Return the URL for editing the tool's Markdown source on GitHub.

    The source URL points to the tool directory in the repository tree,
    so the edit URL is the same path in the edit view plus the file name.
    Returns None when the source location is unknown.
    """
    if not url_source:
        return None
    return f"{url_source.rstrip('/').replace('/tree/', '/edit/', 1)}/{pgm}.md"
...
"""
    ...
    # The edit URL goes into the front matter so that the website templates
    # can link to the source file without any Python hook in the site build.
    edit_url = source_edit_url(url_source, pgm)
    extra_yaml = f"source_edit_url: {edit_url}" if edit_url else None
