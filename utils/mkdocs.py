...
def get_version_branch(major_version, addons_git_repo_url):
    ...
    if source_is_remote():
        branch = subprocess.Popen(  # nosec B607: executable intentionally resolved via PATH
            [
                "git",
                "ls-remote",
                "--heads",
...
def has_src_code_git(src_dir):
    ...
    os.chdir(src_dir)
    try:
        process_result = subprocess.run(  # nosec B607: executable intentionally resolved via PATH
            [
                "git",
                "log",
                "-1",
    ...
    if not grass_addons_dir.exists():
        ...
        try:
            with tempfile.TemporaryDirectory(dir=addons_base_dir) as tmpdir:
                ...
                subprocess.call(  # nosec B607: executable intentionally resolved via PATH
                    [
                        "git",
                        "clone",
                        "-q",
            ...
            if not grass_addons_dir.exists():
                raise
    addons_file_list = subprocess.Popen(  # nosec B607: executable intentionally resolved via PATH
