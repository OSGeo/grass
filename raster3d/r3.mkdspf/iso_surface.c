    ...
    for (y1 = 0; y1 < yloop - 1; y1++) { /* dpg */
        ...
        for (x1 = 0; x1 < xloop - 1; x1++) { /* dpg */
            ...
            for (t_ndx = 0; t_ndx < (linefax->nthres); t_ndx++) {
                ...
                /* currently masks in grid3 files are in xy-plane, so check for
                   cube with one or more (vertical) edges masked out & don't
                   draw polys. This gets rid of walls around masked surfaces,
                   but leaves edges jagged */
#ifdef OLDSTUFF
                if (!DATA[0] && !DATA[4])
                    c_ndx = 0;
                    ...
                    c_ndx = 0;
#endif
                /* CHANGED: use NULLS instead of zeros - if any are null,
                   polygons are undefined - or else need to define polygons
                   which exclude nulls under various combinations e.g., give
                   each null a tetrahedron of influence */
                ...
                if (c_ndx > 0 && c_ndx < 255) { /* -dpg */
                    ...
                    fill_cfax(&CUBE, linefax->litmodel, c_ndx, TEMP_VERT,
                              TEMP_NORM);
                }
            }
            if (!a)
...
static void xings_fnorm(int c_ndx, int t_ndx)
{
    cmndln_info *linefax;
    register int
        i; /* loop count variable incremented to to examine each edge */
    ...
    for (i = 0; i < cell_table[c_ndx].nedges; i++) {
        ...
        case 1:
            TEMP_VERT[1][0] = LINTERP(DATA[0], DATA[1], linefax->tvalue[t_ndx]);
            TEMP_VERT[1][1] = 255;
            TEMP_VERT[1][2] = 0;
            break;
        ...
        case 2:
            TEMP_VERT[2][0] = 255;
            TEMP_VERT[2][1] = LINTERP(DATA[2], DATA[1], linefax->tvalue[t_ndx]);
            TEMP_VERT[2][2] = 0;
            break;

        case 3:
            TEMP_VERT[3][0] = LINTERP(DATA[3], DATA[2], linefax->tvalue[t_ndx]);
            TEMP_VERT[3][1] = 0;
            TEMP_VERT[3][2] = 0;
            break;
        ...
        case 4:
            TEMP_VERT[4][0] = 0;
            TEMP_VERT[4][1] = LINTERP(DATA[3], DATA[0], linefax->tvalue[t_ndx]);
            TEMP_VERT[4][2] = 0;
            break;

        case 5:
            TEMP_VERT[5][0] = LINTERP(DATA[4], DATA[5], linefax->tvalue[t_ndx]);
            TEMP_VERT[5][1] = 255;
            TEMP_VERT[5][2] = 255;
            break;
        ...
        case 6:
            TEMP_VERT[6][0] = 255;
            TEMP_VERT[6][1] = LINTERP(DATA[6], DATA[5], linefax->tvalue[t_ndx]);
            TEMP_VERT[6][2] = 255;
            break;

        case 7:
            TEMP_VERT[7][0] = LINTERP(DATA[7], DATA[6], linefax->tvalue[t_ndx]);
            TEMP_VERT[7][1] = 0;
            TEMP_VERT[7][2] = 255;
            break;
        ...
        case 8:
            TEMP_VERT[8][0] = 0;
            TEMP_VERT[8][1] = LINTERP(DATA[7], DATA[4], linefax->tvalue[t_ndx]);
            TEMP_VERT[8][2] = 255;
            break;
        ...
        case 9:
            TEMP_VERT[9][0] = 0;
            TEMP_VERT[9][1] = 255;
            TEMP_VERT[9][2] = LINTERP(DATA[0], DATA[4], linefax->tvalue[t_ndx]);
            break;
 ...
 **  gradients are stored in temporary variables that are also written to
 *display**  file.
 */
static void xings_grad(float *data[], int c_ndx, int x1, int y1, int z1,
                       int t_ndx)
{
     ...
     ** direction.  gradients are going to be used to calculate relative
     *positions of
     ** vertices to the light source. though not specifically necessary, the
     *offset
     ...
     **  is located in the cell_table[c_ndx].edges[#].  for each edge listed,
     *the
     **  polygon vertices are calculated and stored in temporary array
     *temp_vert[][],
     **  gradients for x,y,z components of these vertices are calculated next as
     **  a,b,c the length l of this gradient vector is computed and the
     *normalizeD
    ...
    for (i = 0; i < nedges; i++) {
        ...
        TEMP_NORM[crnt_edge][0] =
            delta * (Data_Grad[a][0] - Data_Grad[b][0]) + Data_Grad[b][0];
        TEMP_NORM[crnt_edge][1] =
            delta * (Data_Grad[a][1] - Data_Grad[b][1]) + Data_Grad[b][1];
        TEMP_NORM[crnt_edge][2] =
            delta * (Data_Grad[a][2] - Data_Grad[b][2]) + Data_Grad[b][2];
