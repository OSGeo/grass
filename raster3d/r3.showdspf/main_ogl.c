...
void do_draw_multiple_thresholds (file_info *Headp, file_info *G3p, struct dspec *D_spec, 
	...
	switch (cmd)
	{
		...
		*p = 0;	/* throw away rest of line */
		...
		tmp2 = D_spec.Thresh;
                ...
                do_draw_multiple_thresholds(&Headfax, &G3header, &D_spec, &D_Cap, DRAW_ISO|dobox);
                new_swapbuffers(); 
                D_spec.Swap_buf = 1;
                D_spec.c_flag = tmp;
		D_spec.Thresh = tmp2;
...
void draw_multiple()
{
   ...
   temp = D_spec.Thresh;
   D_spec.Swap_buf = 0;
   do_draw_multiple_thresholds(&Headfax, &G3header, &D_spec, &D_Cap, DRAW_ISO|DRAW_BBOX);
   new_swapbuffers(); 
   D_spec.Swap_buf = 1;
   D_spec.Thresh = temp;
...
*/
void
do_draw_multiple_thresholds (file_info *Headp, file_info *G3p, struct dspec *D_spec, 
                  struct Cap *Cap, unsigned int type)
{
    static int first = 1;
    static double x, y, z;
...
void do__bbox (struct dspec *D_spec)
{
  ...
  first = 0;
    ...
    glColor3ub(255,0,0);
    glBegin(GL_LINE_LOOP);glVertex3fv(c[0]);glVertex3fv(c[1]);glVertex3fv(c[2]);glVertex3fv(c[3]);glEnd();
    glBegin(GL_LINES);
    ...
    glBegin(GL_LINE_LOOP);glVertex3fv(c[4]);glVertex3fv(c[5]);glVertex3fv(c[6]);glVertex3fv(c[7]);glEnd();
...
/*	    sprintf(label,"%d",gy);new_charstr(label);*/
	    glBegin(GL_LINE_STRIP);glVertex3fv(gyl[gy]);glVertex3fv(gyh[gy]);glEnd();
	    ...
	    */
	    glBegin(GL_LINE_STRIP);glVertex3fv(gxl[gx]);glVertex3fv(gxh[gx]);glEnd();
...
void
do__draw (file_info *Headp, struct dspec *D_spec)
{
    static int first = 1;
...
void
do__draw_solid (file_info *Headp, file_info *G3header, struct dspec *D_spec, struct Cap *Cap)
{
    int min, max;
...
void options()
{
    ...
    fprintf(stderr,"L        display list of thresholds entered with \"l\" directive\n");
/*
