...
static void write_result(N_array_3d * status, N_array_3d * phead_start,
    N_array_3d * phead, double *result, RASTER3D_Region * region, char *name);
...
void set_params(void)
{
    ...
    param.phead->description =
        _("Input 3D raster map with initial piezometric heads in [m]");
    ...
    param.q->required = NO;
    param.q->description =
        _("Input 3D raster map with sources and sinks in [m^3/s]");
