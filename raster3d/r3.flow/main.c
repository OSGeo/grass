    ...
    int write_scalar, int use_sampled_map)
{
    ...
    *f_info = fi;
    ...
    drvr = db_start_driver_open_database(fi->driver,
        Vect_subst_var(fi->database, flowline_vec));
    if (drvr == NULL) {
...
static void load_input_raster3d_maps(struct Option *scalar_opt,
    ...
    struct Gradient_info *gradient_info, RASTER3D_Region * region)
{
    int i;
...
int main(int argc, char *argv[])
{
    ...
    seed_opt->description = _("If no map is provided, "
        "flow lines are generated " "from each cell of the input 3D raster");
    ...
    integration.cell_size = sqrt(region.ns_res * region.ns_res +
        region.ew_res * region.ew_res + region.tb_res * region.tb_res);
        ...
        while (TRUE) {
            ...
            if (integration.direction_type == FLOWDIR_UP ||
                ...
                compute_flowline(&region, &seed, &gradient_info, flowacc,
                    sampled, &integration, &fl_map, fl_cats, fl_points, &cat,
