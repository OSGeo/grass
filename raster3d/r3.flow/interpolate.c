...
static void trilinear_interpolation(const double *array_values,
    const double x, const double y, const double z, double *interpolated)
{
    int i, j;
...
static void get_relative_coords_for_interp(RASTER3D_Region * region,
    const double north,
    const double east, const double top, double *x, double *y, double *z)
{
    int col, row, depth;
    double temp;
...
int interpolate_velocity(RASTER3D_Region * region, RASTER3D_Map ** map,
    const double north, const double east,
    const double top, double *vel_x, double *vel_y, double *vel_z)
{
    int i, j;
    ...
    double *vel_z)
{
    ...
    get_relative_coords_for_interp(region, north, east, top, &rel_x, &rel_y,
        &rel_z);
    trilinear_interpolation(gradient_info->neighbors_values, rel_x, rel_y,
        rel_z, interpolated);
