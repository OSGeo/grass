...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < 3; i++) {
        output[i] =
            ...
            RASTER3D_USE_CACHE_DEFAULT, &region, DCELL_TYPE, 32);
        if (!output[i]) {
            Rast3d_fatal_error(_("Unable to open 3D raster map <%s>"),
        ...
        while (index_y < region.rows) {
            ...
            while (index_x < region.cols) {
                ...
                Rast3d_get_block(input, start_x, start_y, start_z,
                    n_x, n_y, n_z, blocks[j].input.array, DCELL_TYPE);
                if ((j + 1) == N || i == max_i - 1) {
                    ...
                    for (k = 0; k <= j; k++) {
                        Rast3d_gradient_double(&(blocks[k].input), step,
                            &(blocks[k].dx), &(blocks[k].dy), &(blocks[k].dz));
                    }
