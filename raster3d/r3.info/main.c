...
int main(int argc, char *argv[])
{
    ...
    if (!rflag->answer && !gflag->answer && !hflag->answer) {
        ...
        if (G_asprintf(&line, "Mapset:   %-29.29s  Login of Creator: %s",
        ...
        if (hist_ok) {
            ...
            if (G_asprintf(&line, "   %s", Rast_get_history(&hist,
                        HIST_DATSRC_1)) > 0)
                printline(line);
            else
                G_fatal_error(_("Cannot allocate memory for string"));
            ...
            /**************************************/
                {
                    if (G_asprintf(&line, "   %s", Rast_history_line(&hist,
                                i)) > 0)
                        printline(line);
                    else
                        G_fatal_error(_("Cannot allocate memory for string"));
