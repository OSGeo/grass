...
void G__wps_print_process_description(void)
{
        ...
        while (opt != NULL) {
            ...
            if (is_input == 1) {
                ...
                if (data_type == TYPE_RASTER || data_type == TYPE_VECTOR ||
                    ...
                    wps_print_literal_input_output(WPS_INPUT, min, max,
                        identifier, title, abstract, type, 0, keywords,
                        num_keywords, value, TYPE_OTHER);
                }
            }
            opt = opt->next_opt;
        }
    ...
    if (st->n_flags) {
        ...
        while (flag != NULL) {
            ...
            wps_print_literal_input_output(WPS_INPUT, 0, 1, ident, title, NULL,
                "boolean", 0, val, 2, "false", TYPE_OTHER);
            flag = flag->next_flag;
        }
    }

    /* We have two default options, which define the resolution of the created mapset */
    if (num_raster_inputs > 0 || num_raster_outputs > 0 || num_strds_inputs > 0
        || num_strds_outputs > 0) {
        wps_print_literal_input_output(WPS_INPUT, 0, 1, "grass_resolution_ns",
        ...
        wps_print_literal_input_output(WPS_INPUT, 0, 1, "grass_resolution_ew",
    ...
    if (num_raster_inputs > 0)
        wps_print_literal_input_output(WPS_INPUT, 0, 1, "grass_band_number",
    ...
    if (st->n_opts) {
        ...
        if (found_output == 0)
            wps_print_complex_output("stdout", "Module output on stdout",
...
static void wps_print_process_description_begin(int store, int status,
    const char *identifier, const char *title, const char *abstract,
...
static void wps_print_complex_input(int min, int max, const char *identifier,
    const char *title, const char *abstract, int megs, int type)
{
...
static void wps_print_comlpex_input_output(int inout_type, int min, int max,
    const char *identifier, const char *title, const char *abstract, int megs,
    int type)
{
...
static void wps_print_literal_input_output(int inout_type, int min, int max,
    const char *identifier, const char *title, const char *abstract,
    const char *datatype, int unitofmesure, const char **choices,
    int num_choices, const char *default_value, int type)
{
    int i;
    char range[2][24];
    char *str;
...
static void wps_print_bounding_box_data(void)
{
    ...
    wps_print_ident_title_abstract("BoundingBox",
