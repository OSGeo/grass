...
void G__wps_print_process_description(void)
{
    ...
    if (st->n_flags) {
        ...
        while (flag != NULL) {
            ...
            wps_print_literal_input_output(WPS_INPUT, 0, 1, ident, title,
                                           NULL, "boolean", 0, val, 2,
                                           "false", TYPE_OTHER);
            flag = flag->next_flag;
        }
    }
...
static void wps_print_complex_output(const char *identifier,
                                     const char *title, const char *abstract,
                                     int type)
{
