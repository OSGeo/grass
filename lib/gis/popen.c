...
/*!
   ...
   The child is reaped and the state is cleared, so calling this a second
   time on the same struct reports failure rather than waiting again.

   \param state pipe state filled in by G_popen_read() or G_popen_write()

   \return exit status of the child, as returned by G_wait()
   \return -1 if there was no child to wait for
 */
