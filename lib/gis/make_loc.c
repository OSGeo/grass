...
int G_make_location(const char *location_name,
    ...
    const struct Key_Value *proj_info, const struct Key_Value *proj_units)
{
    char path[GPATH_MAX];
...
int G_make_location_epsg(const char *location_name,
    ...
    const struct Key_Value *proj_units, const struct Key_Value *proj_epsg)
{
    int ret;
...
int G_compare_projections(const struct Key_Value *proj_info1,
    ...
    const struct Key_Value *proj_info2, const struct Key_Value *proj_units2)
{
...
int G_write_projwkt(const char *location_name, const char *wktstring)
{
    ...
    if (!fp)
        G_fatal_error(_("Unable to open output file <%s>: %s"), path,
            strerror(errno));
