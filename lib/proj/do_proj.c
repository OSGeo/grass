    ...
    double *ymin, double *ymax)
{
    ...
    *ymax = window.north;
    ...
    if (window.proj != PROJECTION_LL) {
        ...
        G_asprintf(&tproj.def, "+proj=pipeline +step +inv %s", indef);
...
int GPJ_init_transform(const struct pj_info *info_in,
    const struct pj_info *info_out, struct pj_info *info_trans)
{
    if (info_in->pj == NULL)
    ...
    /* input and output CRS are available */
        ...
        proj_operation_factory_context_set_area_of_interest(PJ_DEFAULT_CTX,
            operation_ctx, xmin, ymin, xmax, ymax);
        proj_operation_factory_context_set_spatial_criterion(PJ_DEFAULT_CTX,
            operation_ctx, PROJ_SPATIAL_CRITERION_STRICT_CONTAINMENT);
     ...
     * assume info_out to be ll equivalent of info_in */
        ...
        if (insrid && outsrid) {
            ...
            G_asprintf(&(info_trans->def),
                "+proj=pipeline +step +inv +init=%s +step +init=%s", indef,
                outdef);
...
int GPJ_transform(const struct pj_info *info_in,
    ...
    const struct pj_info *info_trans, int dir, double *x, double *y, double *z)
{
    int ok = 0;
