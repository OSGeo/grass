...
static char *grass_to_wkt(const struct Key_Value *proj_info,
    ...
    const struct Key_Value *proj_epsg, int esri_style, int prettify)
{
#ifdef HAVE_OGR
...
char *GPJ_grass_to_wkt(const struct Key_Value *proj_info,
    const struct Key_Value *proj_units, int esri_style, int prettify)
{
...
char *GPJ_grass_to_wkt2(const struct Key_Value *proj_info,
    ...
    const struct Key_Value *proj_epsg, int esri_style, int prettify)
{
    return grass_to_wkt(proj_info, proj_units, proj_epsg, esri_style,
        prettify);
}
...
int GPJ_osr_to_grass(struct Cell_head *cellhd, struct Key_Value **projinfo,
    struct Key_Value **projunits, OGRSpatialReferenceH hSRS1, int datumtrans)
{
        ...
        if (pszSemiMajor != NULL && pszInvFlat != NULL) {
            ...
            while (list != NULL) {
                ...
                if ((a == list->a || fabs(a - list->a) < 0.1 ||
                        fabs(1 - a / list->a) < 0.0000001) && ((es == 0 &&
                            list->es == 0) ||
...
int GPJ_wkt_to_grass(struct Cell_head *cellhd, struct Key_Value **projinfo,
    struct Key_Value **projunits, const char *wkt, int datumtrans)
{
#ifdef HAVE_OGR
    int retval;
