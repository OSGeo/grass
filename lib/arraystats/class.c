...
int AS_class_equiprob(double *data, int count, int *nbreaks, double **clbrks_in)
{
    ...
    if (nbclass < 3) {
        ...
        printf("Equiprobable classbreaks currently limited to 10 classes");
        exit(EXIT_FAILURE);
    }
    ...
    if (j < (*nbreaks)) {
        printf("There are classbreaks outside the range min-max. Number of "
                        ...
                        double **clbrks_in)
{
    ...
    for (i = 1; i <= nbclass; i++) {
        ...
        double f = 0.0, xt1 = 0.0, xt2 = 0.0;
        double den = 0.0, d = 0.0, dmax = 0.0;
        int nd = 0, nmax = 0;
        int nf = 0; /* End number */
        ...
        f = (xn[num[jj + 1]] - xn[num[jj - 1]]) /
            (x[num[jj + 1]] - x[num[jj - 1]]);
        f *= n;
        xt1 = (x[num[jj]] - x[num[jj - 1]]) * f;
