...
int test_blas_level_1_double(void)
{
    ...
    if (a != 1.0 * (rows - 1)) {
        G_message("Error in G_math_d_max_norm: %f != %f",
                  (double)1.0 * (rows - 1), a);
        sum++;
    }
...
int test_blas_level_1_float(void)
{
    ...
    if (a != 1.0 * (rows - 1)) {
        G_message("Error in G_math_f_max_norm: %f != %f",
                  (float)1.0 * (rows - 1), a);
        sum++;
    }

    if (b != 1.0 * (rows - 1)) {
        G_message("Error in G_math_isamax: %f != %f", (float)1.0 * (rows - 1),
                  b);
        sum++;
    }
...
#pragma omp parallel default(shared)
    ...
    if (fabs(a - 1.0 * (rows - 1) * rows) > EPSILON) {
        G_message("Error in G_math_f_ax_by 1: %f != %f",
                  (float)1.0 * (rows - 1) * rows, a);
        sum++;
    }

    if (fabs(b - 5.0 * (rows) * (rows / 10)) > EPSILON) {
        G_message("Error in G_math_f_ax_by 2: %f != %f",
                  (float)5.0 * (rows) * (rows / 10), b);
        sum++;
    }
