...
struct fringe_data *Nviz_new_fringe(nv_data * data,
    int id, unsigned long color, double elev, int nw, int ne, int sw, int se)
{
    int num;
    int *surf;
    ...
    data->fringe =
        ...
        data->num_fringes + 1 * sizeof(struct fringe_data *));
...
struct scalebar_data *Nviz_new_scalebar(nv_data * data,
    int bar_id, float *coords, float size, unsigned int color)
{
