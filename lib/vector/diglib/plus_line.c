...
static int add_line(struct Plus_head *plus, int lineid, int type,
    const struct line_pnts *Points, const struct bound_box *box, off_t offset)
{
 ...
 *
 */
int dig_del_line(struct Plus_head *plus, int line, double x, double y,
    double z)
{
    int i;
    plus_t N1 = 0, N2 = 0;
    ...
    if (i == Node->n_lines) {
        G_fatal_error(_("Attempt to delete not registered line %d from node %d"), line,
            N1);
    }
