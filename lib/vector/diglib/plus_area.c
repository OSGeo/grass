...
int dig_area_del_isle(struct Plus_head *plus, int area, int isle)
{
    ...
    if (i == Area->n_isles) {
        G_fatal_error(_("Attempt to delete not registered isle %d from area %d"), isle,
            area);
    }
...
int dig_del_area(struct Plus_head *plus, int area)
{
    ...
    for (i = 0; i < Area->n_isles; i++) {
        ...
        if (Isle == NULL) {
            G_fatal_error(_("Attempt to delete area %d info from dead isle %d"), area,
                Area->isles[i]);
        }
        else {
            /* Isle->area = area_out; */
...
int dig_del_isle(struct Plus_head *plus, int isle)
{
    ...
    if (Isle->area > 0) {
        if (plus->Area[Isle->area] == NULL) {
            G_fatal_error(_("Attempt to delete isle %d info from dead area %d"), isle,
                Isle->area);
        }
        else {
