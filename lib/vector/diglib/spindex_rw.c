    ...
    struct RTree *t, int off_t_size)
{
    ...
    /* should be foolproof */
    sidx_nodesize =
        (int)(2 * PORT_INT + t->nodecard * (off_t_size +
            NUMSIDES * PORT_DOUBLE));
    sidx_leafsize =
        (int)(2 * PORT_INT + t->leafcard * (off_t_size +
            NUMSIDES * PORT_DOUBLE));
        ...
        if (writeout) {
            ...
            if (G_ftell(fp->file) != nextfreepos)
                G_fatal_error("Unable to write spatial index. "
                    "Wrong node position (%" PRI_OFF_T ") in file (should be %"
                    PRI_OFF_T ").", G_ftell(fp->file), nextfreepos);
                ...
                if (lastpos > 0 &&
                    ...
                    for (j = 0; j < maxcard; j++) {
                        ...
                        dig__fread_port_O(&(s[top].pos[j]), 1,
                            &(Plus->spidx_fp), Plus->spidx_port.off_t_size);
                        if (s[top].sn.level == 0) {
                            s[top].sn.branch[j].child.id = (int)s[top].pos[j];
                        }
                        else {
