...
static struct P_node *read_p_node(struct Plus_head *, int, int,
    const char *, const char *, const char *, struct Format_info_pg *, int);
static struct P_line *read_p_line(struct Plus_head *, int,
    const struct line_data *, int, struct Format_info_cache *);
...
struct P_node *read_p_node(struct Plus_head *plus, int n,
    ...
    const char *angles_data, struct Format_info_pg *pg_info, int geom_only)
{
    int i, cnt;
    char **lines, **angles;
...
int Vect__load_map_nodes_pg(struct Map_info *Map, int geom_only)
{
    ...
    if (!res || PQresultStatus(res) != PGRES_TUPLES_OK ||
        ...
        G_warning(_("Inconsistency in topology: number of "
                "nodes %d (should be %d)"), PQntuples(res), plus->n_nodes);
        if (res)
            PQclear(res);
        return -1;
    }
...
int Vect__load_map_lines_pg(struct Map_info *Map)
{
    ...
    if (!res || PQresultStatus(res) != PGRES_TUPLES_OK ||
        ...
        G_warning(_("Inconsistency in topology: number of "
                "points %d (should be %d)"), PQntuples(res), plus->n_plines);
        if (res)
            PQclear(res);
        return -1;
    }
    ...
    if (!res || PQresultStatus(res) != PGRES_TUPLES_OK ||
        ...
        G_warning(_("Inconsistency in topology: number of "
                "lines %d (should be %d)"), PQntuples(res), plus->n_lines);
        if (res)
            PQclear(res);
        return -1;
    }
