/*!
   ...
   SPDX-FileCopyrightText: 2011-2012 by Martin Landa, and the GRASS Development
   Team
    ...
    if (plus->built >= GV_BUILD_BASE) {
        ...
        if (type == GV_BOUNDARY) {
            ...
            if (ret == 0) {
                ...
                cline = dig_add_line(plus, GV_CENTROID, CPoints, &box, fid);
                G_debug(4, "\tCentroid: x = %f, y = %f, cat = %lu, line = %d",
                        x, y, fid, cline);
                dig_cidx_add_cat(plus, 1, (int)fid, cline, GV_CENTROID);
...
off_t V2_rewrite_line_sfa(struct Map_info *Map, off_t line, int type,
                          ...
                          const struct line_cats *cats)
{
    G_debug(3, "V2_rewrite_line_sfa(): line=%d type=%d", (int)line, type);
