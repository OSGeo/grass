...
static int add_geometry_pg(struct Plus_head *,
    ...
    struct feat_parts *, int, int, int, struct geom_parts *);
...
static int add_geometry_ogr(struct Plus_head *,
    struct Format_info_ogr *, OGRGeometryH, int, int, struct geom_parts *);
...
int add_line(struct Plus_head *plus, struct Format_info_offset *offset,
    int type, struct line_pnts *Points, int FID, struct geom_parts *parts)
{
    int line;
    long offset_value;
    ...
    case SF_POLYGON:
        ...
        if (build >= GV_BUILD_CENTROIDS) {
            ...
            ret =
                ...
                (const struct line_pnts **)&pg_info->cache.lines[fparts->
                    idx[ipart]] + 1, fparts->nlines[ipart] - 1, &x, &y);
            if (ret < -1) {
...
int add_geometry_ogr(struct Plus_head *plus,
    ...
    OGRGeometryH hGeom, int FID, int build, struct geom_parts *parts)
{
    ...
    case wkbPolygon:
        ...
        if (build >= GV_BUILD_CENTROIDS) {
            /* create virtual centroid */
            ret =
                Vect_get_point_in_poly_isl((const struct line_pnts *)ogr_info->
                cache.lines[0],
