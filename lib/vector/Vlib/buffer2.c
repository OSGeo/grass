...
static void extract_contour(struct planar_graph *pg, struct pg_edge *first,
    int side, int winding, int stop_at_line_end, struct line_pnts *nPoints)
{
    int j;
    ...
    while (1) {
        ...
        if (opt_side == 1) {
            if (vert->edges[opt_j]->visited_right) {
                ...
                G_debug(4,
                    "ec: v0=%d, v=%d, eside=%d, edge->v1=%d, edge->v2=%d", v,
                    (edge->v1 == v) ? (edge->v2) : (edge->v1), opt_side,
    ...
    for (i = 0; i < np - 1; i++) {
        ...
        if (da != db) {
            ...
            d = dig_distance2_point_to_line(nx + tx, ny + ty, 0, vx, vy, 0,
                wx, wy, 0, 0, NULL, NULL, NULL, NULL, NULL);
...
static void add_line_to_array(struct line_pnts *Points,
    struct line_pnts ***arrPoints, int *count, int *allocated, int more)
{
    if (*allocated == *count) {
        *allocated += more;
        *arrPoints =
...
static void buffer_lines(struct line_pnts *area_outer,
    struct line_pnts **area_isles, int isles_count, int side, double da,
    double db, double dalpha, int round, int caps, double tol,
    struct line_pnts **oPoints, struct line_pnts ***iPoints, int *inner_count)
{
    ...
    *oPoints = Vect_new_line_struct();
    if (auto_side)
        side =
            ...
            area_outer->n_points - 1) ? LEFT_SIDE : RIGHT_SIDE;
    ...
    for (i = 0; i < isles_count; i++) {
        if (auto_side)
            side =
                ...
                area_isles[i]->n_points - 1) ? RIGHT_SIDE : LEFT_SIDE;
...
void Vect_point_buffer2(double px, double py, double da, double db,
    double dalpha, int round, double tol, struct line_pnts **oPoints)
{
    double tx, ty;
