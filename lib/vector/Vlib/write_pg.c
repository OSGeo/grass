...
static off_t write_line_sf(struct Map_info *, int,
    const struct line_pnts **, int, const struct line_cats *);
    ...
    const struct line_pnts *points, const struct line_cats *cats)
{
    ...
    G_asprintf(&stmt,
        ...
        PRI_OFF_T, schema_name, table_name, geom_data, keycolumn, line);
    G_free(geom_data);
...
off_t V2__write_area_pg(struct Map_info *Map,
    const struct line_pnts **points, int nparts, const struct line_cats *cats)
{
...
int create_table(struct Format_info_pg *pg_info)
{
    ...
    sprintf(stmt, "SELECT AddGeometryColumn('%s', '%s', "
        ...
        pg_info->geom_column, pg_info->srid, geom_type, pg_info->coor_dim);
    ...
    sprintf(stmt,
        ...
        pg_info->schema_name, pg_info->table_name, GV_KEY_COLUMN);
    if (Vect__execute_pg(pg_info->conn, stmt) == -1) {
        Vect__execute_pg(pg_info->conn, "ROLLBACK");
        return -1;
    }
...
int create_topo_schema(struct Format_info_pg *pg_info, int with_z)
{
    ...
    if (!pg_info->topo_geo_only) {
        if (-1 == set_constraint_to_deferrable(pg_info, "node", "face_exists",
            ...
            -1 == set_constraint_to_deferrable(pg_info, "edge_data",
                "end_node_exists", "end_node", "node", "node_id") ||
            -1 == set_constraint_to_deferrable(pg_info, "edge_data",
                "left_face_exists", "left_face", "face", "face_id") ||
            -1 == set_constraint_to_deferrable(pg_info, "edge_data",
                "right_face_exists", "right_face", "face", "face_id") ||
            -1 == set_constraint_to_deferrable(pg_info, "edge_data",
                "start_node_exists", "start_node", "node", "node_id"))
            return -1;
    }
    ...
    if (!pg_info->topo_geo_only) {
        ...
        sprintf(stmt, "CREATE TABLE \"%s\".%s (node_id SERIAL PRIMARY KEY, "
            "lines integer[], angles float[])", pg_info->toposchema_name,
            TOPO_TABLE_NODE);
        if (Vect__execute_pg(pg_info->conn, stmt) == -1) {
            Vect__execute_pg(pg_info->conn, "ROLLBACK");
            return -1;
    ...
    const struct line_pnts **points, int nparts, const struct line_cats *cats)
{
    ...
    if (cats->n_cats > 0) {
        ...
        if (cats->n_cats > 1) {
            G_warning(_("Feature has more categories, using "
                    "category %d (from layer %d)"), cat, field);
        }
    }
    ...
    if (offset_info->array_num >= offset_info->array_alloc) {
        ...
        offset_info->array = (int *)G_realloc(offset_info->array,
            offset_info->array_alloc * sizeof(int));
    }
    ...
    const struct line_pnts *points, const struct line_cats *cats)
{
    ...
    if (cats && cats->n_cats > 0) {
        if (Fi) {
            ...
            if (cats->n_cats > 1) {
                G_warning(_("Feature has more categories, using "
                        "category %d (from layer %d)"), cat, cats->field[0]);
            }
        }
        /* assume layer=1 */
    ...
    if (plus->built >= GV_BUILD_BASE) {
        if (is_node) {
            ...
            line =
                -1 * dig_add_node(plus, points->x[0], points->y[0],
                points->z[0]);
        }
        else {
            off_t offset;
...
unsigned char *point_to_wkb(int byte_order,
    const struct line_pnts *points, int with_z, int *nsize)
{
...
unsigned char *polygon_to_wkb(int byte_order,
    const struct line_pnts **points, int nrings, int with_z, int *nsize)
{
                ...
                for (col = 0; col < ncol; col++) {
                    ...
                    if (!db_test_value_isnull(value)) {
                        ...
                        case DB_C_TYPE_STRING:{
                                ...
                                value_tmp =
                                    G_str_replace(db_get_value_string(value),
                                    "'", "''");
    ...
    const char *geom_data)
{
    ...
    if (plus->built >= GV_BUILD_BASE) {
        if (id > 0) {           /* -> feature */
            ...
            if (Line->type & GV_POINTS) {
                ...
                topo_id =
                    Vect_get_num_primitives(Map,
                    GV_POINTS) + Vect_get_num_nodes(Map);
            }
    ...
    case GV_POINT:{
            ...
            G_asprintf(&stmt, "INSERT INTO \"%s\".node (node_id, geom) VALUES "
                "(%d, '%s'::GEOMETRY)", pg_info->toposchema_name, topo_id,
                geom_data);
#endif
            break;
        }
    ...
    case GV_BOUNDARY:{
            ...
            if (Line) {
                ...
                for (i = 0; i < 2; i++) {
                    ...
                    if (next_edge != 0) {
                        ...
                        G_warning(_("Unable to determine next left/right edge for edge %d"),
                            topo_id);
                    }
                }
            }
            ...
            G_asprintf(&stmt,
                ...
                offset->array[n2 - 1], nle, abs(nle), nre, abs(nre),
                geom_data);
#endif
            break;
        }
    case GV_CENTROID:{
            ...
            G_asprintf(&stmt,
                ...
                "(%d, %d, '%s'::GEOMETRY)", pg_info->toposchema_name, topo_id,
                area, geom_data);
#endif
            break;
        }
...
int set_constraint_to_deferrable(struct Format_info_pg *pg_info,
    ...
    const char *column, const char *ref_table, const char *ref_column)
{
    char stmt[DB_SQL_MAX];
