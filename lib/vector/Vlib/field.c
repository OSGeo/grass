...
int Vect_map_add_dblink(struct Map_info *Map, int number, const char *name,
    const char *table, const char *key, const char *db, const char *driver)
{
    int ret;
...
static int read_dblinks_ogr(struct Map_info *Map)
{
    ...
    Vect_add_dblink(dbl, 1, Map->fInfo.ogr.layer_name,
        Map->fInfo.ogr.layer_name, ogr_fid_col, Map->fInfo.ogr.dsn, "ogr");
#else
    dbDriver *driver;
    dbCursor cursor;
    dbString sql;
    ...
    if (FID) {
        ...
        Vect_add_dblink(dbl, 1, Map->fInfo.ogr.layer_name,
            Map->fInfo.ogr.layer_name, "FID", Map->fInfo.ogr.dsn, "ogr");
    }
    else {
        if (OGC_FID) {
            G_debug(3, "Using ogc_fid column in OGR DB");
            Vect_add_dblink(dbl, 1, Map->fInfo.ogr.layer_name,
                Map->fInfo.ogr.layer_name, "ogc_fid", Map->fInfo.ogr.dsn,
                "ogr");
        }
        else {
            if (OGR_FID) {
                ...
                Vect_add_dblink(dbl, 1, Map->fInfo.ogr.layer_name,
                    Map->fInfo.ogr.layer_name, "ogr_fid", Map->fInfo.ogr.dsn,
                    "ogr");
            }
            else {
                if (GID) {
...
int Vect_write_dblinks(struct Map_info *Map)
{
    ...
    for (i = 0; i < dbl->n_fields; i++) {
        ...
        fprintf(fd, "%s|%s|%s|%s|%s\n", buf, dbl->field[i].table,
            dbl->field[i].key, dbl->field[i].database, dbl->field[i].driver);
