...
Vect_net_ttb_build_graph(struct Map_info *Map,
    ...
    int tucfield,
    const char *afcol,
    const char *abcol, const char *ncol, int geo, int algorithm)
{
    ...
    for (i = 1; i <= nnodes; i++) {
        ...
        if (tucfound > 1)
            G_warning(_("There exists more than one point of node <%d> with unique category field  <%d>.\n"
        ...
        if (ncol != NULL) {
            ...
            if (Vect_cidx_find_next
                (Map, tucfield_idx, isec, GV_POINT, 0, &type,
                    &node_pt_id) == -1) {
                G_warning(_("Unable to find point representing intersection <%d> in unique categories field <%d>.\n"
                ...
                if (node_pt_id == 0) {
                    G_warning(_("Unable to find node for point representing intersection <%d> in unique categories field <%d>.\n"
                            "Cost for the intersection was set to 0.\n"
                            "The unique categories layer is not valid therefore you will probably get incorrect results."),
                        isec, tucfield);
                }
...
Vect_net_build_graph(struct Map_info *Map,
    ...
    int nfield,
    const char *afcol,
    const char *abcol, const char *ncol, int geo, int version)
{
    ...
    if (ncol != NULL)
        dglInitialize(gr, (dglByte_t) version, sizeof(dglInt32_t),
