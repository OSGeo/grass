...
void Vect_break_polygons_file(struct Map_info *Map, int type, struct Map_info *Err)
{
	...
	for (j = 0; j < Points->n_points; j++) {
		...
		if (read(xpntfd, &XPnt, sizeof(XPNT2)) <= 0)
                    G_warning(_("Error or no bytes returned on read()"));
		if (XPnt.cross == 1)
		...
		if (cross) {
		    ...
		    if (write(xpntfd, &XPnt, sizeof(XPNT2)) <= 0)
                        G_warning(_("Error or no bytes written on write()"));
		}
		else {
	...
	for (j = 1; j < Points->n_points; j++) {
	    ...
	    lseek(xpntfd, (off_t) (fpoint - 1) * sizeof(XPNT2), SEEK_SET);
	    if (read(xpntfd, &XPnt, sizeof(XPNT2)) <= 0)
                G_warning(_("Error or no bytes read on read()"));
		...
		if (Err) {
			...
			lseek(xpntfd, (off_t) (fpoint - 1) * sizeof(XPNT2), SEEK_SET);
			if (write(xpntfd, &XPnt, sizeof(XPNT2)) <= 0)
                            G_warning(_("Error or no bytes returned on write()"));
		    }
		}
