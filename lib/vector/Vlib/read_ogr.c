    ...
    struct line_pnts *line_p, struct line_cats *line_c, off_t offset)
{
    ...
    if (line_p != NULL) {
        ...
        if (ogr_info->cache.fid != fid) {
            ...
            ogr_info->feature_cache = OGR_L_GetFeature(ogr_info->layer, fid);
            if (ogr_info->feature_cache == NULL) {
                G_warning(_("Unable to get feature geometry, fid %ld"), fid);
                return -1;
            }
...
int cache_feature(struct Map_info *Map, OGRGeometryH hGeom, int ftype)
{
    ...
    if (line == ogr_info->cache.lines_alloc) {
        ...
        for (i = ogr_info->cache.lines_num; i < ogr_info->cache.lines_alloc;
            i++)
    ...
    struct line_pnts *Points)
{
    ...
    case wkbPoint:
        G_debug(4, "\t->Point");
        if (Points) {
            Vect_append_point(Points, OGR_G_GetX(hGeom, 0), OGR_G_GetY(hGeom,
                    0), OGR_G_GetZ(hGeom, 0));
        }
        return GV_POINT;
        break;
