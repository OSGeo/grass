    ...
    struct line_cats *line_c, int line)
{
    ...
    if (line_p)
        Vect_append_points(line_p, pg_info->cache.lines[cache_idx],
            GV_FORWARD);
...
int read_next_line_pg(struct Map_info *Map,
    struct line_pnts *line_p, struct line_cats *line_c, int ignore_constraints)
{
    int itype;
...
SF_FeatureType get_feature(struct Map_info *Map, int fid, int type)
{
    ...
    if (fid < 1) {              /* sequantial access */
        if (pg_info->cursor_name == NULL &&
            Vect__open_cursor_next_line_pg(pg_info, FALSE,
                Map->plus.built) != 0)
            return -1;
    }
    else {                      /* random access */
    ...
    if (pg_info->toposchema_name) {
        if (fid < 0) {
            ...
            if (type & GV_POINTS) {
                if (type == GV_POINT &&
                    strlen(PQgetvalue(pg_info->res, pg_info->next_line,
                            1)) != 0)
    ...
    pg_info->cache.sf_type = Vect__cache_feature_pg(data,
        FALSE, force_type, &(pg_info->cache), NULL);
    ...
    for (i = 0; i < (*nbytes); i++) {
        wkb_data[i] =
            (unsigned char)((hex_data[2 * i] >
                ...
                '9' ? hex_data[2 * i] - 0x37 : hex_data[2 * i] - 0x30) << 4);
        wkb_data[i] |=
            (unsigned char)(hex_data[2 * i + 1] >
            'F' ? hex_data[2 * i + 1] -
            0x57 : hex_data[2 * i + 1] >
            '9' ? hex_data[2 * i + 1] - 0x37 : hex_data[2 * i + 1] - 0x30);
    }
...
SF_FeatureType Vect__cache_feature_pg(const char *data, int skip_polygon,
    int force_type, struct Format_info_cache *cache, struct feat_parts *fparts)
{
    ...
    if (ftype == SF_POINT) {
        ...
        ret =
            point_from_wkb(wkb_data, nbytes, byte_order, is3D,
        ...
        ret =
            linestring_from_wkb(wkb_data, nbytes, byte_order, is3D,
...
int linestring_from_wkb(const unsigned char *wkb_data, int nbytes,
    int byte_order, int with_z, struct line_pnts *line_p, int is_ring)
{
...
int polygon_from_wkb(const unsigned char *wkb_data, int nbytes,
    int byte_order, int with_z, struct Format_info_cache *cache, int *nrings)
{
...
int geometry_collection_from_wkb(const unsigned char *wkb_data, int nbytes,
    ...
    struct Format_info_cache *cache, struct feat_parts *fparts)
{
    ...
    if (!pg_info->toposchema_name) {
        ...
        if (pg_info->where) {
            ...
            sprintf(stmt,
                ...
                pg_info->fid_column, pg_info->schema_name, pg_info->table_name,
                tokens[0], tokens[1], pg_info->fid_column);
        ...
        sprintf(stmt,
            ...
            pg_info->cursor_name,
            GV_POINT, pg_info->fid_column, pg_info->toposchema_name,
            ...
            pg_info->fid_column, pg_info->toposchema_name,
            pg_info->schema_name, pg_info->table_name,
            pg_info->topogeom_column, pg_info->topogeom_column, GV_BOUNDARY,
            pg_info->fid_column, pg_info->toposchema_name,
            pg_info->schema_name, pg_info->table_name,
            pg_info->topogeom_column, pg_info->topogeom_column);
    }
...
/*!
   ...
   \return -1 on failure
 */
int Vect__open_cursor_line_pg(struct Format_info_pg *pg_info, int fid,
    int type)
{
    char stmt[DB_SQL_MAX];
    ...
    G_asprintf(&(pg_info->cursor_name),
        "%s_%s_%d_%p", pg_info->schema_name, pg_info->table_name, fid,
        pg_info->conn);
    ...
    if (!pg_info->toposchema_name) {
        ...
        if (type & GV_POINTS) {
            ...
            sprintf(stmt,
                ...
                pg_info->fid_column, pg_info->toposchema_name,
                pg_info->schema_name, pg_info->table_name,
                pg_info->topogeom_column, topotype, pg_info->topogeom_column,
                nodeid, fid);
        }
        else {
            sprintf(stmt,
...
void Vect__reallocate_cache(struct Format_info_cache *cache, int num, int incr)
{
    ...
    cache->lines = (struct line_pnts **)G_realloc(cache->lines,
        cache->lines_alloc * sizeof(struct line_pnts *));
