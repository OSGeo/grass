    ...
    struct Map_info *Out)
{
    ...
    for (i = 1; i <= nlines; i++) {
        ...
        if (topo == TOPO_POSTGIS && (type & GV_LINES)) {
            ...
            if (n1 > 0 && (n1 > offset->array_num ||
                    offset->array[n1 - 1] == 0))
                copy_line_nodes(In, n1, with_z, NPoints, Out);
            if (n2 > 0 && (n2 > offset->array_num ||
                    offset->array[n2 - 1] == 0))
    ...
    for (area = 1; area <= nareas; area++) {
        ...
        if (nisles + 1 > nparts_alloc) {
            ...
            Points = (struct line_pnts **)G_realloc(Points,
                (nisles + 1) * sizeof(struct line_pnts *));
...
int Vect_copy_table(const struct Map_info *In, struct Map_info *Out,
    int field_in, int field_out, const char *field_name, int type)
{
...
int Vect_copy_table_by_cat_list(const struct Map_info *In,
    struct Map_info *Out, int field_in, int field_out, const char *field_name,
