    ...
    const struct line_pnts *points, const struct line_cats *cats)
{
    ...
    if (0 != V2__delete_line_from_topo_nat(Map, line, type, old_points,
            old_cats))
        return -1;
...
int V1_restore_line_nat(struct Map_info *Map, off_t offset, off_t line)
{
    ...
    G_debug(3,
        "V1_restore_line_nat(): offset = %" PRI_OFF_T ", line (not used) = %"
...
int V2_restore_line_nat(struct Map_info *Map, off_t offset, off_t line)
{
    ...
    G_debug(3,
        "V2_restore_line_nat(): offset = %" PRI_OFF_T ", line = %" PRI_OFF_T,
        offset, line);
    ...
    const struct line_pnts *points, const struct line_cats *cats)
{
    ...
    if (0 != dig_del_line(plus, line, points->x[0], points->y[0],
            points->z[0]))
        return -1;
...
int V2__add_line_to_topo_nat(struct Map_info *Map, off_t offset, int type,
    const struct line_pnts *points, const struct line_cats *cats,
    int restore_line, int (*external_routine)(const struct Map_info *, int))
{
