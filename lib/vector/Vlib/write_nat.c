			  ...
			  const struct line_pnts *points, const struct line_cats *cats)
{
    ...
    G_debug(3, "V1_rewrite_line_nat(): offset = %" PRId64,
	    offset);
...
int V1_restore_line_nat(struct Map_info *Map, off_t offset, off_t line)
{
    ...
    G_debug(3, "V1_restore_line_nat(): offset = %" PRId64 ", line (not used) = %" PRId64, offset, line);
...
int V2_restore_line_nat(struct Map_info *Map, off_t offset, off_t line)
{
    ...
    G_debug(3, "V2_restore_line_nat(): offset = %" PRId64 ", line = %" PRId64, offset, line);

    if (line < 1 || line > plus->n_lines) {
        G_warning(_("Attempt to access feature with invalid id (%" PRId64 ")"), line);
        return -1;
    }
                             ...
                             int (*external_routine) (const struct Map_info *, int))
{
    ...
    G_debug(3, "V2__add_line_to_topo_nat(): offset = %" PRId64 " (build level = %d)", offset, plus->built);
