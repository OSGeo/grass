 ...
 * branches of a node.
 */
void RTreeNodeCover(struct RTree_Node *n, struct RTree_Rect *r,
    struct RTree *t)
{
    int i, first_time = 1;
 ...
 */
int RTreePickBranch(struct RTree_Rect *r, struct RTree_Node *n,
    struct RTree *t)
{
    struct RTree_Rect *rr;
    int i, first_time = 1;
...
static void RTreeRemoveBranches(struct RTree_Node *n, struct RTree_Branch *b,
    struct RTree_ListBranch **ee, struct RTree_Rect *cover, struct RTree *t)
{
    ...
    for (j = 0; j < t->ndims; j++) {
        center_n[j] =
            (new_cover->boundary[j + t->ndims_alloc] +
            new_cover->boundary[j]) / 2;
    }
...
static void RTreePrintBranch(struct RTree_Branch *b, int depth,
    struct RTree *t)
{
