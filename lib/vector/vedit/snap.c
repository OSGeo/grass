...
int Vedit_snap_point(struct Map_info *Map,
    int line, double *x, double *y, double *z, double thresh, int vertex)
{
    ...
    if (line2snap > 0) {
        ...
        for (i = 0; i < Points->n_points; i++) {
            ...
            dist = Vect_points_distance(*x, *y, *z,
                Points->x[i], Points->y[i], Points->z[i], WITHOUT_Z);
...
int Vedit_snap_lines(struct Map_info *Map, struct Map_info **BgMap,
    int nbgmaps, struct ilist *List, double thresh, int to_vertex)
{
    int i, line, type;
