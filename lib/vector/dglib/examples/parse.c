        ...
        if (fInOpaque == 0 && fInBody == 0) {
            if (regexec(&reVersion, sz, 64, aregm, 0) == 0) {
                ...
                nret = dglInitialize(&graphOut,
                    nVersion, nNodeAttrSize, nEdgeAttrSize, anOpaque);
                if (nret < 0) {
            ...
            if (fInOpaque == 1) {
                ...
                printf("opaque 4: %ld %ld %ld %ld\n",
                    anOpaque[12], anOpaque[13], anOpaque[14], anOpaque[15]);
#endif
                fInOpaque = 0;
            }
        }
            ...
            if (regexec(&reNodeFrom, sz, 64, aregm, 0) == 0) {
                ...
                nret = dglAddEdgeX(&graphOut,
                    nNodeFrom,
                    nNodeTo,
                    nCost, nUser, pbNodeAttr, pbToNodeAttr, pbEdgeAttr, 0);
