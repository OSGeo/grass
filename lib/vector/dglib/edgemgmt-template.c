...
int DGL_ADD_EDGE_FUNC(dglGraph_s * pgraph,
    ...
    dglInt32_t nCost,
    dglInt32_t nEdge,
    void *pvHeadAttr, void *pvTailAttr, void *pvEdgeAttr, dglInt32_t nFlags)
{
    dglInt32_t *pHead;
    dglInt32_t *pTail;
    dglInt32_t *pEdgeset;
    ...
    if ((pinEdgeset = DGL_T_NODEITEM_InEdgesetPTR(pTailNodeItem)) == NULL) {
        ...
        pinEdgeset = DGL_EDGESET_REALLOC(pinEdgeset,
            DGL_EDGESET_EDGECOUNT(pinEdgeset) + 1, pgraph->EdgeAttrSize);
    ...
    if (nFlags & DGL_STRONGCONNECT) {
        return DGL_ADD_EDGE_FUNC(pgraph, nTail, nHead, nCost, nEdge,
            pvHeadAttr, pvTailAttr, pvEdgeAttr, nFlags & ~DGL_STRONGCONNECT);
    }
