 ...
 */
int DGL_ADD_EDGE_FUNC(dglGraph_s *pgraph, dglInt32_t nHead, dglInt32_t nTail,
                      ...
                      void *pvTailAttr, void *pvEdgeAttr, dglInt32_t nFlags)
{
...
#ifdef DGL_STATS
    {
        ...
        if ((pHeadNodeItem = DGL_T_NODEITEM_Add(pgraph->pNodeTree, nHead)) ==
                NULL ||
            (pTailNodeItem = DGL_T_NODEITEM_Add(pgraph->pNodeTree, nTail)) ==
                NULL) {
    ...
    if ((pEdgeset = DGL_T_NODEITEM_OutEdgesetPTR(pHeadNodeItem)) == NULL) {
        ...
        pEdgeset =
            DGL_EDGESET_REALLOC(pEdgeset, DGL_EDGESET_EDGECOUNT(pEdgeset) + 1,
                                pgraph->EdgeAttrSize);
    ...
    if ((pinEdgeset = DGL_T_NODEITEM_InEdgesetPTR(pTailNodeItem)) == NULL) {
        ...
        pinEdgeset = DGL_EDGESET_REALLOC(pinEdgeset,
                                         DGL_EDGESET_EDGECOUNT(pinEdgeset) + 1,
                                         pgraph->EdgeAttrSize);
...
#if defined(_DGL_V1)
    pEdge = DGL_EDGESET_EDGE_PTR(pEdgeset, DGL_EDGESET_EDGECOUNT(pEdgeset),
                                 pgraph->EdgeAttrSize);
    DGL_EDGESET_EDGECOUNT(pEdgeset)++;
#endif

    DGL_EDGE_HEADNODE_OFFSET(pEdge) =
        nHead; /* will be an offset after flattening */
    DGL_EDGE_TAILNODE_OFFSET(pEdge) =
        nTail; /* will be an offset after flattening */
    ...
    if (pgraph->nOptions & DGL_GO_EdgePrioritize_COST) {
        if (dgl_edge_prioritizer_add(pgraph, DGL_EDGE_ID(pEdge),
                                     DGL_EDGE_COST(pEdge)) < 0) {
            return -pgraph->iErrno;
        }
    }
#endif
    ...
    if (nFlags & DGL_STRONGCONNECT) {
        return DGL_ADD_EDGE_FUNC(pgraph, nTail, nHead, nCost, nEdge, pvHeadAttr,
                                 pvTailAttr, pvEdgeAttr,
...
>>>>>>> 8422103f4c (wxpyimgview: explicit conversion to int (#2704))
{
    ...
    if (DGL_DEL_NODE_INEDGE_FUNC(pgraph, DGL_EDGE_TAILNODE_OFFSET(pEdge),
                                 DGL_EDGE_ID(pEdge)) < 0) {
        return -pgraph->iErrno;
    }

    if (DGL_DEL_NODE_OUTEDGE_FUNC(pgraph, DGL_EDGE_HEADNODE_OFFSET(pEdge),
                                  DGL_EDGE_ID(pEdge)) < 0) {
        return -pgraph->iErrno;
    }
     ...
     */
    if (pgraph->nOptions & DGL_GO_EdgePrioritize_COST) {
        if (dgl_edge_prioritizer_del(pgraph, DGL_EDGE_ID(pEdge),
                                     DGL_EDGE_COST(pEdge)) < 0) {
            return -pgraph->iErrno;
        }
    }
