    ...
    int *ivals, int nvals)
{
    ...
    if (selcol) {
        if (!ivals || (ivals && nvals == 0)) {
            G_warning(_("Array of values to select from column <%s> is empty"),
                selcol);
            return DB_FAILED;
        }
    ...
    if (db_list_tables(to_driver, &tblnames, &count, 0) != DB_OK) {
        G_warning(_("Unable to get list tables in database <%s>"), to_dbname);
        db_close_database_shutdown_driver(to_driver);
        if (from_driver != to_driver)
            db_close_database_shutdown_driver(from_driver);
    ...
    for (i = 0; i < count; i++) {
        ...
        if (strcmp(to_drvname, "pg") == 0) {
            ...
            if (p) {
                ...
                sprintf(buf, "%s.%s",
                    connection.schemaName ? connection.schemaName : "public",
                    to_tblname);
                if (strcmp(buf, tbl) == 0)
    ...
    for (col = 0; col < ncols; col++) {
        ...
        if (selcol && G_strcasecmp(colname, selcol) == 0) {
            if (ctype != DB_C_TYPE_INT)
                G_fatal_error(_("Column <%s> is not integer"), colname);
            selcol_found = 1;
        }
    ...
    if (db_create_table(to_driver, out_table) != DB_OK) {
        G_warning(_("Unable to create table <%s>"), to_tblname);
        db_close_database_shutdown_driver(to_driver);
        if (from_driver != to_driver) {
            db_close_database_shutdown_driver(from_driver);
        }
    ...
    while (1) {
        ...
        for (col = 0; col < ncols; col++) {
            ...
            case DB_C_TYPE_DOUBLE:
                ...
                break;
            default:
                G_warning(_("Unknown column type (column <%s>)"), colname);
                db_close_cursor(&cursor);
                db_close_database_shutdown_driver(to_driver);
                if (from_driver != to_driver) {
                    db_close_database_shutdown_driver(from_driver);
    ...
    const char *to_dbname, const char *to_tblname)
{
    return copy_table(from_drvname, from_dbname, from_tblname,
        to_drvname, to_dbname, to_tblname, NULL, NULL, NULL, NULL, 0);
}
...
int db_copy_table_where(const char *from_drvname, const char *from_dbname,
    const char *from_tblname, const char *to_drvname,
    const char *to_dbname, const char *to_tblname, const char *where)
{
    return copy_table(from_drvname, from_dbname, from_tblname,
        to_drvname, to_dbname, to_tblname, where, NULL, NULL, NULL, 0);
}
...
int db_copy_table_select(const char *from_drvname, const char *from_dbname,
    const char *from_tblname, const char *to_drvname,
    const char *to_dbname, const char *to_tblname, const char *select)
{
    return copy_table(from_drvname, from_dbname, from_tblname,
        to_drvname, to_dbname, to_tblname, NULL, select, NULL, NULL, 0);
}
...
{
    return copy_table(from_drvname, from_dbname, from_tblname,
        to_drvname, to_dbname, to_tblname, NULL, NULL, selcol, ivals, nvals);
}
