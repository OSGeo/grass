...
/*!
   ...
   Opens the existing cell file <i>name</i> in the <i>subproject</i> for
   reading by Rast_get_row() with mapping into the current window.

   This routine opens the raster map <i>name</i> in <i>subproject</i> for
   ...
   \param name map name
   \param subproject subproject name where raster map <i>name</i> lives

   \return nonnegative file descriptor (int)
 */
int Rast_open_old(const char *name, const char *subproject)
{
    int fd = Rast__open_old(name, subproject);
...
/*!  \brief Lower level function, open cell files, supercell files,
   ...
   \param name map name
   \param subproject subproject of cell file to be opened

   \return open file descriptor
 */
int Rast__open_old(const char *name, const char *subproject)
{
    struct fileinfo *fcb;
    int cell_fd, fd;
    char *cell_dir;
    const char *r_name;
    const char *r_subproject;
    ...
    reclass_flag = Rast_get_reclass(name, subproject, &reclass);

    switch (reclass_flag) {
    case 0:
	r_name = name;
	r_subproject = subproject;
	break;
    case 1:
	r_name = reclass.name;
	r_subproject = reclass.subproject;
	if (!G_find_raster2(r_name, r_subproject))
	    G_fatal_error(_("Unable to open raster map <%s@%s> since it is a reclass "
			    "of raster map <%s@%s> which does not exist"),
			  name, subproject, r_name, r_subproject);
	break;
    ...
    Rast_get_cellhd(r_name, r_subproject, &cellhd);

    /* now check the type */
    MAP_TYPE = Rast_map_type(r_name, r_subproject);
    if (MAP_TYPE < 0)
	...
	G_fatal_error(_("Raster map <%s> is in different zone (%d) than current region (%d)"),
		      G_fully_qualified_name(name, subproject), cellhd.zone, R__.rd_window.zone);
	...
	G_fatal_error(_("Raster map <%s>: bytes per cell (%d) too large"),
		      G_fully_qualified_name(name, subproject), CELL_nbytes);
    ...
    gdal = Rast_get_gdal_link(r_name, r_subproject);
    vrt = Rast_get_vrt(r_name, r_subproject);
    cell_fd = -1;
    if (gdal) {
#ifdef HAVE_GDAL
	cell_fd = -1;
	...
	cell_fd = G_open_old(cell_dir, r_name, r_subproject);
	if (cell_fd < 0)
	    G_fatal_error(_("Unable to open %s file for raster map <%s@%s>"),
			  cell_dir, r_name, r_subproject);
    }
    ...
    /* save name and subproject */
    fcb->name = G_store(name);
    fcb->subproject = G_store(subproject);
	...
	if (fcb->reclass_flag)
	    Rast_read_quant(fcb->reclass.name, fcb->reclass.subproject,
			    &(fcb->quant));
	else
	    Rast_read_quant(fcb->name, fcb->subproject, &(fcb->quant));
    }
	...
	fcb->null_fd = G_open_old_misc("cell_misc", NULL_FILE, r_name, r_subproject);
	if (fcb->null_fd < 0) {
	    fcb->null_fd = G_open_old_misc("cell_misc", NULLC_FILE, r_name, r_subproject);
	    if (fcb->null_fd >= 0) {
...
/*!
   ...
   Opens a new cell file <i>name</i> in the current subproject for writing
   by Rast_put_row().
...
/*!
   ...
   Opens a new floating-point map <i>name</i> in the current subproject for
...
#ifdef HAVE_GDAL
static int open_raster_new_gdal(char *map, char *subproject,
				RASTER_MAP_TYPE map_type)
{
    int fd;
    struct fileinfo *fcb;
			   ...
			   RASTER_MAP_TYPE map_type)
{
	...
	G_fatal_error(_("Raster map <%s> is not in the current subproject (%s)"),
		      name, G_subproject());
    map = G_store(xname);
    subproject = G_store(xsubproject);
	...
	return open_raster_new_gdal(map, subproject, map_type);
#endif
     ...
     */
    G_make_subproject_element(cell_dir);
    ...
    /* save name and subproject, and tempfile name */
    fcb->name = map;
    fcb->subproject = subproject;
...
int Rast__open_null_write(const char *name)
{
    ...
    if (!G_find_raster2(name, G_subproject()))
	G_fatal_error(_("Raster map <%s> does not exist in the current subproject (%s)"),
		      name, G_subproject());
    ...
    Rast_get_cellhd(map, subproject, &fcb->cellhd);
...
/*!
   ...
   Returns true (1) if raster map <i>name</i> in <i>subproject</i>
   ...
   \return 0 int
 */
int Rast_map_is_fp(const char *name, const char *subproject)
{
    char path[GPATH_MAX];
    const char *xsubproject;

    xsubproject = G_find_raster2(name, subproject);
    if (!xsubproject)
	G_fatal_error(_("Raster map <%s> not found"),
		      G_fully_qualified_name(name, subproject));

    G_file_name(path, "fcell", name, xsubproject);
    if (access(path, 0) == 0)
	return 1;

    G_file_name(path, "g3dcell", name, xsubproject);
    if (access(path, 0) == 0)
	return 1;
...
/*!
   ...
   \param name map name 
   \param subproject subproject where map <i>name</i> lives

   \return raster data type
 */
RASTER_MAP_TYPE Rast_map_type(const char *name, const char *subproject)
{
    char path[GPATH_MAX];
    const char *xsubproject;
	...
	if (subproject && *subproject)
	    G_fatal_error(_("Raster map <%s> not found in subproject <%s>"),
			  name, subproject);
	else
    ...
    if (access(path, 0) == 0)
	return Rast__check_fp_type(name, xsubproject);
 ...
 */
RASTER_MAP_TYPE Rast__check_fp_type(const char *name, const char *subproject)
{
    char path[GPATH_MAX];
    ...
    G_file_name_misc(path, "cell_misc", FORMAT_FILE, name, xsubproject);
