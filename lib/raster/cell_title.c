...
/**************************************************************
 * char *Rast_get_cell_title (name, subproject)
 *   char *name        name of map file
 *   char *subproject      subproject containing name
 *
 ...
 * <b>name</b> in <b>subproject</b> directly from the category file, and returns
...
char *Rast_get_cell_title(const char *name, const char *subproject)
{
    FILE *fd;
    int stat;
    char title[1024];
    ...
    stat = -1;
    fd = G_fopen_old("cats", name, subproject);
    if (fd) {
	stat = 1;
