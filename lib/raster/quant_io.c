...
/* redundant: integer range doesn't exist now: it is defined by
   the quant rules */
static int quant_load_range(struct Quant *quant, const char *name, const char *subproject)
{
    ...
    if (Rast_read_fp_range(name, subproject, &fprange) <= 0)
	return 0;
...
/*!
  ...
  Reads quantization rules for raster map <i>name</i> in <i>subproject</i>
  and stores them in the quantization structure "quant". If the map is
  in another subproject, first checks for quant2 table for this map in
  current subproject.
  ...
  \return -1 if map name is fully qualified and subproject is not the current one
  \return 0 if quantization file does not exist, or the file is empty, 
  \return 1 if non-empty quantization file exists.
*/
int Rast__quant_import(const char *name, const char *subproject,
		       struct Quant *quant)
{
    char buf[1024];
    char xname[GNAME_MAX], xsubproject[GMAPSET_MAX], element[GNAME_MAX + 7];
    int parsStat;
    FILE *fd;
	...
	if (strlen(subproject) == 0)
	    subproject = xsubproject;
	else if (strcmp(xsubproject, subproject) != 0)
	    return -1;
	name = xname;
    }

    /* first check if quant2/subproject/name exists in the current subproject */
    sprintf(element, "quant2/%s", subproject);
    if ((fd = G_fopen_old(element, name, G_subproject()))) {
    ...
    if (!(fd = G_fopen_old_misc("cell_misc", QUANT_FILE_NAME, name, subproject))) {

	/* int range doesn't exist anymore if (quant_load_range (quant, name, subproject)>0) return 3; */
	...
	if (parsStat)
	    return 1;
	/* int range doesn't exist anymore if (quant_load_range (quant, name, subproject)>0) return 2; */
...
/*!
  ...
  . If the subproject is the same as the current subproject, the quant file is
  created in 'cell_misc/name' directory, otherwise it is created in
  'quant2/subproject' directory, much like writing colors for map in
  another subproject.  The rules are written in decreasing order of
  ...
  \return 1 otherwise.
*/
int Rast__quant_export(const char *name, const char *subproject,
		       const struct Quant *quant)
{
    ...
    if (strcmp(G_subproject(), subproject) == 0) {
	G_remove_misc("cell_misc", QUANT_FILE_NAME, name);
	G__make_subproject_element_misc("cell_misc", name);
