    ...
    int *nbytes)
{
    ...
    if (lseek(fcb->data_fd, t1, SEEK_SET) < 0)
        G_fatal_error(_("Error seeking fp raster data file for row %d of <%s>: %s"), row,
    ...
    int *nbytes)
{
    ...
    if (lseek(fcb->data_fd, t1, SEEK_SET) < 0)
        G_fatal_error(_("Error seeking raster data file for row %d of <%s>: %s"), row,
            fcb->name, strerror(errno));
    ...
    int *nbytes)
{
    ...
    *nbytes = fcb->nbytes;
    ...
    err =
        ...
        fcb->cellhd.cols, 1, buf, fcb->cellhd.cols, 1, fcb->gdal->type, 0, 0);
    ...
    if (err != CE_None)
        G_fatal_error(_("Error reading raster data via GDAL for row %d of <%s>"), row,
            fcb->name);
}
#endif
...
static void cell_values_int(int fd, const unsigned char *data,
    const COLUMN_MAPPING * cmap, int nbytes, void *cell, int n)
{
    CELL *c = cell;
...
static void gdal_values_int(int fd, const unsigned char *data,
    const COLUMN_MAPPING * cmap, int nbytes, CELL * cell, int n)
{
    struct fileinfo *fcb = &R__.fileinfo[fd];
    const unsigned char *d;
    COLUMN_MAPPING cmapold = 0;
...
static void gdal_values_float(int fd, const float *data,
    const COLUMN_MAPPING * cmap, int nbytes, FCELL * cell, int n)
{
    COLUMN_MAPPING cmapold = 0;
    int i;
...
static void gdal_values_double(int fd, const double *data,
    const COLUMN_MAPPING * cmap, int nbytes, DCELL * cell, int n)
{
    COLUMN_MAPPING cmapold = 0;
    int i;
...
static void transfer_to_cell_XX(int fd, void *cell)
{
    ...
    if (fcb->gdal)
        (gdal_values_type[fcb->map_type]) (fd, fcb->data, fcb->col_map,
            fcb->cur_nbytes, cell, R__.rd_window.cols);
    else
#endif
...
static void get_map_row_no_reclass(int fd, void *rast, int row,
    RASTER_MAP_TYPE data_type, int null_is_zero, int with_mask)
{
    ...
    int row, size_t size, int fd)
{
    ...
    if (lseek(null_fd, t1, SEEK_SET) < 0)
        G_fatal_error(_("Error seeking compressed null data for row %d of <%s>"), row,
            fcb->name);

    if (readamount == size) {
        if (read(null_fd, flags, size) != size) {
            G_fatal_error(_("Error reading compressed null data for row %d of <%s>"), row,
                fcb->name);
        }
        return 1;
    }
...
static void get_null_value_row_gdal(int fd, char *flags, int row)
{
    ...
    for (i = 0; i < R__.rd_window.cols; i++)
        ...
        flags[i] = !fcb->col_map[i] ||
            tmp_buf[i] == fcb->gdal->null_val || tmp_buf[i] != tmp_buf[i];
