 ...
 * in <i>subproject</i> into the <i>hist</i> structure.
 *
 ...
 * \return 0 on success
 */
int Rast_read_history(const char *name, const char *subproject,
		      struct History *hist)
{
    FILE *fp;
    ...
    fp = G_fopen_old("hist", name, subproject);
    if (!fp) {
 ...
 * <i>name</i> in the current subproject from the <i>hist</i> structure.
 *
 * A diagnostic message is printed and -1 is returned if there is an
			...
			struct History *hist)
{
    ...
    Rast_set_history(hist, HIST_MAPSET, G_subproject());
