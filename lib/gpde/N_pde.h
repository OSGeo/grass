...
/*!
 * \brief the maximum number of available cell states (eg: boundary condition,
 * inactiven active)
 * */
 ...
 * */
typedef struct {
    int planimetric; /*If the projection is not planimetric (0), the array
                        calculation is different for each row */
    double *area;    /* the vector of area values for non-planimetric projection
                        for each row */
    int dim;         /* 2 or 3 */
...
extern N_geom_data *N_init_geom_data_3d(RASTER3D_Region *region3d,
                                        N_geom_data *geodata);
...
typedef struct {
    ...
    CELL *cell_array;   /*The data is stored in an one dimensional array
                           internally */
    FCELL *fcell_array; /*The data is stored in an one dimensional array
                           internally */
    DCELL *dcell_array; /*The data is stored in an one dimensional array
                           internally */
} N_array_2d;
...
typedef struct {
    ...
    float *fcell_array;  /*The data is stored in an one dimensional array
                            internally */
    double *dcell_array; /*The data is stored in an one dimensional array
                            internally */
} N_array_3d;

extern N_array_3d *N_alloc_array_3d(int cols, int rows, int depths, int offset,
                                    int type);
...
extern N_data_star *N_create_9star(double C, double W, double E, double N,
                                   ...
                                   double SE, double V);
extern N_data_star *
N_create_27star(double C, double W, double E, double N, double S, double NW,
                double SW, double NE, double SE, double T, double W_T,
                double E_T, double N_T, double S_T, double NW_T, double SW_T,
                double NE_T, double SE_T, double B, double W_B, double E_B,
                double N_B, double S_B, double NW_B, double SW_B, double NE_B,
 ...
 * */
typedef enum {
    N_OPT_SOLVER_SYMM,     /*! solver for symmetric, positive definite linear
                              equation systems */
    N_OPT_SOLVER_UNSYMM,   /*! solver for unsymmetric linear equation systems */
    N_OPT_MAX_ITERATIONS,  /*! Maximum number of iteration used to solver the
                              linear equation system */
    N_OPT_ITERATION_ERROR, /*! Error break criteria for the iterative solver
                              (jacobi, sor, cg or bicgstab) */
    N_OPT_SOR_VALUE, /*! The relaxation parameter used by the jacobi and sor
                        solver for speedup or stabilizing */
...
extern N_gradient_neighbours_x *
N_create_gradient_neighbours_x(double NWN, double NEN, double WC, double EC,
...
extern N_gradient_neighbours_y *
N_create_gradient_neighbours_y(double NWW, double NEE, double NC, double SC,
...
extern N_gradient_neighbours_z *
N_create_gradient_neighbours_z(double NWZ, double NZ, double NEZ, double WZ,
                               double CZ, double EZ, double SWZ, double SZ,
                               double SEZ);
...
extern N_gradient_neighbours_3d *N_create_gradient_neighbours_3d(
...
extern N_gradient_field_2d *
N_compute_gradient_field_2d(N_array_2d *pot, N_array_2d *weight_x,
                            N_array_2d *weight_y, N_geom_data *geom,
                            N_gradient_field_2d *gradfield);
extern void N_compute_gradient_field_components_2d(N_gradient_field_2d *field,
                                                   N_array_2d *x_comp,
                                                   N_array_2d *y_comp);
...
extern void N_compute_gradient_field_components_3d(N_gradient_field_3d *field,
                                                   N_array_3d *x_comp,
                                                   N_array_3d *y_comp,
                                                   N_array_3d *z_comp);
