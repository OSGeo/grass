...
Rast3d_range_update_from_tile(RASTER3D_Map * map, const void *tile, int rows,
    int cols, int depths, int xRedundant, int yRedundant, int zRedundant,
    int nofNum, int type)
{
    int y, z, cellType;
        ...
        for (z = 0; z < depths; z++) {
            ...
            if (yRedundant)
                tile =
                    ...
                    map->tileX * yRedundant * Rast3d_length(type));
        }
        return;
    }
 ...
 /* adapted from Rast_write_fp_range */
{
    ...
    if (write(fd, xdr_buf,
            RASTER3D_XDR_DOUBLE_LENGTH * 2) != RASTER3D_XDR_DOUBLE_LENGTH * 2)
        goto error;
