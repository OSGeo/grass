...
void Rast3d_change_type(void *map, const char *nameOut)
{
    ...
    map2 =
        Rast3d_open_cell_new(nameOut, FCELL_TYPE, RASTER3D_USE_CACHE_DEFAULT,
        &region);

    if (map2 == NULL)
        Rast3d_fatal_error
            ("Rast3d_change_type: error in Rast3d_open_cell_new");
    ...
    for (z = 0; z < nz; z++)
        for (y = 0; y < ny; y++)
            for (x = 0; x < nx; x++) {
                if (!Rast3d_read_tile(map, Rast3d_tile2tile_index(map, x, y,
                            z), data, typeIntern))
                    Rast3d_fatal_error
                        ("Rast3d_change_type: error in Rast3d_read_tile");
                Rast3d_copy_values(data, 0, typeIntern, data2, 0, typeIntern2,
                    tileSize);
                if (!Rast3d_write_tile(map2, Rast3d_tile2tile_index(map2, x, y,
                            z), data2, typeIntern2))
                    Rast3d_fatal_error
                        ("Rast3d_change_type: error in Rast3d_write_tile");
            }
