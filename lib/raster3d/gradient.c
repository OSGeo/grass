...
void Rast3d_gradient_double(RASTER3D_Array_double * array, double *step,
    ...
    RASTER3D_Array_double * grad_y, RASTER3D_Array_double * grad_z)
{
    int col, row, depth;
    ...
    for (depth = 0; depth < array->sz; depth++) {
        for (row = 0; row < array->sy; row++) {
            ...
            if (Rast_is_d_null_value(&val0))
                Rast_set_null_value(&RASTER3D_ARRAY_ACCESS(grad_x, 0, row,
                        depth), 1, DCELL_TYPE);
            ...
            if (Rast_is_d_null_value(&val2))
                Rast_set_null_value(&RASTER3D_ARRAY_ACCESS(grad_x,
                        array->sx - 1, row, depth), 1, DCELL_TYPE);
            ...
            for (col = 1; col < array->sx - 1; col++) {
                ...
                if (Rast_is_d_null_value(&val1))
                    Rast_set_null_value(&RASTER3D_ARRAY_ACCESS(grad_x, col,
                            row, depth), 1, DCELL_TYPE);
                else if (Rast_is_d_null_value(&val0) ||
                    Rast_is_d_null_value(&val2))
    ...
    for (depth = 0; depth < array->sz; depth++) {
        for (col = 0; col < array->sx; col++) {
            ...
            if (Rast_is_d_null_value(&val0))
                Rast_set_null_value(&RASTER3D_ARRAY_ACCESS(grad_y, col, 0,
                        depth), 1, DCELL_TYPE);
            else if (Rast_is_d_null_value(&val1) ||
                Rast_is_d_null_value(&val2))
            ...
            if (Rast_is_d_null_value(&val2))
                Rast_set_null_value(&RASTER3D_ARRAY_ACCESS(grad_y, col,
                        array->sy - 1, depth), 1, DCELL_TYPE);
            else if (Rast_is_d_null_value(&val0) ||
                Rast_is_d_null_value(&val1))
    ...
    for (row = 0; row < array->sy; row++) {
        for (col = 0; col < array->sx; col++) {
            ...
            if (Rast_is_d_null_value(&val0))
                Rast_set_null_value(&RASTER3D_ARRAY_ACCESS(grad_z, col, row,
                        0), 1, DCELL_TYPE);
            else if (Rast_is_d_null_value(&val1) ||
                Rast_is_d_null_value(&val2))
            ...
            for (depth = 1; depth < array->sz - 1; depth++) {
                ...
                if (Rast_is_d_null_value(&val1))
                    Rast_set_null_value(&RASTER3D_ARRAY_ACCESS(grad_z, col,
                            row, depth), 1, DCELL_TYPE);
                else if (Rast_is_d_null_value(&val0) ||
                    Rast_is_d_null_value(&val2))
