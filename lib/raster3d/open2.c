 ...
 * are stored in memory with <em>typeIntern</em> which must be one of
 ...
 * <em>region</em> specifies the 3d region. The map is created using the
 * <em>type</em> which must be of FCELL_TYPE or DCELL_TYPE. The methods for
 * compression can be specified LZW or RLE. The digits of the floating point
 * mantissa can be specified. In case of FCELL_TYPE 0-23 and 0-52 in case of
 * DCELL_TYPE. The number cells in X, Y and Z direction defines the size of each
 * tile. Returns a pointer to the cell structure ... if successful, NULL ...
 * otherwise.
 *
 ...
 * \brief
 *
 * Opens new g3d-file with <em>name</em> in the current mapset. This method
 * tries to compute optimal tile size based on the number of rows, cols and
 * depths and the maximum allowed tile size in KB. Tiles are stored in memory
 * using RASTER3D_TILE_SAME_AS_FILE method. <em>cache</em> specifies the
 ...
 * positive integer which specifies the number of tiles buffered in the cache.
 * <em>region</em> specifies the 3d region. The map is created using the
 * <em>type</em> which must be of FCELL_TYPE or DCELL_TYPE. Returns a pointer to
 * the cell structure ... if successful, NULL ... otherwise.
 *
 *  \param name The name of the map
 *  \param cache The type of the caching
 *  \param region The region of the map
