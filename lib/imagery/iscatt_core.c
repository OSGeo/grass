...
static int get_rows_and_cols_bounds(struct Cell_head *A, struct Cell_head *B,
    struct Cell_head *A_bounds, struct Cell_head *B_bounds)
{
    float ns_res, ew_res;
...
int I_insert_patch_to_cat_rast(const char *patch_rast,
    struct Cell_head *cat_rast_region, const char *cat_rast)
{
...
static void update_cat_scatt_plts(struct rast_row *bands_rows,
    unsigned short *belongs_pix, struct scScatts *scatts)
{
...
static int compute_scatts_from_chunk_row(struct scCats *scatt_conds,
    ...
    struct rast_row *bands_rows, struct scCats *scatts, int *fd_cats_rasts)
{
...
int I_compute_scatts(struct Cell_head *region, struct scCats *scatt_conds,
    ...
    int n_bands, struct scCats *scatts, const char **cats_rasts)
{
    const char *mapset;
    char header[1024];
...
int I_apply_colormap(unsigned char *vals, unsigned char *vals_mask,
    unsigned nvals, unsigned char *colmap, unsigned char *col_vals)
{
    unsigned int i_val;
    int v, i, i_cm;
