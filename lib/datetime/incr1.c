...
static int _datetime_subtract_field (DateTime *src, DateTime *incr, int field)
{
	...
	switch(field){
		...
		if(src->day <= incr->day){ 
			...
			newdays += datetime_days_in_month(
				   cpsrc.year, cpsrc.month, cpsrc.positive);
			borrow++;
		    }
		    src->day = newdays;
		}
...
/* check here & below - srcinc may be a day-second interval - mess anything up? */
		...
		if(src->year <= incr->year){ /* SIGN CHANGE */
			...
			for(i = src->month -1; i > 0; i--){
			    tinc.day += datetime_days_in_month(
				       src->year, i, src->positive);
			}
			tinc.hour = src->hour;
...
_datetime_carry (DateTime *dt, int absolute)
{
  int i, carry;
	...
	while(dt->day > datetime_days_in_month (dt->year, dt->month, dt->positive))
	{
