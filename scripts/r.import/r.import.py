...
# % descriptions: nearest;nearest neighbor;bilinear;bilinear interpolation;
# %  bicubic;bicubic interpolation;lanczos;lanczos filter;
# %  bilinear_f;bilinear interpolation with fallback;bicubic_f;
# %  bicubic interpolation with fallback;lanczos_f;lanczos filter with fallback
# % answer: nearest
# % guisection: Output
# %end
# %option
...
# % descriptions: estimated;estimated resolution;value;
# %  user-specified resolution;region;current region resolution
# % guisection: Output
# %end
# %option
...
# %flag
# % key: l
# % description: Force Lat/Lon maps to fit into geographic coordinates
# %  (90N,S; 180E,W)
# %end
# %flag
# % key: o
# % label: Override projection check (use current project's CRS)
# % description: Assume that the dataset has the same coordinate reference
# %  system (CRS) as the current project
# %end
# %rules
# % required: output,-e
# %end
...
def main():
    ...
    for outfile in outfiles:
        ...
        if tgtres == "estimated":
            ...
            gs.message(
                _(
                    "Using given resolution for input band <{out}>: {res}"
                ).format(
                    out=outfile, res=res
                )
            )
