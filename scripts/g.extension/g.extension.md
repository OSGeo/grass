...
-   Python scripts: they are installed without the need of compilation
    or (usually) the need of special dependencies.
...
*[g.extension.all](g.extension.all.html)* module is available.
...
[variables](variables.html)). *g.extension* checks if the user has
...
repository, also the grass-dev\* package (commonly named \"grass-dev\"
or \"grass-devel\", sometimes along with the version number) must be
installed.
...
(Python rewrite)\
Vaclav Petras, [NCSU OSGeoREL](http://gis.ncsu.edu/osgeorel/) (support
for general sources, partial refactoring)
