...
def download_source_code_official_github(url, name, outdev, directory=None):
    ...
    if (
        gs.call(
            ["git", "clone", "--no-checkout", "--depth=1", "--filter=tree:0", GIT_URL],
            stdout=outdev,
        )
        != 0
    ):
        gs.fatal(_("Could not clone repository <{url}> not found").format(url))

    # Get a tree of files and directories
    gs.call(["git", "ls-tree", "--name-only", "-r", "HEAD"], cwd="./grass-addons")
    addons_list = gs.call(
        ["git", "ls-tree", "--name-only", "-r", "HEAD"],
        cwd="./grass-addons",
        stdout=PIPE,
    )
    addons_list = addons_list.decode("UTF8").split("\n")
    # addons_list = set(["|".join(addon_file.split("/")[1:3]) for addon_file in addons_list if len(addon_file.split("/")) >=3 and addon_file.startswith("src") and "Makefile" not in addon_file])
