...
int GPJ_transform_array(const struct pj_info *, const struct pj_info *,
    const struct pj_info *, int, double *, double *, double *, int);

/* old API, to be removed */
int pj_do_proj(double *, double *, const struct pj_info *,
    const struct pj_info *);
int pj_do_transform(int, double *, double *, double *, const struct pj_info *,
    const struct pj_info *);

/* get_proj.c */
/* TODO: rename pj_ to GPJ_ to avoid symbol clash with PROJ lib */
int pj_get_kv(struct pj_info *, const struct Key_Value *,
...
#ifdef HAVE_OGR
OGRSpatialReferenceH GPJ_grass_to_osr(const struct Key_Value *,
    const struct Key_Value *);
OGRSpatialReferenceH GPJ_grass_to_osr2(const struct Key_Value *,
