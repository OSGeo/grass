 ...
 */
template < class T > class BlockHeapElement {
  public:
    T value;
    MEM_STREAM < T > *run;
 ...
 */
template < class T, class Compare > class ReplacementHeapBlock {
  private:
...
template < class T, class Compare >
    ReplacementHeapBlock < T,
    Compare >::ReplacementHeapBlock(queue < MEM_STREAM < T > *>*runList)
{
...
/*****************************************************************/
template < class T, class Compare >
    ReplacementHeapBlock < T, Compare >::~ReplacementHeapBlock < T,
    Compare > () {
    ...
    void ReplacementHeapBlock < T, Compare >::addRun(MEM_STREAM < T > *r)
{
    ...
    if (size == arity) {
        cerr << "ReplacementHeapBlockBlock::addRun size =" << size << ",arity="
            << arity << " full, cannot add another run.\n";
        assert(0);
        exit(1);
    }
    assert(size < arity);
    ...
    void ReplacementHeapBlock < T, Compare >::heapify(size_t i)
{
    ...
    assert(i >= 0 && i < size);
    if ((lc < size) &&
        (cmpobj.compare(mergeHeap[lc].value,
                mergeHeap[min_index].value) == -1)) {
        min_index = lc;
    }
    if ((rc < size) &&
        (cmpobj.compare(mergeHeap[rc].value,
                mergeHeap[min_index].value) == -1)) {
        min_index = rc;
    }
