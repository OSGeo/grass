...
template < class T > class im_buffer {
...
/************************************************************/
//copy constructor
template < class T > im_buffer < T >::im_buffer(const im_buffer & b)
{
...
template < class T > bool im_buffer < T >::insert(T & x)
{
...
//(quick)sort (ascending order) the buffer (in place); 
//the buffer is overwritten; recursive for the time being..
template < class T > void im_buffer < T >::sort()
{
    if (!is_empty()) {
        if (!sorted) {
...
//pivot
template < class T >
    unsigned long im_buffer < T >::partition(unsigned long start,
    unsigned long end)
{
...
/************************************************************/
//shift n items to the left: in effect this deletes the first n items
template < class T > void im_buffer < T >::shift_left(unsigned long n)
{
    assert(n <= size);
    //remains sorted
    if (n) {
...
template < class T > void im_buffer < T >::print_range() const
{
...
template < class T > void im_buffer < T >::print() const
{
    cout << "[";
    for (unsigned long i = 0; i < size; i++) {
...
template < class T > AMI_STREAM < T > *im_buffer < T >::save2str()const
{
