...
template < class T, class Key > EMPQueueAdaptive < T, Key >::EMPQueueAdaptive()
{
    regim = INMEM;
    EMPQAD_DEBUG cout << "EMPQUEUEADAPTIVE: starting in-memory pqueue" << endl;
    ...
    void EMPQueueAdaptive < T, Key >::initPQ(size_t initMem)
{
    ...
    EMPQAD_DEBUG cout << "sz_stream: " << sz_stream << " buf_arity: " <<
        buf_arity << " mm_overhead: " << mm_overhead << " mm_avail: " <<
        initMem << ".\n";
...
template < class T, class Key > long EMPQueueAdaptive < T, Key >::maxlen() const
{
    long m = -1;
...
template < class T, class Key > bool EMPQueueAdaptive < T, Key >::is_empty() const
{
    bool v = false;
...
//return true if full
template < class T, class Key > bool EMPQueueAdaptive < T, Key >::is_full() const
{
...
template < class T, class Key > bool EMPQueueAdaptive < T, Key >::min(T & elt)
{
    bool v = false, v1;
    T tmp;
...
template < class T, class Key > void EMPQueueAdaptive < T, Key >::clear()
{
    switch (regim) {
    case INMEM:
        im->clear();
...
template < class T, class Key > void EMPQueueAdaptive < T, Key >::verify()
{
    switch (regim) {
    case INMEM:
        im->verify();
...
template < class T, class Key > long EMPQueueAdaptive < T, Key >::size() const
{
    long v = 0, v1;
...
template < class T > class baseCmpType {
  public:
