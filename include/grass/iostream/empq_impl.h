...
template < class T, class Key > class ExtendedEltMergeType {
  ...
  public:
    ...
    ExtendedEltMergeType(T & e, unsigned short bid, unsigned int sid):x(e),
        buf_id(bid), str_id(sid)
    {
    }
...
template < class T, class Key >
    ...
    unsigned int buf_ar):pqsize(pq_sz), bufsize(buf_sz), max_nbuf(nb_buf),
crt_buf(0), buf_arity(buf_ar)
{
    ...
    if ((ae = dummy.main_memory_usage(&sz_stream,
                MM_STREAM_USAGE_MAXIMUM)) != AMI_ERROR_NO_ERROR) {
...
template < class T, class Key > em_pqueue < T, Key >::em_pqueue()
{
    ...
    AMI_STREAM < T > *amis)
{
    ...
    if (!(pqcurrentsize <= pqsize)) {
        cout << "EMPQ: pq maxsize=" << pqsize << ", pq crtsize=" <<
            pqcurrentsize << "\n";
        assert(0);
        exit(1);
    }
    ...
    long em_pqueue < T, Key >::maxlen(unsigned short i)
{
    ...
    em_buffer < T, Key > *tmp =
        new em_buffer < T, Key > (i + 1, bufsize, buf_arity);
    if (!tmp) {
...
template < class T, class Key > long em_pqueue < T, Key >::maxlen()
{
    long len = 0;
...
template < class T, class Key > unsigned long em_pqueue < T, Key >::size()
{
...
template < class T, class Key > bool em_pqueue < T, Key >::is_empty()
{
...
template < class T, class Key > bool em_pqueue < T, Key >::fillpq()
{
...
template < class T, class Key > bool em_pqueue < T, Key >::min(T & elt)
{
...
template < class T, class Key > bool em_pqueue < T, Key >::extract_min(T & elt)
{
...
//************************************************************/
//clean buffers in case some streams have been emptied
template < class T, class Key > void em_pqueue < T, Key >::cleanup()
{
...
template < class T, class Key > bool em_pqueue < T, Key >::insert(const T & x)
{
    bool ok;
 ...
 */
template < class T, class Key > bool em_pqueue < T, Key >::empty_buff_0()
{
#ifdef EMPQ_ASSERT_EXPENSIVE
    long init_size = size();
#endif
...
template < class T, class Key >
    ...
    unsigned short arity, ExtendedMergeStream * outstream, long K)
{
...
//************************************************************/
template < class T, class Key > void em_pqueue < T, Key >::clear()
{
    pq->clear();
    buff_0->clear();
...
//************************************************************/
template < class T, class Key > void em_pqueue < T, Key >::print_range()
{
...
//************************************************************/
template < class T, class Key > void em_pqueue < T, Key >::print()
{
    cout << "EM_PQ: [pq=" << pqsize
        << ", b=" << bufsize
        << ", bufs=" << max_nbuf << ", ar=" << buf_arity << "]\n";
...
//************************************************************/
template < class T, class Key > void em_pqueue < T, Key >::print_size()
{
    //sum up the lengths(nb of elements) of the external buffers 
    long elen = 0;

    cout << "EMPQ: pq=" << pq->size() << ",B0=" << buff_0->
        get_buf_len() << endl;
    cout.flush();
    for (unsigned short i = 0; i < crt_buf; i++) {
        assert(buff[i]);
    ...
    cout << "total: " << elen + pq->size() +
        buff_0->get_buf_len() << endl << endl;
    cout.flush();
}
...
/*****************************************************************/
template < class T, class Key > void em_pqueue < T, Key >::print_stream_sizes()
{
    for (unsigned short i = 0; i < crt_buf; i++) {
        cout << "[";
        buff[i]->print_stream_sizes();
