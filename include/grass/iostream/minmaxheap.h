...
template < class T > class BasicMinMaxHeap {
  protected:
    HeapIndex maxsize;
  ...
  private:
    ...
    int isOnMaxLevel(HeapIndex i) const
    {
        return (log2(i) % 2);
    };
    int isOnMinLevel(HeapIndex i) const
    {
        return !isOnMaxLevel(i);
    };

    HeapIndex leftChild(HeapIndex i) const
    {
        return 2 * i;
    };
    HeapIndex rightChild(HeapIndex i) const
    {
        return 2 * i + 1;
    };
    int hasRightChild(HeapIndex i) const
    {
    ...
    HeapIndex parent(HeapIndex i) const
    {
        return (i / 2);
    };
    HeapIndex grandparent(HeapIndex i) const
    {
        return (i / 4);
    };
...
template < class T > long BasicMinMaxHeap < T >::log2(long n) const
{
    long i = -1;
...
template < class T > void BasicMinMaxHeap < T >::swap(HeapIndex a, HeapIndex b)
{
    T tmp;
...
// child must exist
template < class T > T BasicMinMaxHeap < T >::leftChildValue(HeapIndex i) const
{
...
// child must exist
template < class T > T BasicMinMaxHeap < T >::rightChildValue(HeapIndex i) const
{
...
template < class T > HeapIndex BasicMinMaxHeap < T >::largestChild(HeapIndex i) const
{
    assert(hasChildren(i));
    if (hasRightChild(i) && (leftChildValue(i) < rightChildValue(i))) {
        return rightChild(i);
...
template < class T > void BasicMinMaxHeap < T >::trickleDownMin(HeapIndex i)
{
    HeapIndex m;
    bool done = false;
...
// unverified
template < class T > void BasicMinMaxHeap < T >::trickleDownMax(HeapIndex i)
{
    HeapIndex m;
    bool done = false;
...
template < class T > void BasicMinMaxHeap < T >::trickleDown(HeapIndex i)
{
    if (isOnMinLevel(i)) {
        trickleDownMin(i);
    }
...
// ----------------------------------------------------------------------
template < class T > void BasicMinMaxHeap < T >::bubbleUp(HeapIndex i)
{
    HeapIndex m;
...
// ----------------------------------------------------------------------
template < class T > void BasicMinMaxHeap < T >::bubbleUpMin(HeapIndex i)
{
    HeapIndex m;
...
// ----------------------------------------------------------------------
template < class T > void BasicMinMaxHeap < T >::bubbleUpMax(HeapIndex i)
{
    HeapIndex m;
...
#if(0)
// ----------------------------------------------------------------------
template < class T > void BasicMinMaxHeap < T >::print_rajiv() const
{
    HeapIndex i;
...
// ----------------------------------------------------------------------
template < class T > void BasicMinMaxHeap < T >::print() const
{
    cout << "[";
...
// ----------------------------------------------------------------------
template < class T > void BasicMinMaxHeap < T >::print_range() const
{
    cout << "[";
    T a, b;
...
// ----------------------------------------------------------------------
template < class T > void BasicMinMaxHeap < T >::insert(const T & elt)
{
#ifdef SAVE_MEMORY
    if (!A) {
...
// ----------------------------------------------------------------------
template < class T > bool BasicMinMaxHeap < T >::extract_min(T & elt)
{
...
// ----------------------------------------------------------------------
//extract all elts with min key, add them and return their sum
template < class T > bool BasicMinMaxHeap < T >::extract_all_min(T & elt)
{
    T next_elt;
    bool done = false;
...
// ----------------------------------------------------------------------
template < class T > bool BasicMinMaxHeap < T >::extract_max(T & elt)
{
...
// ----------------------------------------------------------------------
template < class T > bool BasicMinMaxHeap < T >::min(T & elt) const
{
...
// ----------------------------------------------------------------------
template < class T > bool BasicMinMaxHeap < T >::max(T & elt) const
{
...
template < class T > void BasicMinMaxHeap < T >::reset()
{
#ifdef SAVE_MEMORY
    assert(empty());
    MY_LOG_DEBUG_ID("minmaxheap: deallocation");
...
template < class T > void BasicMinMaxHeap < T >::clear()
{
    lastindex = 0;
}

// ----------------------------------------------------------------------
template < class T > T * BasicMinMaxHeap < T >::allocateHeap(HeapIndex n)
{
    T *p;
...
// ----------------------------------------------------------------------
template < class T > void BasicMinMaxHeap < T >::freeHeap(T * p)
{
    if (p) {
#ifdef USE_LARGEMEM
        LargeMemory::free(p);
...
template < class T > void BasicMinMaxHeap < T >::destructiveVerify()
{
    HeapIndex n = size();
    T val, prev;
    bool ok;
...
template < class T > void BasicMinMaxHeap < T >::verify()
{
    long n = size();
    T *dup;
...
template < class T > class MinMaxHeap:public BasicMinMaxHeap < T > {
  public:
...
//if size > maxsize, insert first maxsize elements from array;
//return nb of elements that did not fit;
template < class T > HeapIndex MinMaxHeap < T >::fill(T * arr, HeapIndex n)
{
    HeapIndex i;
...
template < class T > class UnboundedMinMaxHeap:public BasicMinMaxHeap < T > {
  public:
...
template < class T > void UnboundedMinMaxHeap < T >::grow()
{
    T *old = this->A;
