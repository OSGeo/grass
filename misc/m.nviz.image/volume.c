...
int add_isosurfs(const struct GParams *params, nv_data *data UNUSED)
{
    ...
    for (i = 0; params->isosurf_level->answers[i]; i++) {
        ...
        if (GVL_isosurf_set_att_const(id, nisosurfs - 1, ATT_TOPO, level) < 0) {
            G_fatal_error(
                _("Unable to set isosurface (%d) attribute (%d) of volume %d"),
        ...
        if (i < ncolor_map &&
            ...
            if (GVL_isosurf_set_att_map(id, nisosurfs - 1, ATT_COLOR,
                                        params->isosurf_color_map->answers[i]) <
                0)
                G_fatal_error(_("Unable to set isosurface (%d) attribute (%d) "
                                "of volume %d"),
            ...
            if (GVL_isosurf_set_att_const(
                    id, nisosurfs - 1, ATT_COLOR,
                    Nviz_color_from_str(
                        params->isosurf_color_const->answers[i - ncolor_map])) <
                0)
                G_fatal_error(_("Unable to set isosurface (%d) attribute (%d) "
                                "of volume %d"),
                              nisosurfs - 1, ATT_COLOR, id);
        ...
        if (i < nshine_map &&
            ...
            if (GVL_isosurf_set_att_map(id, nisosurfs - 1, ATT_SHINE,
                                        params->isosurf_shine_map->answers[i]) <
                0)
                G_fatal_error(_("Unable to set isosurface (%d) attribute (%d) "
                                "of volume %d"),
            ...
            if (GVL_isosurf_set_att_const(
                    ...
                    atof(
                        params->isosurf_shine_const->answers[i - nshine_map])) <
                0)
                G_fatal_error(_("Unable to set isosurface (%d) attribute (%d) "
                                "of volume %d"),
                              nisosurfs - 1, ATT_SHINE, id);
...
int add_slices(const struct GParams *params, nv_data *data UNUSED)
{
    ...
    for (i = 0; params->slice->answers[i]; i++) {
        ...
        if (G_number_of_tokens(tokens) != 2)
            G_fatal_error(_("Error tokenize '%s'"), params->slice->answers[i]);
        num = atoi(tokens[0]);
        ...
        /* set transparency */
        if (GVL_slice_set_transp(id, nslices - 1,
                                 atoi(params->slice_transp->answers[i])) < 0)
            G_fatal_error(
                _("Unable to set slice (%d) transparency of volume %d"),
                nslices - 1, id);
    }
