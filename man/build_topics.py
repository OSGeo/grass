...
for fname in htmlfiles:
    with Path(path, fname).open() as fil:
...
with Path(path, "topics.html").open("w") as topicsfile:
    topicsfile.write(
    ...
    for key, values in sorted(keywords.items(), key=lambda s: s[0].lower()):
        with Path(path, f"topic_{key}.html").open("w") as keyfile:
            keyfile.write(
                header1_tmpl.substitute(
                    title="GRASS GIS %s Reference Manual: Topic %s"
