...
if(WIN32)
    set(null_device nul)
    set(search_command findstr /r /v)
    ...
    if(G_ADDONS)
        ...
        if(NOT G_NO_DOCS)
            ...
            add_custom_command(
                ...
                COMMAND
                    ${_pythonpath} ${CMAKE_COMMAND} -E copy
                    ${_src_dir}/${G_NAME}.html
                    ${CMAKE_CURRENT_BINARY_DIR}/${G_NAME}.html
                COMMAND
                    ${_pythonpath} MODULE_TOPDIR=$ENV{GISBASE}
                    ${PYTHON_EXECUTABLE} ${MKHTML_PY} ${G_NAME} >
                    ${_out_html_file}
                COMMAND
                    ${_pythonpath} ${CMAKE_COMMAND} -E copy
                    ${_src_dir}/${G_NAME}.md
...
function(_build_addon)
    cmake_parse_arguments(
        G
        "NO_DOCS;IS_SCRIPT"
        "NAME;SRC_DIR;PYTHONPATH"
        "SOURCES;DEPENDS;OPTIONAL_DEPENDS;GRASSLIBS;DOCFILES"
        ${ARGN}
    )
    ...
    if(G_IS_SCRIPT)
        ...
        if(
            has_script_file LESS "0"
            OR NOT EXISTS ${G_SRC_DIR}/${SRC_SCRIPT_FILE}
        )
        ...
        configure_file(
            ${G_SRC_DIR}/${G_NAME}.py
            ${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/${G_NAME}${SCRIPT_EXT}
            COPYONLY
        )
        file(
        ...
        if(WIN32)
            install(
                PROGRAMS ${OUTDIR}/${GRASSAddon_ScriptDIR}/${G_NAME}.bat
                DESTINATION ${GRASSAddon_ScriptDIR}
            )
        endif()

        install(
            PROGRAMS ${OUTDIR}/${GRASSAddon_ScriptDIR}/${G_NAME}${SCRIPT_EXT}
            DESTINATION ${GRASSAddon_ScriptDIR}
        )
    ...
    if(NOT G_NO_DOCS)
        ...
        add_custom_command(
            OUTPUT ${_out_html_file}
            COMMAND
                ${CMAKE_COMMAND} -E copy ${G_SRC_DIR}/${G_NAME}.html
                ${CMAKE_CURRENT_BINARY_DIR}/${G_NAME}.html
            COMMAND
                ${G_PYTHONPATH} GISBASE=$ENV{GISBASE} GISRC=$ENV{GISRC}
                VERSION_NUMBER=${GRASS_VERSION_STRING}
                VERSION_DATE=${GRASS_VERSION_DATE} ${_execute}
                ${OUTDIR}/${install_dest}/${G_NAME}${SCRIPT_EXT}
                --html-description < ${null_device} | ${search_command}
                ${html_search_str} > ${_tmp_html_file}
            COMMAND
                MODULE_TOPDIR=$ENV{GISBASE} SOURCE_URL=${SOURCE_URL}
                VERSION_NUMBER=${GRASS_VERSION_STRING} ${PYTHON_EXECUTABLE}
                ${MKHTML_PY} ${G_NAME} > ${_out_html_file}
            COMMAND
                ${CMAKE_COMMAND} -E copy ${G_SRC_DIR}/${G_NAME}.md
                ${CMAKE_CURRENT_BINARY_DIR}/${G_NAME}.md
            COMMAND
                ${G_PYTHONPATH} GISBASE=$ENV{GISBASE} GISRC=$ENV{GISRC}
                VERSION_NUMBER=${GRASS_VERSION_STRING}
        ...
        add_custom_target(
            ${G_NAME}-docs
            ALL
            DEPENDS ${G_NAME} ${_out_html_file}
        )
        target_sources(
            ${G_NAME}-docs
            PRIVATE ${html_doc} ${md_doc} ${img_files}
        )
...
macro(find_M)
    if(UNIX AND NOT TARGET LIBM)
        ...
        if(MATH_LIBRARY)
            set_property(
                TARGET LIBM
                PROPERTY INTERFACE_LINK_LIBRARIES ${MATH_LIBRARY}
            )
        else()
