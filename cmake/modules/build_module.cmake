...
# Keep stubs updated when changing options, oneValueArgs, or multiValueArgs
...
function(build_module)
    ...
    foreach(PRIMARY_DEPEND ${G_PRIMARY_DEPENDS})
        if(NOT TARGET ${PRIMARY_DEPEND})
            message(
                STATUS
                "${G_NAME} disabled because ${PRIMARY_DEPEND} is not available"
            )
            return()
        else()
    ...
    if(G_EXE)
        ...
        elseif("${G_NAME}" MATCHES "^m[\.]")
            set_target_properties(
                ${G_NAME}
                PROPERTIES FOLDER Tools/Miscellaneous
            )
    ...
    foreach(G_DEPEND ${G_DEPENDS})
        ...
        target_include_directories(
            ${G_NAME}
            PUBLIC "$<BUILD_INTERFACE:${${G_NAME}_INCLUDE_DIRS}>"
        )
    endforeach()
    ...
    if(NOT G_EXE)
        ...
        install(
            ...
            LIBRARY DESTINATION ${GRASS_INSTALL_LIBDIR}
            ARCHIVE DESTINATION ${GRASS_INSTALL_LIBDIR}
            RUNTIME DESTINATION ${GRASS_INSTALL_BINDIR}
            INCLUDES DESTINATION ${GRASS_INSTALL_INCLUDEDIR}
        )
