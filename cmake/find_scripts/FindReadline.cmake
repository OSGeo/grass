...
foreach(component ${Readline_FIND_COMPONENTS})
    ...
    list(
        APPEND _Readline_REQUIRED_VARS
...
if(NOT Readline_FOUND)
    ...
    if(Readline_INCLUDE_DIR AND Readline_LIBRARY)
        ...
        if(EXISTS "${Readline_INCLUDE_DIR}/readline/readline.h")
            file(
                STRINGS "${Readline_INCLUDE_DIR}/readline/readline.h"
                version_major_str
                REGEX "^#define[\t ]+RL_VERSION_MAJOR[\t ]+[0-9]+"
            )
            string(
                REGEX REPLACE "^#define[\t ]+RL_VERSION_MAJOR[\t ]+([0-9]+)"
                "\\1"
                version_major
                "${version_major_str}"
            )
            ...
            string(
                REGEX REPLACE "^#define[\t ]+RL_VERSION_MINOR[\t ]+([0-9]+)"
                "\\1"
                version_minor
                "${version_minor_str}"
            )
...
if(Readline_USE_HISTORY AND NOT History_FOUND)
    ...
    find_library(History_LIBRARY NAMES history HINTS ${History_ROOT_DIR}/lib)
...
if(Readline_USE_HISTORY AND History_FOUND AND NOT TARGET Readline::History)
